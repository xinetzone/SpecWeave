...
source = ".agents/skills/retrospective-cmd/SKILL.md#7, .agents/skills/insight-cmd/SKILL.md#7, .agents/skills/export-report-cmd/SKILL.md#7, .agents/skills/atomization-cmd/SKILL.md#7, .agents/skills/atomic-commit-cmd/SKILL.md#7"

[bindings]
rules = ["../../.agents/rules/stage-guardrails.md"]
references = ["../../.agents/skills/", "../retrospective/patterns/code-patterns/structured-lightweight-logging.md"]
skills = ["retrospective-cmd", "insight-cmd", "export-report-cmd", "atomization-cmd", "atomic-commit-cmd"]
---
...
CMD-LOG 是 SpecWeave 命令集（Skill门面）的结构化执行日志规范，是项目日志体系的第三类日志（继 SG-LOG 阶段守卫日志、PDR-LOG 前置文档读取日志之后），用于追踪5大命令集执行过程中的关键节点，支持故障排查、断点续传、执行审计和事后复盘。

本规范与项目已有的 [结构化轻量日志格式](../retrospective/patterns/code-patterns/structured-lightweight-logging.md) 模式保持一致，采用统一前缀+键值对+JSON上下文的单行格式，零依赖、grep友好、机器可解析。

## 2. 适用范围

本规范适用于以下5个命令集Skill门面：
...
| 步骤 | retrospective | insight | export-report | atomization | atomic-commit |
|------|--------------|---------|---------------|-------------|---------------|
| S0 | 启动 | 启动 | 启动 | 启动 | 启动 |
| S1 | 收集事实 | 数据采集 | 验证源报告 | 源文件分析 | 检查范围 |
| S2 | 分析过程 | 趋势分析 | 准备内容 | 制定方案 | 预提交验证 |
| S3 | 提炼洞察 | 根因分析 | 格式转换 | 执行拆分 | 构建信息 |
| S4 | 生成报告 | 异常检测 | 生成文件 | 更新引用 | 执行提交 |
| S5 | 归档沉淀 | 建议生成 | 归档索引 | 收尾验证 | 结果验证 |
| S6 | — | 沉淀 | 链接验证 | 索引更新 | 推送通知 |
...
| 日志类型 | 前缀 | 适用场景 | 文档 |
|---------|------|---------|------|
| 阶段守卫日志 | SG-LOG | 开发流程阶段边界拦截、跳转审批 | [.agents/rules/stage-guardrails.md](../../.agents/rules/stage-guardrails.md) |
| 前置文档读取日志 | PDR-LOG | 必读文档读取确认、缺失告警 | [.agents/protocols/pre-document-reading.md](../../.agents/protocols/pre-document-reading.md) |
| **命令集执行日志** | **CMD-LOG** | **5大命令集Skill门面执行追踪** | **本文件** |
