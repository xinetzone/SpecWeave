...
| 文档层 | Sphinx 构建失败、链接断裂 | `docs/`（内容与构建）与 `.agents/scripts/`（检查脚本） |
...
[resolve_project_root()](../../../.agents/scripts/lib/project.py#L18-L53) 的逻辑是：
...
- [GuardrailRuntime](../../../.agents/scripts/lib/stage_guardrails/runtime.py#L83-L207)
- [BoundaryChecker](../../../.agents/scripts/lib/stage_guardrails/boundary.py#L450-L549)
- [StageStateManager](../../../.agents/scripts/lib/stage_guardrails/state/manager.py#L21-L115)
...
[parse_log_file()](../../../.agents/scripts/sg_dashboard/parser.py#L39-L77) 只会处理包含以下前缀的行：
...
[docs/tasks/docs.py](../../../docs/tasks/docs.py#L28-L53) 通过 `subprocess.run()` 调用 `sphinx-build -M`，如果系统中没有 `sphinx-build`，会直接抛出友好的 `Exit` 错误。
