...
| `.agents/` | 规范主权区、脚本实现区（面向 AI 智能体的规范与执行资产） | [AGENTS.md](../../../AGENTS.md#L49-L80) |
| `.trae/specs/` | Spec 驱动开发的过程文档 | [context-routing.md](../../../.agents/context-routing.md#L53-L55) |
...
| `docs/` | 唯一文档中心（知识库/复盘/标准/模式/Code Wiki）+ Sphinx 站点工程 | [docs/tasks/docs.py](../../../docs/tasks/docs.py#L72-L130) |
...
速查表来源见 [scripts/README.md](../../../.agents/scripts/README.md#L24-L53)。
...
| 模块 | 职责 | 关键入口 |
|---|---|---|
| `project.py` | 解析项目根、`.agents/` 和脚本目录路径 | [resolve_project_root()](../../../.agents/scripts/lib/project.py#L18-L53) |
| `cli.py` | 安全输出、CLI 通用选项与打印行为 | `lib/cli.py` |
| `atomic_write.py` | 原子写入文本和缓存 | `lib/atomic_write.py` |
| `frontmatter.py` | 统一 YAML/TOML frontmatter 解析 | [parse_frontmatter_unified()](../../../.agents/scripts/lib/frontmatter.py#L508-L536) |
...
| 模块 | 职责 | 关键入口 |
|---|---|---|
| `link_fixer/` | 断链检测与自动修复 | [fix_broken_links()](../../../.agents/scripts/lib/link_fixer/cli.py#L32-L110) |
| `spec_loader.py` | 规范渐进式加载 | [SpecLoader](../../../.agents/scripts/lib/spec_loader.py#L376-L497) |
| `stage_guardrails/` | 阶段状态机、边界校验、运行时拦截 | [StageStateManager](../../../.agents/scripts/lib/stage_guardrails/state/manager.py#L21-L115) |
...
核心数据模型见 [models.py](../../../.agents/scripts/sg_dashboard/models.py#L25-L107)，日志解析入口见 [parser.py](../../../.agents/scripts/sg_dashboard/parser.py#L29-L77)。
...
根 `docs/` 目录是仓库唯一的文档中心（OKF v0.2），知识库、复盘报告、标准规范、方法论模式与 Code Wiki 等全部长期文档资产都在其中：

- 承载全部对外可读的内容页，并通过 Sphinx 构建公开文档站
- 通过 [docs/tasks/docs.py](../../../docs/tasks/docs.py#L72-L130) 封装的 invoke 任务调用 Sphinx make-mode（`invoke html` / `invoke linkcheck` 等）
- 与 `.agents/` 的分工是：`docs/` 存放面向人类读者的长期知识资产，`.agents/` 存放面向 AI 智能体的规范与执行资产

## 模块观察结论

- `.agents/` 是智能体规范与脚本生态的主体，`docs/` 是长期文档资产主体，`apps/`、`projects/`、`vendor/` 更像承载层。
