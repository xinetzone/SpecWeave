...
type: Guide
id: "standards-readme"
title: "团队规范（Standards）"
x-toml-ref: "../../../.meta/toml/docs/tech/standards/README.toml"
---

# 团队规范（Standards）

> **文档边界说明**：成熟的AI智能体规范已归档至 `.agents/` 目录。本目录保留为人类读者的规范导航与待成熟规范暂存区。
> - 权威规范（机器可读，AI智能体必须遵循）→ [`.agents/rules/`](../../../.agents/rules/README.md)
> - 本目录（人类可读导航/暂存）→ 当前位置
...
| 规范文档 | 说明 | 权威版本位置 |
|---------|------|-------------|
| [CMD-LOG命令集执行日志规范](cmd-log-specification.md) | 5大命令集Skill门面的结构化执行日志格式、事件枚举、解析方法和实施检查清单 | [.agents/rules/cmd-log-specification.md](../../../.agents/rules/cmd-log-specification.md) |
...
执行经验形成(.agents/commands/或.agents/skills/)
    → 提炼为待成熟规范(docs/tech/standards/暂存)
    → 至少1次实际执行验证
    → 归档至.agents/对应子目录（权威版本）
    → docs/tech/standards/保留导航指针
```

## 与其他目录的关系

```
 docs/tech/standards/  ← 人类读者导航+待成熟规范暂存
└── (导航指针指向.agents/权威版本)
...
`docs/tech/references/development-standards.md` ← 开发规范（代码风格、提交规范）
