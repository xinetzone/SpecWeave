...
title: "开发规范"
x-toml-ref: "../../../.meta/toml/docs/tech/references/development-standards.toml"
---
...
7. 命名规范：文件名kebab-case、纯英文、两位及以上数字前缀（如 `NN-` 或 `NNN-`，用于章节排序或视角编号）

### 术语解释强制规范

> **来源**：LLM Token优化知识库V阶段对抗审查发现——核心术语首次出现无解释是新手阅读的最大障碍（N-01严重问题），属于"知识的诅咒"系统性偏差。

所有知识类文档（Wiki教程、最佳实践、方法论、学习笔记等）必须满足以下术语解释要求：

1. **首次出现解释**：任何非日常用语的专业术语、缩写、行业黑话，在文档正文中**第一次出现时**必须给出一句话通俗解释，格式为：`术语（一句话解释）`或`术语——一句话解释`。
   - ✅ 正确：`KV Cache（键值缓存，Transformer推理时缓存已计算token的注意力结果以避免重复计算）`
   - ❌ 错误：直接使用"KV Cache"不做解释（默认读者知道）
2. **核心术语速查表**：涉及专业领域的文档，在**文档开头部分**（前言/概述之后、正文之前）必须提供"核心术语速查表"，包含≥15个对理解本文至关重要的术语。表格格式：
   ```markdown
   | 术语 | 一句话解释 |
   |------|-----------|
   | Token | LLM处理文本的基本单位，通常是子词或字符片段 |
   ```
3. **独立术语表**：当一个知识库/系列文档包含≥3个文档时，必须在根目录提供独立的 `glossary.md` 术语表文件，汇集所有文档中的术语。
4. **解释深度原则**：术语解释使用**新手能理解的语言**，不要用另一个专业术语解释专业术语（避免"循环定义"）。

**质量门检查**：G3洞察门和V对抗门的强制检查项，不满足的文档不予通过。

### 收益表述三前提规范

> **来源**：LLM Token优化知识库V阶段对抗审查发现——无前提的收益承诺（"能省X%""提升X倍"）会严重误导决策者（C-01严重问题），属于"技术乐观主义"系统性偏差。

文档中出现的所有量化收益表述（如"成本降低X%""性能提升X倍""命中率≥X%""节省X个token"），必须同时附带以下**三个前提条件**，缺一不可：

| 前提条件 | 必须说明的内容 | 示例 |
|---------|--------------|------|
| **①适用场景** | 该收益在什么场景/业务类型/系统规模下成立？什么场景不成立？ | "适用于日调用量≥10万次的客服对话场景；创意写作/代码生成场景命中率显著降低（<20%）" |
| **②团队/环境基线** | 基于什么技能水平的团队？什么基础设施？对比的基线是什么？ | "基于有1名ML工程师的团队，已有向量数据库基础设施；对比基线为'零优化直接调用API'" |
| **③测量方式与假设** | 数据是怎么测出来的？样本量多大？有什么关键假设？ | "基于1000轮真实对话A/B测试，P95延迟变化在±5%以内；假设缓存Key设计合理且无大规模语义漂移" |

**禁止的表述方式**：
- ❌ "成本降低50-70%，吞吐量提升5-10倍"（无任何前提）
- ❌ "缓存命中率目标≥50%"（不说明什么场景能达到）
- ❌ "35种技术预期token节省20-90%"（范围太大等于没说）

**正确的表述示例**：
> 语义缓存在**高频重复查询的客服场景**下（日调用量≥10万次，问题重复率≥40%），**对已有向量数据库基础设施的中大型团队**，可实现**40-60%的token节省**（基于1000轮A/B测试，P95延迟上升<10%）；在创意写作等低重复场景效果有限（<10%）。

**标注要求**：所有经验估算数据必须明确标注"行业经验估算，实际效果以实测为准"或类似免责声明，避免读者将经验值当作确定承诺。

**质量门检查**：G2原理门和G3洞察门的强制检查项，不满足的文档不予通过。
