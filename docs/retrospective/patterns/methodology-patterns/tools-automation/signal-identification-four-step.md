...
| 敏感信息硬编码 | 字符串匹配 `sk-` 前缀、PEM 头部、`password=` 赋值 |
...
> 详细消歧策略见 [ast-disambiguation-five-methods.md](../../code-patterns/ast-disambiguation-five-methods.md)
...
2. **分层防御**：pre-commit 做快速增量检查，CI 做全量深度扫描，Code Review 做最终兜底
