...
tags: ["产品学习", "竞品分析", "Spec Mode", "任务模板", "供应商研究", "知识库", "Mermaid图表", "结构化笔记", "外部研究", "任务分解", "云原生", "沙箱技术"]
trigger_conditions: ["需要系统性学习外部产品/供应商方案", "竞品/替代方案深度分析", "产品页内容结构化学习", "AI基础设施/API服务调研", "供应商产品能力评估", "建立知识库产品学习笔记", "云原生/安全沙箱/基础设施类产品分析"]
problem_solved: "对外部产品（尤其是ToB/AI/云服务/基础设施类产品）进行系统性学习时，任务分解无章法，容易遗漏关键分析维度（如技术架构、应用场景、竞争格局、行业趋势），产出的学习笔记结构不一致、深度参差不齐。本模板提供经过验证的标准化任务分解，确保产品学习全面、深入、可复用。"
validation_count: 2
reuse_count: 0
---
...
> **验证次数**：2次（火山引擎SearchInfinity搜索产品 + 火山引擎Sandbox云原生基础设施产品）；部分步骤在之前的Viking AI、贝锐AI等产品学习中也有验证
...
2. 提取并结构化以下信息：
   ...
   - 客户案例/合作伙伴/信任背书元素
3. 输出为结构化JSON格式，字段分类清晰
4. 建议：如果需要分析UX，一定要用集成浏览器MCP提取CTA按钮等交互元素细节
...
当前版本为L1（1次完整验证），后续可在以下方向迭代：
1. 增加更多实战案例（3-5个不同类型产品验证，向L2演进）
2. 针对不同产品类型（API产品/无代码/开源商业/SaaS）制作差异化模板
3. 补充Mermaid图表模板片段，直接复用减少绘图时间
4. 增加竞品横向对比分析的附加章节模板
