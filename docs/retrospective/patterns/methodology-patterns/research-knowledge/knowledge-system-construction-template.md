...
> **⚠️ 目录职责规范（v1.2新增，v1.3强化）**：`.trae/specs/`目录**只存放规划三件套**（spec.md/tasks.md/checklist.md）。执行阶段产出的中间分析文档（事实收集、决策分析、挑战分析、方法论分析、洞察草稿）必须存放于正式报告目录下的`supporting-analysis/`子目录，最终报告（README.md）需在"分析过程溯源"章节明确链接。禁止将执行产物放入spec目录——规划与产出的严格分离是可追溯性的基础。
>
> **🔒 产出路径强制原则（v1.3新增）**：tasks.md中**每个Task必须包含Output字段**，明确指定产出物的目标文件路径。禁止依赖执行者的"默认判断"——在无显式路径约束时，所有执行者（人或AI）都会默认将文件放在当前工作目录（即spec目录），这是工具和认知系统的普遍行为，不是疏忽。路径规范是任务定义的必要组成部分。
>
> **🔍 Spec目录纯净度检查（v1.3新增）**：项目完成收尾时，必须执行结构验证：`ls .trae/specs/<project-name>/` 应恰好输出3个文件（spec.md/tasks.md/checklist.md）。这是架构不变量——它不仅是目录美观问题，更是：①可预测的工具脚本假设；②防止scope creep的物理屏障；③项目状态的可扫描信号（3文件=已规划，>3文件=执行中或产物错位）。
...
- **收尾双重验证（v1.3新增）**：
  - 内容验证：check-links.py检查链接有效性、文件名规范、frontmatter格式
  - 结构验证：`ls .trae/specs/<project>/` 确认恰好3个文件（spec.md/tasks.md/checklist.md）；产出物目录结构符合规范；所有Task的Output路径下文件实际存在
  - 两类验证缺一不可——内容验证确保"文件里的东西是对的"，结构验证确保"对的东西在对的地方"
