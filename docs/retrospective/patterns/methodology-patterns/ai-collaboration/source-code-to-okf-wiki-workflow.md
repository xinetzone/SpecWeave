...
2. 萃取通用prompt模板（见下方）
3. 补充反模式和迁移验证
4. 模式存入 `docs/retrospective/patterns/methodology-patterns/ai-collaboration/`
5. 更新模式库索引
...
任务完成后，通用模板和workflow留在`.trae/specs/`规划目录中，不沉淀到模式库。后果：下次做同类任务时无法复用，经验浪费。**正确做法**：C阶段将通用模式迁移到 `docs/retrospective/patterns/` 对应分类目录。
