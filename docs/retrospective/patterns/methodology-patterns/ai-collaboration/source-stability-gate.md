...
x-toml-ref: "../../../../../.meta/toml/docs/retrospective/patterns/methodology-patterns/ai-collaboration/source-stability-gate.toml"
rules: []
references: []
skills:
...
1. **注册为 git submodule**：固定到具体 tag/commit，纳入 vendor/ 管理（推荐用于第三方源码，参见 [VENDOR-INTEGRATION.md](../../../../../.agents/VENDOR-INTEGRATION.md)）
...
选定选项1/2后，固定到哪个版本坐标（release tag 还是文档生成时 commit）不是"哪个版本号更新"或"哪个更接近生成时点"的直觉判断，而是**集合论判定**：文档引用集合 ∩ 版本变更集合 = ∅。四步执行：

**前置条件**："生成时 commit"须在信源准备阶段记录（文档生成时临时克隆的 HEAD，登记于 facts.md 或 frontmatter）。若当时未记录，无法执行下述差异比对，只能将候选 tag 检出后逐一验证引用集合中全部文件存在（存在性兜底），其保障强度低于交集判定。

1. **血缘判定**：`git merge-base --is-ancestor <候选tag-commit> <生成时commit>` 确认候选 tag 是否为文档生成时 commit 的祖先（祖先关系保证 tag 内容不超出文档已验证范围）。若候选 tag 不是祖先（tag 晚于文档生成发布），两版本间 diff 将包含生成后变更，交集大概率为非空，按第4步选生成时 commit 或改选更早的 tag
2. **变更文件清单**：`git diff --name-status <候选tag-commit> <生成时commit>` 列出两版本间全部变更文件
3. **求交集**：将变更文件清单与文档引用集合（全部 `file:///` 链接目标 + frontmatter 信源字段）求交集
4. **决策**：
   - 交集为空 → 选 release tag（可辨识、可审计），并记录 tag 对应的 commit hash（轻量 tag 可能被上游移动重打，hash 才是不可变坐标）
   - 交集非空 → 选生成时 commit，并记录差异文件清单（文档引用的文件在两版本间有差异，固定 release tag 会引入文档未验证的内容）

**实际案例**（veadk-python，2026-08-29）：候选为生成时 commit `7bd1207`（main 尖端，无 tag）与最新 tag `1.0.10`（`ffbf295`，早 16 小时）。血缘判定确认 `ffbf295` 是 `7bd1207` 的祖先（间隔 5 个 commit）；变更文件集中于 `frontend/`、`veadk/webui/` 构建产物与 CLI 前端集成；Wiki 引用集合零命中这些变更文件 → 交集为空，选定 `1.0.10`。

**为什么不用两种直觉**："固定生成时 commit"把非发布版本锁死且丧失 tag 的可辨识性；"固定最新 release"可能引入文档未覆盖的内容或丢失文档引用的文件。正确问题只有一个：**文档引用的文件集合在两个版本间是否有差异**。
...
<!-- changelog -->
- 2026-09-02 | docs | v2.4：闭环 veadk 里程碑 E 阶段「待入库」增量——tag 选型子步骤（洞察 I-3）入库第二步：血缘判定（`merge-base --is-ancestor`）→ 变更文件清单（`diff --name-status`）→ 与文档引用集合求交集 → 交集为空选 release tag、非空选生成时 commit 并记录差异清单；附 veadk 案例实际决策数据与两种直觉的反常识说明；与技能侧 source-code-to-okf-wiki v1.3.0 G0 预检判据对齐。V 阶段 4 视角审查采纳 3 条修正：删除"漂移风险归零"自利归因表述、补充非祖先情形处理与生成时 commit 前置条件、决策项补充记录 tag 对应 commit hash
