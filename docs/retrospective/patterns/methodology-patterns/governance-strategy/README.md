...
| [Harness架构分层模式](./harness-architecture-layered-model.md) | Harness是模型接入真实世界之前的运行骨架，包含Workflow/Tools/Permissions/Memory/Evaluation五层架构，最小核心骨架为chooseTool→runTool→updateContext→saveArtifacts→shouldStop。核心反常识结论：模型只是引擎，Harness才是整辆车。操作步骤包括Workflow设计（打破单轮问答循环）、文件系统记忆（冷热分层）、权限与恢复设计（三级权限）。反模式包括参数崇拜、上下文窗口越大越好、单轮问答逻辑。迁移验证覆盖智能体测试平台、文档自动化工具、数据分析平台、传统软件开发流程。 | L1 | `Harness` `Agent架构` `运行时设计` |
...
| [分层链式规格：AI 编程工作流的阶段分离与链式传递](./layered-chained-spec.md) | 分层链式规格：AI 编程工作流的阶段分离与链式传递 | L1.5 | `sdd` `spec-driven-development` `ai-programming` |
...
| [分层修复验证法（Layered Repair Verification）](./layered-repair-verification.md) | 分层修复验证法（Layered Repair Verification） | L1 | `分层修复` `验证闭环` `分层错误屏蔽` |
| [Learn-Validate-Adopt：外部标准采用三步法](./learn-validate-adopt.md) | Learn-Validate-Adopt：外部标准采用三步法 | L1 |  |
| [台账存量审计：注册清单与磁盘存量的一致性审计法](./ledger-stock-consistency-audit.md) | 台账存量审计：注册清单与磁盘存量的一致性审计法 | L2 | `ledger` `inventory-audit` `shadow-assets` |
...
| [AI Agent 平台选型 9 维度评分卡（可直接套用）](./P-AGENT-SELECT-001-agent-platform-selection-scorecard.md) | 企业级 AI Agent 平台/框架选型的 9 维度加权评分卡（含权重、1-5分标准、加权公式、示例打分、决策建议）。复制本文件填写候选平台分数，计算加权总分直接排序。可用于 Dify/Coze/LangGraph/LlamaIndex/AutoGen/AgentKit 等任何 Agent 平台或 AI 基础设施选型。 | - | `选型评估` `Agent平台` `决策工具` |
| [智能体从Demo到生产的12项检查清单](./P-DEMO-TO-PROD-003-demo-to-prod-checklist.md) | 智能体从Demo到生产的12项检查清单 | L1 |  |
...
| [性能优化五步法：测量→诊断→优化→验证→萃取](./performance-optimization-five-step-method.md) | 性能优化五步法：测量→诊断→优化→验证→萃取 | L1-实验性 | `performance-optimization` `five-step-method` `measure-diagnose-optimize-verify-extract` |
...
| [插件桥接规范集成法](./plugin-bridge-standard-integration.md) | 插件桥接规范集成法 | L1 | `agent-platform` `workspace-governance` `plugin` |
...
| [Python 标准库系统优化四步法：基线采集→能力映射→量化验证→测试兜底](./python-stdlib-systematic-optimization-four-steps.md) | Python 标准库系统优化四步法：基线采集→能力映射→量化验证→测试兜底 | L1-draft | `python` `stdlib` `code-coverage` |
...
| [风格锚定一致性保证法](./style-anchoring-consistency.md) | 风格锚定一致性保证法 | L1 |  |
...
| [子模块目录约定模式（Submodule Directory Convention）](./submodule-directory-convention.md) | 子模块目录约定模式（Submodule Directory Convention） | - |  |
| [对称目录结构设计（Symmetric Directory Structure）](./symmetric-directory-structure.md) | 本目录是 <区域名称> 区域的规范元数据容器。 | L3 | `directory-structure` `architecture-symmetry` `governance` |
...
| [三态决策模式：做/明确不做/推迟（Do/Explicitly-No/Defer）](./three-state-decision.md) | 三态决策模式：做/明确不做/推迟（Do/Explicitly-No/Defer） | L1 | `decision-making` `governance` `retrospective` |
