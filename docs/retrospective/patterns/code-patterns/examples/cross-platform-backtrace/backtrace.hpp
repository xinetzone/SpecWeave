...
/*!
 * \file backtrace.hpp
 * \brief Cross-platform stack backtrace utility for memory leak diagnosis.
 *
 * Supports Windows (DbgHelp) and Linux (execinfo.h).
 * Compile with DEMO_ENABLE_BACKTRACE to enable real backtrace; otherwise
 * returns a friendly placeholder string (no crash, no link errors).
 *
 * Usage:
 *   std::string bt = backtrace::GetBacktrace(skip_frames);
...
#if defined(_WIN32) && defined(DEMO_ENABLE_BACKTRACE)
...
#if defined(__linux__) && defined(DEMO_ENABLE_BACKTRACE)
  #include <execinfo.h>
#endif
...
#elif defined(__linux__) && defined(DEMO_ENABLE_BACKTRACE)
  void* frames[kMaxFrames];
...
#else
  oss << "  (backtrace not available: rebuild with DEMO_ENABLE_BACKTRACE=ON)\n";
  (void)skip_frames;
#endif
...
#define BT_GET()        ::backtrace::GetBacktrace(2)
#define BT_GET_SKIP(s)  ::backtrace::GetBacktrace(2 + (s))
#define BT_PRINT()      ::backtrace::PrintBacktrace(2)
