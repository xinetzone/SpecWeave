...
## 反模式（不要这么做）
- ❌ 硬编码单一绝对路径（如`<USER_HOME>\anaconda3`）：换机器立即失效，维护成本极高
...
## 参考实现
- PowerShell实现：[NativeBuild.psm1](../../../../.agents/scripts/lib/NativeBuild.psm1) 中 `Get-CondaRoots`、`Find-CondaEnvPython`、`Find-VisualStudio` 函数
