...
| tag 选型子步骤（✅ 已入库 2026-09-02，模式 v2.4） | 祖先判定 → 变更文件清单 → 与引用集合求交集 → 交集为空选 tag；入库时经 V 阶段 4 视角审查采纳 3 条修正（删除漂移归零自利归因、补充非祖先情形与前置条件、补充记录 tag 对应 commit hash） | 洞察 I-3，F-011/F-012/F-013 |
