...
type: "Report"
description: "对 docs/ 文档中心（3491文件/114.31MB）的首次全面复盘：33条客观事实覆盖规模结构、质量门禁、导航索引、跨区演进四维；3条核心洞察（生成-消费断裂、双体系引用负债、门禁报警-灭火断层）；萃取1个L1.5模式（生成-登记同步法）；6项原子行动项；经4视角10条对抗审查修正"
status: "stable"
source: "seven-concepts-cmd session sc-20260831-docs-retrospective（复盘对象：d:\\AI/docs 全目录）"
milestone-name: "docs 文档中心全面复盘与导航债务登记"
time-range: "2026-08-31（单日完成 R→I→E→V→导出）"
methodology: "七概念方法论（R→I→E→V链路，standard深度，4视角对抗审查）"
quality-gates:
  G1: "事实无因果词 ✅（33条事实，全部可命令复现）"
  G2: "洞察四元组完整 ✅（3条洞察，均引用F编号）"
  G3: "模式可迁移验证 ✅（生成-登记同步法，L1.5）"
  V: "4视角对抗审查 ✅（10条意见，采纳10条修正）"
...
> **方法论编排**：七概念 R→I→E→V 链路（里程碑复盘场景，standard 深度，4 视角对抗审查；C 原子提交由并行执行会话完成，见第六章执行状态）
...
> G1 质量门：✅ 通过（33 条事实均为可验证的客观陈述，无"因为/所以/导致/错误/失误"等因果推断词；关键数字均附采集命令）
...
| F-003 | 11 个一级子目录中，`knowledge/` 含 2,436 文件/30.86 MB，`_build/` 含 945 文件/82.43 MB，两者合计占总体积 99.1%；`_build/` 为 Sphinx 构建产物目录（有意排除于版本控制，见 F-005），非内容债务 |
...
| F-012a | check-toctrees.py 的"未收录"判据为"所有非 readme 的 .md 必须被某 toctree 收录"（[check-toctrees.py](../../../../scripts/check-toctrees.py) 源码 `all_md_under` + `check_unreachable`）；该判据未区分"应收录而未收录"与"有意不收录"（模板/归档/草稿），故 2,187 为**门禁原始检出数**，实际债务数需经判据验证后确定 |
...
| F-033 | 本项目存在门禁工具假阳性先例：[veadk-a3-a6-closure-retrospective-20260829.md](veadk-a3-a6-closure-retrospective-20260829.md) 记录 GATE-SPS 工具锚点假阳性缺陷，修复 568 个存量误报并新增 2 个回归测试（提交 `ba8272c6`） |
...
- **陈述**：docs 体系的内容生产能力已规模化（批量生成 OKF 知识包、Wiki 转换），而导航登记（toctree 收录、索引表格、frontmatter 补全）仍依赖手工跟随维护；两者速率失配形成结构性债务，且债务集中于批量生成产物。
- **证据**：F-010（2,245 处导航问题）、F-012（2,187 处未收录，聚集于批量生成产物 best-practices/okf-bundles/operations）、F-012a（该数字为门禁原始检出数，判据未区分"应收录"与"有意不收录"）、F-013/F-014（1,604 处 frontmatter 违规，85.9% 在 learning）、F-004（learning 占 2,131 文件）、F-017（10 个编号目录整体未收录）、F-020（4 份报告漏登记于表格）。
- **反常识**：默认假设是"文档质量问题=内容写得不好"。实测相反——内容层质量高（UTF-8 门禁 100% 通过，F-009；OKF 包 frontmatter 完整，F-029；Sphinx 警告已清零，F-026），失效的是连接内容与可发现性的元数据层。批量生成工具产出内容但不产出导航登记，消费者（读者/构建器）看不到未被收录的文件——它们存在但不可达。
- **行动**：为批量生成管线（OKF 包生成、Wiki 转换、报告归档）增加"导航登记"后置强制步骤——生成即登记（见第四章模式 E-1）；将 check-toctrees 从事后 CI 拦截前移为生成时伴随校验。

### 洞察 I-2：双文档体系并存使路径引用从"资产"转为"负债"，迁移的引用修复成本显著高于文件移动成本
...
- **反常识**：默认假设是"迁移=移动文件"，一次提交即可完成。实测迁移提交之后仍需独立的断链修复提交（F-025），且引用修复不随迁移提交终结：迁移前的目录结构仍被索引文件引用（F-018 的 `reports/milestone` 等 4 路径是迁移前旧结构），每次新报告沿用旧路径都会使债务复发。引用修复的工作量与迁移文件数不成正比，而与存量引用密度成正比。
...
- **陈述**：docs 具备完整三门禁体系（脚本+invoke 封装+CI 拦截），门禁确实能检出问题（原始检出合计 3,849 处），但检出的问题没有对应的修复行动流——门禁在"报警"，无人"灭火"。
- **证据**：F-007（三门禁脚本与 gates.py 封装存在且可运行）、F-009（utf8 门禁通过，证明门禁可运行且内容层达标）、F-010/F-013（另两道门禁检出 2,245+1,604 处问题）、F-012a/F-033（检出数含判据局限与假阳性先例，实际债务数需判据验证）、F-026（Sphinx 警告 30→0 证明集中修复能力存在）、F-008（log.md 无修复记录）、F-031（最近 20 条提交无导航修复类提交）。
- **反常识**：默认假设是"建立质量门禁=质量有保障"。实测门禁只是感知层：30 条 Sphinx 警告能被集中清零（F-026），说明修复能力与组织意愿都存在；但门禁检出问题悬置无修复提交，说明缺的不是能力而是"门禁结果→修复台账→批量修复→复验归零"的闭环管道。门禁结果若不消费，会退化为被忽略的背景噪音。
- **行动**：建立门禁问题台账并分批消费：先修复 7 处断链（ACT-1，确定性最高），再对未收录问题先做判据验证（区分"应收录"与"有意不收录"，见 F-012a）后分流处理（ACT-2），1,452 处缺 type 用脚本按目录规则批量推断补全（ACT-3）；每批修复在 log.md 留痕。
...
**成熟度**：L1.5（同谱系双案例：案例 1 为 docs OKF v0.2 规范化改造，见 [okf-wiki-conversion-milestone-20260828.md](okf-wiki-conversion-milestone-20260828.md)，该报告洞察 I-1 已识别"导航结构瓶颈"；案例 2 为本报告全量审计，量化确认债务主体为批量生成物的导航缺登记。两案例同仓库、间隔 3 天、案例 2 审计对象包含案例 1 产物，属同谱系互证而非完全独立；证据以失败侧为主，尚无正向执行记录。待第三个跨项目独立案例正向验证（门禁增量=0）后升级 L2。）

**触发场景**：
- **适用于**：批量生成结构化内容（知识包、Wiki、报告、代码骨架）且内容需通过导航层（toctree/索引表/注册表）被消费的场景；前提是生成过程可脚本化（步骤 1 的"清单随产"依赖管线可编程）
- **不适用于**：单文件手工创作（登记成本低于机制成本）；导航层与内容同文件自包含的场景（如单页应用）；生成过程完全人工、无管线可挂载登记步骤的场景
