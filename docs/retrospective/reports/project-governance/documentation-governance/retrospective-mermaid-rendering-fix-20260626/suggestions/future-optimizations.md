...
| 模板化预防 | 在智能体生成 Mermaid 的系统提示词中强制注入五规则；为各类工作流生成的 Mermaid 图表提供专用模板；开发 Mermaid 代码片段生成器 | 从生产端杜绝问题，而非仅在检测端拦截 | ⏳ 待执行：提示词注入与专用模板尚未实施 |
...
当前双防线中，**检测端已达到生产可用水平**（10种图表类型分发检测、7种节点形状、14种箭头、dry-run预览、CI门禁、自动修复51个代码块）；**预防端**提示词注入与专用模板待完善。AST级别校验作为终极方案保留，待Node.js工具链引入后实施。
