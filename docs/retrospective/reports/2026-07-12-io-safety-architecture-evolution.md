...
创建 [atomic_write.py](../../../.agents/scripts/lib/atomic_write.py)，解决核心原子性问题：
...
创建 [io_safety.py](../../../.agents/scripts/lib/io_safety.py)，在 atomic_write 之上提供**面向开发者的高层组合API**：
...
- [lib/__init__.py](../../../.agents/scripts/lib/__init__.py) → 统一门面，支持 `from lib import staged_timer, retry_on_lock, atomic_write_bytes`

**冗余清理**（本次优化收尾）：
- 移除 [spec_loader.py](../../../.agents/scripts/lib/spec_loader.py) 中未使用的 `_atomic_replace_with_retry` 向后兼容薄包装器
...
| 测试文件 | 用例数 | 覆盖场景 |
|----------|--------|----------|
| [test_atomic_write.py](../../../.agents/scripts/tests/test_atomic_write.py) | 70个 | 基础写入、唯一命名、stale清理、重试、并发、fsync、二进制边缘、深层路径、原子编辑、快速顺序写入 |
| [test_windows_file_lock.py](../../../.agents/scripts/tests/test_windows_file_lock.py) | 7个 | **真实msvcrt.locking加锁**、锁超时清理、脉冲锁、装饰器、失败日志、便捷API |
| [test_spec_loader.py](../../../.agents/scripts/tests/test_spec_loader.py) | 53个 | 分层加载、磁盘缓存、mtime失效、并发保存、原子重试（间接覆盖atomic_write） |
