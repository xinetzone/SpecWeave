...
source: "MDI项目完整复盘唯一权威文档（合并02/03/04/05/06/07/08：阶段一+阶段二过程分析+核心洞察+模式沉淀+后续行动计划+项目结论+导出状态）"
...
> 本文档是MDI项目复盘的唯一权威来源，合并了阶段一过程分析、阶段一洞察、阶段二原子化战役复盘、后续行动计划、项目结论、导出概览的全部内容，包含阶段一/二过程分析、11个核心洞察、模式沉淀、量化数据、根因分析、后续行动计划、项目结论与导出状态。
...
阶段一识别的不足之处（MCP未深度集成、CLI测试生成器缺失、Jest生成器简陋、双向转换未实现）已整理为结构化的改进建议（P0/P1/P2优先级+Gantt+ROI），见本文"后续行动计划"章节；异常项检测与状态跟踪见本文"异常检测摘要"章节。
...
阶段二完成P1-High优先级6个核心文件拆分：versioning.py(872→5模块)、validator.py(639→9模块)、forum-bot.py(1174→13模块，薄入口+延迟导入)、generate-sg-dashboard.py(863→9模块)、link_fixer.py(958→11模块)、vendor.py(985→重构整合)。额外完成check-skill-quality、check-spec-adoption、analyze-xlsx-test-report等文件拆分。战役成果：🟠橙色高风险区清零（14→0），安全文件194→286+，159+测试全部通过无回归。

剩余18个黄色预警文件主要是测试文件和500-600行小文件，属于P1-Low/P1-Watch。

## 后续行动计划（P0/P1/P2）

基于 `module-size-bug-correlation`、`semi-structured-parsing-complexity-budget`、`mvp-unvalidated-code-debt` 三个模式制定。

### 🔴 P0高优先级（核心质量风险）

| # | 改进项 | 类别 | 对应模式 | 验收标准 | 工时 |
|---|-------|------|---------|---------|------|
| 1 | 拆分parser.py(1465行) | 代码结构 | module-size-bug + parsing-complexity | 每文件<500行，259测试全过，3案例输出一致 | ~2h |
| 2 | 未验证代码STATUS标记 | 技术债务 | unvalidated-code-debt | mcp/jest/graphql有明确STATUS标记 | ~15m |
| 3 | Jest生成器对齐pytest_gen | 功能完善 | unvalidated-code-debt | Jest用例含example/checklist，功能达pytest 90%+ | ~3h |
...
| # | 改进项 | 对应模式 | 验收标准 | 工时 |
|---|-------|---------|---------|------|
| 4 | Parser边界case测试(10→20+) | parsing-complexity | 新增≥10边界测试，全过 | ~1h |
| 5 | MCP验证或删代码决策 | unvalidated-code-debt | A)有验证案例 B)删942行代码 | A:4h/B:20m |
| 6 | CLI专用测试生成器 | 原有action | file-cli.md生成可执行CLI测试 | ~2h |

**代码结构（剩余5个核心文件）**：lib/__init__.py(627)、lib/patterns.py(534)、boundary.py(601)、runtime.py(597)、pattern-maturity.py(603)，总计约3.5小时。

### 🟡 P2低优先级（体验优化）

- 补全GraphQL验证或标记实验性（A:2h/B:10m）
- OpenAPI→MDI反向转换（~4h）
- 6个剩余黄色预警脚本拆分/标记LEGACY（~3h）
- CI文件大小门禁（>800告警，>1200阻断）（~30m）
- Parser复杂度预算checklist（~20m）

### 拆分验收通用标准

1. ✅ 拆分后每个文件 <500行（橙色文件<400行）
2. ✅ 所有现有单元测试通过
3. ✅ 端到端验证案例输出与拆分前一致
4. ✅ 单一职责原则，非机械切分
5. ✅ 更新__init__.py导出，从ALLOWLIST移除（如有）
...
| P1 | ~10.5h | 偿还功能债务+核心结构优化 |
| P2 | ~9h | 剩余优化+流程固化 |
| **总计** | **~24.75h** | 质量提升、债务清零、踩坑率降低 |

P0的5小时投入可避免未来10-20小时Bug排查，ROI > 2:1。
...
| 上一章 | 目录 | 下一章 | 相关文档 |
|--------|------|--------|---------|
| [01-phase1-facts.md](01-phase1-facts.md) | [README.md](README.md) | 返回 [README.md](README.md) | [MDI模式应用指南](../../../../../.agents/scripts/mdi/PATTERN-APPLICATION.md) |

## Changelog

<!-- changelog -->
- 2026-07-03 | docs | v6.0：合并07-improvement-recommendations.md——新增后续行动计划（P0/P1/P2）+拆分验收标准+ROI估算，成为包含事实+洞察+行动的完整复盘唯一权威文档
