...
| 指标 | 初始复盘值 | insight-cmd校正值 | 阶段二完成后 | P1完成后 | P2完成 | 说明 |
|------|-----------|-------------------|-------------|---------|---------|------|
| 核心代码行数 | 8,501行 | **8,970行** | 8,970行 | 8,970行 | 8,970行 | 行数不变，结构优化 |
| Python文件总数 | - | 223个 | **304个** | **314个** | **391个** | +168个文件（原子化拆分+测试拆分+CLI lib子包） |
| 🟢 安全文件（<500行） | - | 194个 | **286+个** | **297个** | **388个** | +194个安全文件 |
| 🟠 高风险文件（>800行） | - | 14个 | **0个** | **0个** | **0个** | 阶段二清零，P1/P2保持清零 |
| 🟡 黄色预警文件（500-800行） | - | 15个 | - | **15个** | **2个** | boundary.py(601)+runtime.py(597)，核心模块，内聚度高 |
| 🔴 ALLOWLIST红色文件 | - | - | - | 1个 | **1个** | mermaid.py(1228行)，历史遗留待重构 |
| 测试代码行数 | 2,308行 | **3,368行** | 3,368行 | **~3,500行** | **~3,800行** | 含CLI工具模块测试+边界case扩充 |
| Profile数量 | 3个 | **5个** | 5个 | 5个 | 5个 | webapi/skill/cli/graphql/mcp |
| 重构相关单元测试 | - | - | **159+个** | **191个** | **513个** | 全部通过，无回归 |
| 代码/测试比 | - | **1:0.38** | 1:0.38 | **1:0.39** | **1:0.42** | 测试代码占核心代码42% |
| .agents/文档原子化 | - | - | **17→88个** | 88个 | 88个 | 所有文档<300行 |
| 本地链接验证 | - | - | **2,081个** | 2,081个 | 2,081个 | 330个MD文件，100%有效 |
...
✅ **P0+P1+P2全部完成（含1项future决策）**：🟠橙色高风险区持续清零，🟡黄色预警从15→2个（核心模块保留），513个测试全部通过。
