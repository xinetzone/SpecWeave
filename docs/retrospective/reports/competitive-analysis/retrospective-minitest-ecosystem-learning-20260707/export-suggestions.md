...
| **P2** | 整合阶段信息取舍逻辑文档化 | 最终报告产出后追加integration-notes.md，记录：<br>• 哪些发现因重叠被合并<br>• 哪些细节因粒度问题被降级/省略<br>• 哪些判断存在不确定性/需要验证<br>• 明确定义"洞察升级标准"（跨模块共性、反直觉发现、设计模式识别、风险信号） | 低投入（每次任务后追加记录）、中收益（将隐性整合知识显性化，便于后续同类任务复用） | 建议执行 |
