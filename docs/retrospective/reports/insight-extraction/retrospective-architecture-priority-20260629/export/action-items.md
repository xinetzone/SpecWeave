...
6. ✅ **完成剩余指令集SKILL化**：5个命令集Skill全部完成
7. ✅ **实施 P0模块3（Agent Onboarding协议）**：已创建 `.agents/protocols/onboarding-protocol.md`
8. **沉淀 6 个可复用模式到模式库**：P-ARCH-001（渐进式披露架构）已通过 `.agents/capabilities/ARCHITECTURE.md` 落地为正式规范；其余5个模式（Markdown即接口、瓶颈优先重构法、不重构清单、架构决策三角验证、元能力依赖倒置）待沉淀至 `docs/retrospective/patterns/`
9. 第一批5个高频脚本Skill化
