...
| 编辑-验证分离模式 | 将文档编辑分为结构编辑和内容验证两阶段，Step 5覆盖7类可复用资产（模式/检查清单/模板/脚本/索引/规范/导航）+9项增强验证，配套[edit-verify-workflow-template.md](../templates/edit-verify-workflow-template.md)可直接套用 | 所有文档编辑/重构任务，尤其是AI辅助的结构化编辑和可复用资产的创建/更新 |
