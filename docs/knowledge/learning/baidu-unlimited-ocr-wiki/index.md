...
| 角色 | 典型痛点 | 建议关注章节 |
|------|---------|-------------|
| **AI工程师** | 长文档处理成本高、速度慢、长序列OCR精度下降 | 全部章节，尤其[01-核心架构](/concepts/01-core-architecture.md)、[03-快速上手](/concepts/03-quick-start.md) |
| **OCR研究者** | 探索文档理解新架构、注意力机制改进方向 | [01-核心架构](/concepts/01-core-architecture.md)、[02-性能数据](/references/02-performance-data.md)、[05-架构启示](/concepts/05-architecture-insights.md) |
| **架构设计者** | 寻找小模型超越大模型的路径、专用模型架构设计思路 | [01-核心架构](/concepts/01-core-architecture.md)、[02-性能数据](/references/02-performance-data.md)、[05-架构启示](/concepts/05-architecture-insights.md)、[06-迁移模式](/concepts/06-transferable-patterns.md) |
| **Agent开发者** | 多智能体长上下文管理、RAG与文档处理集成 | [05-架构启示](/concepts/05-architecture-insights.md)、[06-迁移模式](/concepts/06-transferable-patterns.md)、[07-SpecWeave启示](/concepts/07-specweave-implications.md) |
| **技术学习者** | 理解AI架构创新方法论、学习技术传播策略 | 全部章节，尤其[05-架构启示](/concepts/05-architecture-insights.md)、[08-总结FAQ](/references/08-summary-faq.md) |
...
| 章节 | 标题 | 内容概要 | 难度 |
|------|------|---------|------|
| 00 | [概述](/index.md)（当前页） | 背景痛点、项目简介、核心特性、R-SWA架构图、阅读路径 | ⭐ |
| 01 | [核心架构与设计理念](/concepts/01-core-architecture.md) | R-SWA非对称注意力原理、DeepEncoder视觉编码、固定KV cache、免费午餐解读 | ⭐⭐ |
| 02 | [性能数据与基准测试](/references/02-performance-data.md) | OmniDocBench基准对比、长文档表现、推理效率TPS、参数效率反差解读 | ⭐⭐ |
| 03 | [快速上手指南](/concepts/03-quick-start.md) | 环境要求、gundam/base双图像模式、Transformers/SGLang/vLLM三种部署、infer.py批量推理、选型建议 | ⭐ |
| 04 | [局限性与风险提示](/references/04-limitations-risks.md) | 5大局限性分析、项目成熟度评估、适用场景建议、风险提示 | ⭐⭐ |
| 05 | [架构创新深度启示](/concepts/05-architecture-insights.md) | 归纳偏置力量、"软遗忘"哲学、静态/动态信息分区、视觉token不稀释原理 | ⭐⭐⭐⭐ |
| 06 | [可迁移模式与行业启示](/concepts/06-transferable-patterns.md) | MoE效率哲学、小模型路线、R-SWA迁移性分析（代码/客服等场景）、技术传播模式 | ⭐⭐⭐⭐ |
| 07 | [对SpecWeave的可行动启示](/concepts/07-specweave-implications.md) | 规范前置+对话滑窗中间件、文档编码器+按需检索机制 | ⭐⭐⭐ |
| 08 | [总结与常见问题](/references/08-summary-faq.md) | 核心要点回顾、速查表、7个FAQ、延伸阅读 | ⭐ |
...
```
1. 读完当前概述了解项目核心价值
2. 直接跳转到[快速上手指南](/concepts/03-quick-start.md)安装环境、运行示例
...
00 → 01 → 02 → 05 → 06
```
1. 理解[核心架构与设计理念](/concepts/01-core-architecture.md)，掌握"抄书注意力"本质
2. 通过[性能数据与基准测试](/references/02-performance-data.md)验证技术效果，理解500M打败235B的原因
3. 深入[架构创新深度启示](/concepts/05-architecture-insights.md)，理解归纳偏置和软遗忘哲学
4. 最后通过[可迁移模式与行业启示](/concepts/06-transferable-patterns.md)萃取可复用的架构设计模式
