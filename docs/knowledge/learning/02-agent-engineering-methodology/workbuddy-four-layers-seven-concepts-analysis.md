...
vetted: true
fact_checked: true
tags: ["七概念", "Agent工程", "Context Engineering", "Harness Engineering", "Loop Engineering", "四层工程范式", "事实核查通过"]
---
# 七概念框架视角下的WorkBuddy四层工程深度分析报告

## 执行摘要

本报告以SpecWeave七概念方法论框架（R-复盘、I-洞察、E-萃取、C-原子提交、A-原子化、F-第一性原理、V-对抗性审查）为透镜，对《从 Prompt 到 Loop：四层工程打造稳定可控的 AI Agent》一文进行系统性深度分析。文章提出的四层工程架构（Prompt Engineering→Context Engineering→Harness Engineering→Loop Engineering）与七概念五层认知模型呈现出高度的同构性：F-第一性原理为整个架构提供"模型是无状态函数"的公理基础；A-原子化直接映射Context Engineering的粒度寻优；V-对抗性审查对应P-G-E三权分立与分层反馈机制；E-萃取与Skill/Plugin知识沉淀高度吻合；R-复盘体现为持续反馈闭环；I-洞察可形式化为四元组表达实践规律；C-原子提交对应任务隔离与回滚防护。分析发现两个体系在V-对抗性审查的部署层级和F-第一性原理的定位上存在关键差异，最终提炼出5条跨领域通用元原则：粒度适中性原则、纵深防御原则、形式化沉淀原则、反事实闭环原则、角色分离原则。
...
- **发布平台**：微信公众号「Founder Park」
- **原文链接**：https://mp.weixin.qq.com/s/GkhemHUAhKWV-3Uxaa1Mqg

### 1.2 核心论点

文章系统阐述了WorkBuddy（国内AI Agent产品，兼容国产模型，由WorkBuddy团队研发）的工程实践经验，从产品视角拆解Agent运行机制，核心论点包括：

1. **Agent工程超越Prompt Engineering**：仅靠提示词不足以构建生产环境可靠的Agent，工具接入、上下文组织、权限边界、结果验证、反馈纠正和跨会话延续都会直接影响产品可靠性
2. **模型是无状态函数**：从第一性原理推导，`输出 = 模型 (系统提示词 + 工具 + 会话历史 + 其他上下文 + 用户指令)`，模型本身不持有状态，状态必须在外部管理
3. **四层工程范式协同**：Prompt Engineering解决"如何表达请求"，Context Engineering解决"给对信息"，Harness Engineering解决"引导约束验证"，Loop Engineering解决"长期任务循环"
4. **Harness是控制系统**：通过前馈（Feedforward）提供目标规则环境，通过反馈传感器（Feedback sensors）观察结果并纠错，实现Agent的稳定可控
...
| 章节 | 标题 | 核心概括 |
|---|---|---|
| 01 | 先把模型看成一个无状态的函数 | 将模型抽象为无状态函数，给出模型调用公式，提出两条核心约束（无状态、知识截止），推导出上层工程存在的理由 |
| 02 | 用户能感知到的四个概念：工具调用/MCP/Skill/Plugin | 系统讲解工具调用（Function Call）、System Prompt、MCP（三类原语）、Skill（验证过的工作流程）、Plugin（能力组合打包），给出外接能力形态选择矩阵 |
| 03 | 全景视图：一次完整任务长什么样 | 以"调研三家Harness实践"为真实案例，展示一次完整任务的步骤，引出ReAct循环和Context Engineering的必要性 |
| 04 | Context Engineering：模型这一刻该看到什么 | 定义Context Engineering，提出五动作（Write/Select/Retrieve/Compress/Isolate），讲解Prompt Cache前缀复用规则和渐进式加载机制 |
| 05 | Memory：让正确的过去在正确的时候重现 | 区分五类长期记忆，提出"陈述性记忆入Memory、程序性记忆存为Skill"的关键决策，定义Skill"五可"质量门槛 |
| 06 | Harness Engineering：引导、约束与整合 | 系统阐述驾驭/约束/整合三类能力，分析OpenAI/Anthropic/LangChain三家实践，提出WorkBuddy五层Harness架构 |
| 07 | Loop Engineering：任务如何跨时间继续 | 定义Loop Engineering的四层位置，列举Loop必要组件，强调Goal≠Loop |
| 08 | 还没解决的问题 | 诚实指出功能和业务正确性验证缺口、代码库Harnessability决定建设难度、案例适用边界、Harness需持续投入、人仍负责主线任务 |
| 09 | 总结 | 总结两个核心结论 |
...
1. **OpenAI Codex**：3人小组用Codex从空仓库开始，全程不手写代码，5个月产出约100万行代码、1500个PR。**关键实践**：让Codex直接操作浏览器、读取DOM/日志/监控指标；用linter和结构测试自动检查架构分层、依赖方向、命名和文件大小；把AGENTS.md改成目录入口、详细知识结构化放进docs供按需查询；后台运行周期性任务扫描代码漂移、自动创建重构PR。局限：Harness主要集中在代码内部质量和可维护性上，未说明功能和业务正确性如何维护。
2. **Anthropic（双角色+P-G-E三角色）**：第一篇论文用"初始化Agent + Coding Agent"双角色方案——200+条功能清单（JSON）每条标pass/fail并禁止删条目或降标准、一次只处理一项任务、统一启动脚本init.sh、浏览器自动化做端到端验证、进度文件+Git历史做交接恢复回滚。第二篇论文（P-G-E三角色）在此基础上借鉴GAN对抗评估思路，增加Evaluator独立验收角色：Planner定规格、Generator逐sprint实现、Evaluator用Playwright端到端验证定位bug到行号后打回。
3. **LangChain**：从框架构建者角度给出Harness宽定义——Agent中除模型之外的一切都纳入Harness（系统提示词、Tools/Skills/MCP、文件系统、沙箱、浏览器、编排逻辑与Hooks），公式为 **Agent = Model + Harness**。**核心观点**：Agent要有持久状态能跨会话工作；给Agent一台计算机（Bash+代码执行）；安全可扩展地运行代码、能自我验证；用Compaction、Tool Call Offloading、Skills渐进式加载应对Context Rot；用Ralph Loop（Hook拦截提前结束信号，在新上下文里重新注入目标继续执行）；模型和Harness共同进化，评估对象应是"模型+Harness"组合。

### 1.5 四层工程架构概览

文章提出清晰的四层工程架构（第07章表格），从底到顶依次为：

| 层级 | 名称 | 核心问题 | 简单例子 |
|---|---|---|---|
| L1 | Prompt Engineering | 本次请求应如何表达？ | 写清目标、格式和约束 |
| L2 | Context Engineering | 这一次决策前，模型该看什么？ | 加载相关文件、工具、历史和记忆 |
| L3 | Harness Engineering | Agent如何被引导、约束、观测、验证和纠正？ | 规则、沙箱、审批、测试、日志、编排 |
| L4 | Loop Engineering | 任务如何被触发、流转、验收、继续与停止？ | 定时任务、工作树、子Agent、记忆、反馈闭环 |

四层架构遵循单向依赖原则：上层依赖下层提供的能力，下层不感知上层策略。Loop依赖Harness提供的约束和验证机制，Harness中的能力通过Loop持续运行。
...
### 2.1 F 第一性原理：从"模型=无状态函数"推导上层工程

F-第一性原理要求剥离经验假设，回归事物最本质的约束，自洽地重构推导。文章是第一性原理思维在Agent工程领域的典范应用——它没有从"怎么做Prompt"的经验技巧出发，而是从模型的本质抽象出发，推导整个工程体系的存在理由。

#### 假设剥离：从函数抽象开始

文章开篇即剥离了"Prompt写得好就能解决一切"的经验假设，给出一个最朴素的抽象：

> 「对产品侧而言，不需要展开Transformer的底层原理，只需要一个抽象：**模型是一个根据输入产生后续文字的函数。**」
> ——第01章 先把模型看成一个无状态的函数

文章先解释模型能力来自三个训练阶段（预训练→后训练→偏好优化与强化学习），然后给出模型调用的本质公式：
...
基于"模型=函数"这一抽象，文章明确推导出两条决定上层工程存在理由的约束：

> 「**1. 模型是无状态的。** 它不会自动保留上一次调用的内容。模型虽然无状态，但产品可以有状态。对话历史、Memory、数据库由产品在模型外部保存，需要时再放进本次输入。
>
> **2. 模型的知识截止到训练日期。** 训练之后发生的事，模型默认不掌握。询问训练日期之后的实时信息时，模型无法回答，需要先用工具查询再放进上下文。」
> ——第01章

从这两条约束出发，文章进一步推导出两个工程必要性（在后续章节展开）：上下文窗口有限（需要Context Engineering选择和组织信息）、多轮一致性需要外部维护（需要Harness Engineering提供反馈和验证）。这四个要素——无状态、知识截止、窗口限制、一致性需求——构成了Agent工程的第一性原理基础。

#### 重构推导：从约束到工程体系

两条核心约束不是孤立罗列，而是形成自洽的因果链条：无状态是最基础的本质定义，知识截止是无状态在时间维度的推论（模型无法获取训练后新信息），上下文窗口限制是无状态在空间维度的约束（单次输入有限），多轮一致性问题是前三者共同作用的结果。

从这一本质出发，文章推导出上层工程存在的理由：
- 无状态约束 → 需要外部Memory和状态维护 → 产品侧有状态管理
- 知识截止约束 → 需要工具调用和MCP接入外部能力 → 工具系统
- 窗口有限约束 → 需要选择、压缩、渐进式加载 → Context Engineering
- 一致性需求 → 需要约束、验证、反馈机制 → Harness Engineering
- 长期任务需求 → 需要触发、流转、停止条件 → Loop Engineering

> 「模型本身提供的是语言理解、推理和生成能力；但像"当前世界杯赛况"这类实时信息，或者读文件、查数据库这类外部动作，都不是模型自带的能力。产品需要在模型外接入工具和执行环境。」
> ——第01章

**方法论对照**：文章作者虽然没有使用"第一性原理"这一术语，但其推导路径完美符合F四要素——假设剥离（剥离"Prompt万能论"）→要素拆解（模型=函数+两条约束）→公理自洽（约束间因果链条）→重构推导（从约束推导出四层工程）。这是F思维在工程实践中的自然体现。
...
2. **多角攻击（多视角交叉火力）**：Planner从需求角度审查规格完整性，Generator从实现角度自检，Evaluator从用户角度端到端验证；计算型vs推断型控制分层（确定性程序vs模型语义判断两类武器）；反馈时机分层（左移快速检查+后置高价值检查）
...
特别值得注意的是"共享误解"问题的识别——这是V-对抗性审查最深刻的洞见之一。原文明确指出"自我评估不可靠"是P-G-E三角色分离的直接动因：Agent评价自己产出时倾向给正面结论，尤其在前端设计这类缺少确定性测试的任务中，这种偏差更加明显。
...
文章对控制类型做了关键的二分（覆盖前馈和反馈两个方向），这是V-对抗性审查在工程实现上的重要创新：

| 控制类型 | 判定者 | 示例 | V机制 | 置信度 |
|---|---|---|---|---|
| 计算型控制 | 确定性程序 | LSP、类型检查、linter、单元测试、结构测试、依赖扫描 | 客观、确定、快、便宜、可重复 | 100% |
| 推断型控制 | 模型判断 | Review Agent、架构审查、AI judge、设计评估 | 概率性、更慢、更贵、不确定 | ~70-80% |

> 「这两类控制还可以按执行方式分成计算型和推断型。**计算型**控制由确定性程序执行，例如LSP、类型检查、linter、单元测试、结构测试、依赖扫描、脚本和codemod，优点是快、便宜、可重复，适合在Agent每次修改后反复运行。**推断型**控制依赖模型做语义判断，例如Review Agent、架构审查Agent、AI judge和设计评估，优点是能覆盖"是否过度设计""是否误解需求""是否符合团队约定"这类难以写成规则的问题，缺点是更慢、更贵，也更不确定。」
> ——第6.3节

> 「WorkBuddy的原则是：**能用计算型信号解决的问题，优先交给确定性程序；需要语义判断的问题，再交给审查Agent。**」
> ——第6.3节
...
| Loop层 | 检测型V | 工具错误捕获、JSON校验、测试运行、计算型控制（lint/类型检查/结构测试） | 审计可溯+证伪导向 | 执行中检测 |
| Harness层 | 多角攻击V | P-G-E角色分离、Evaluator独立端到端验证 | 多角攻击（三视角交叉火力） | 执行后审查 |
| 迭代层 | 周期型V | 熵管理（Garbage Collection）、周期性漂移扫描 | 偏差防御（周期性校准） | 周期校准 |
...
> 「**陈述性记忆放进Memory，经过验证的工作方法保存为Skill**——Skill可版本化、可评审、可测试、可回滚、按需加载。」
> ——第5.2节 Memory

这一决策是E-萃取的关键洞察：程序性知识（"怎么做"）与陈述性知识（"是什么"）本质不同，应该有不同的沉淀载体——前者进Skill（结构化、版本化），后者进Memory（非结构化、上下文注入）。

#### Skill"五可"质量门槛

文章为Skill设立了严格的准入标准——"五可"原则，这是E-萃取质量门槛的工程化表达：

1. **版本化（Versionable）**：每个Skill有版本号，变更可追溯
2. **可评审（Reviewable）**：Skill定义作为代码管理，可CR可审计
3. **可测试（Testable）**：有测试用例验证正确性
4. **可回滚（Rollbackable）**：新版本出问题可一键回退到旧版本
5. **按需加载（Load-on-demand）**：不是所有Skill都默认加载，用的时候才注入上下文

"五可"原则确立了Skill作为工程化产物的质量标准——Skill不是自然语言提示词，而是需要经过版本管理、评审、测试、可回滚的正式工程资产。
...
| R4 因果转化 | Skill沉淀（从反复错误中提炼"怎么做"的程序性知识）；Harness规则更新（从卡住信号中补充规则缺口）；熵管理周期性处理漂移 | ★★★ |
...
文章有一个重要的R-洞察："Agent卡住"不是失败，而是信号。OpenAI Codex团队的做法是：

> 「"Agent卡住时，我们把它当作信号——找出缺了什么（工具、护栏、文档），反哺回仓库——而且总是让Codex自己写这个修复。"」
> ——第6.3节（引用OpenAI实践）

WorkBuddy也将Agent受阻视为"环境中缺少工具、规则或文档"的信号（第6.3节反馈层），通过反馈循环把错误返回给Agent自我纠正。这与R-复盘"将问题视为改进信号"的反事实推演理念一致。但文章更多是"持续反馈"（实时错误处理），对"阶段性反事实推演"（周期性回顾"如果当时做了不同选择会怎样"）着墨较少。

#### 因果转化：熵管理作为周期性复盘

文章提到的"熵管理（Garbage Collection）"概念，是周期性R-复盘因果转化的工程表达：

> 「**熵管理（Garbage Collection）：持续处理规则、代码和运行状态的漂移。** 现有检查主要防新增问题，对历史问题还要周期性扫描：WORKBUDDY.md/OpenSpec与代码是否一致、历史代码是否违反新规则、是否有重复实现/失效文档/过期依赖。还可以接入运行时健康传感器，例如延迟、错误率、可用性SLO、日志异常。」
> ——第6.3节

熵管理就是R-复盘在系统层面的因果转化应用：定期回顾系统状态，处理跨任务积累的漂移（规则与代码不一致、重复实现、失效文档、过期依赖），将"什么导致了熵增"的因果认知转化为清理和优化行动。Skill的版本演进则是因果转化的长期沉淀——从反复错误中提炼的"怎么做"知识最终固化为可复用的程序性记忆。
...
- **A（行动）**：陈述性知识进Memory（上下文动态注入），程序性知识封装为Skill（版本化、代码管理）
- **B（收益）**：上下文窗口节省50%+，Skill可独立测试迭代，程序性变更不影响会话历史

**洞察2：计算型/推断型控制分层处理原则**
- **C（条件）**：Agent执行过程中会收到各类控制信号（前馈引导、反馈错误、评估结果）
- **M（机制）**：控制信号有置信度差异——确定性程序判定的计算型控制置信度100%，模型判定的推断型控制置信度较低且更不确定
- **A（行动）**：计算型控制直接处理（测试没过直接修），推断型控制需谨慎校验（模型说有问题，换个模型/方法再验证）
- **B（收益）**：避免"评估模型误判导致的无效修复循环"，控制处理准确率提升
...
**"一次只处理一项任务"原则**是职责内聚的核心约束——这是Anthropic在长任务实践中总结的关键经验：如果Agent同时处理多项任务，某个工具调用可能同时服务于多个目标，一旦失败无法判断影响范围，回滚也无法精确进行。

**200+条JSON任务清单与"禁止删条目或降标准"**是Anthropic第一篇论文中职责内聚的强制性保障：

> 「一开始就把要做的事拆成 200+ 条具体行为描述的功能清单（JSON）、**每条标 pass/fail 并禁止删条目或降标准**；一次只处理一项任务；……用进度文件 + Git 历史做交接、恢复和回滚。」
> ——第06章 Harness Engineering（引用Anthropic第一篇论文实践）
...
| L2 认知层 | F第一性原理推导、I洞察发现、A粒度寻优 | L1 模型抽象 + L2 Context层 | 模型=无状态函数的核心约束、Context五动作 | ★★★★★ |
...
七概念模型中F是L2认知层的启动环节（任务开始时做第一性原理分析）；WorkBuddy中模型=无状态函数的抽象是**架构基石**——不仅启动时推导一次，而且作为核心约束贯穿所有工程决策，甚至在迭代层可以基于经验修正对模型能力的假设。七概念可以考虑将F从"一次性分析"扩展为"可基于R反馈修正的动态约束集"。
...
|---|---|---|---|---|
| F-第一性原理 | 假设剥离→要素拆解→公理自洽→重构推导 | 模型=无状态函数+两条核心约束→四层工程架构推导 | ★★★★★ | 文章从模型本质抽象出发，推导整个工程体系的存在理由，是F思维的教科书级范例 |
...
| V-对抗性审查 | 证伪导向→多角攻击→偏差防御→审计可溯 | P-G-E三权分立+计算/推断控制分层+Approval Gate+四层V纵深防御 | ★★★★★ | 映射最丰富：V机制比七概念现有定义更体系化，提供了"纵深防御"和"分层控制"新范式 |
| E-萃取 | 显化转换→抽象提升→漏斗过滤→形式化编码 | Skill"五可"原则+四层知识漏斗+程序性/陈述性知识分离+Tool→Skill→Plugin抽象跃迁 | ★★★★★ | 高度同构："五可"质量门槛是漏斗过滤的优秀实例，知识漏斗补充了E与R的递进关系 |
| R-复盘 | 事实采集→时序结构化→反事实推演→因果转化 | Audit Log+200+条pass/fail任务清单+"卡住即信号"+熵管理 | ★★★★ | 事实采集和时序结构化很强，反事实推演和因果转化的显式流程可加强 |
...
| 3 | F-公理自洽 | 两条核心约束（无状态/知识截止）+窗口限制+一致性需求的自洽性验证 | ★★★★★ | 约束之间形成因果链条而非孤立罗列 | 文章未用"公理"术语，两条是明说约束，两条是推论 |
| 4 | F-重构推导 | 从两条核心约束推导出Prompt→Context→Harness→Loop四层工程架构 | ★★★★★ | 从约束自洽地重构整个体系 | 无显著差异 |
...
| 10 | V-多角攻击 | P-G-E三角色分离+计算型/推断型控制分层+左移/后置检查时机分层 | ★★★★★ | 多视角、多武器、多层级交叉验证 | 比七概念V定义更丰富，增加了"控制类型分层"维度 |
...
| 20 | R-因果转化 | Skill沉淀+Harness规则更新+熵管理（Garbage Collection） | ★★★★ | 从时序经验中提炼因果知识 | 熵管理是R因果转化的周期性工程表达 |
| 21 | I-条件识别 | 规律适用边界（当前技术vs公理）+AI自治度三级分级（高/中/低） | ★★★★ | 明确洞察成立的条件范围 | 自治度分级是I条件识别的典型工程应用 |
...
|---|---|---|
| MCP（Model Context Protocol） | 工具/数据的标准化接入协议（Resources/Tools/Prompts三类原语） | C/A原子边界的标准化接口定义 |
...
| 计算型/推断型控制二分 | 控制信号按置信度分层（确定性程序vs模型判断） | V-对抗性审查的控制分层策略 |
| 上下文窗口工程 | Token预算管理、渐进式加载应对信息过载 | A-原子化的空间约束优化 |
| 前馈+反馈双轮驱动 | 前馈提高首次正确率，反馈让Agent自我纠正 | V-对抗性审查的时序部署策略 |
| 渐进式加载 | 索引→入口→按需深入的能力发现机制 | A-原子化的渐进式披露工程范式 |
| 熵管理（Garbage Collection） | 周期性扫描规则/代码/状态漂移，处理历史问题 | R-复盘的周期性系统维护机制 |
| Goal ≠ Loop | 目标定义方向，Loop还需触发器/环境/工具/验证/停止条件 | C-原子提交的任务边界定义原则 |
...
3. **引入"计算型/推断型控制"二分法**：V审查的结论应区分置信度——确定性逻辑判定可直接行动，模型推断性结论需二次校验
...
4. **"每一步可回溯到公理"的推导规范**：F-第一性原理要求所有设计决策必须能追溯到公理支撑，不能依赖"最佳实践"或"大家都这么做"。文章在这方面做得很好，但这是作者Anne个人的思维素养，而非可复制的方法论规范；七概念将"假设剥离→公理提炼→自洽验证→重构推导"显式化为标准步骤，任何人都可以按步骤执行。
...
2. **互补性发现**：七概念在方法论完整性和形式化表达上有优势（如I四元组、F的显式方法论步骤、V四要素框架），WorkBuddy在工程化深度上有优势（如V纵深防御、Skill五可质量门槛、计算/推断控制分层），两者可以互相反哺。
...
1. > 「对产品侧而言，不需要展开Transformer的底层原理，只需要一个抽象：**模型是一个根据输入产生后续文字的函数。**」
   > ——第01章

2. > 「`输出 = 模型 (系统提示词 + 工具 + 会话历史 + 其他上下文 + 用户指令)`」
   > ——第01章

3. > 「**1. 模型是无状态的。** 它不会自动保留上一次调用的内容。模型虽然无状态，但产品可以有状态。**2. 模型的知识截止到训练日期。** 训练之后发生的事，模型默认不掌握。」
   > ——第01章

4. > 「模型本身提供的是语言理解、推理和生成能力；但像"当前世界杯赛况"这类实时信息，或者读文件、查数据库这类外部动作，都不是模型自带的能力。产品需要在模型外接入工具和执行环境。」
   > ——第01章

5. > 「**System Prompt 只能引导，不能强制。** 权限校验、Sandbox、Approval Gate、审计仍由模型外部的系统执行。」
   > ——第2.2节

6. > 「在一次模型决策前，设计哪些信息进入上下文、以什么形式进入、放在什么位置、何时更新或移出，以提高模型做出正确下一步决策的概率。」
   > ——第04章（Context Engineering定义）

7. > 「Context Engineering追求相关、准确、及时，不是单纯堆token。」
   > ——第04章

8. > Context Engineering五动作：**写入（Write）**、**选择（Select）**、**检索（Retrieve）**、**压缩（Compress）**、**隔离（Isolate）**
   > ——第04章

9. > 「**陈述性记忆放进Memory，经过验证的工作方法保存为Skill** ——Skill可版本化、可评审、可测试、可回滚、按需加载。」
   > ——第5.2节

10. > 「**能用计算型信号解决的问题，优先交给确定性程序；需要语义判断的问题，再交给审查Agent。**」
    > ——第6.3节

11. > Harness三类能力：**驾驭（Steer）**、**约束（Constrain）**、**整合（Integrate）**
    > ——第6.1节

12. > Anthropic P-G-E三角色方案（借鉴GAN对抗评估思路）：Planner定范围、Generator逐sprint实现、Evaluator独立验收并用Playwright操作
    > ——第6.2节

13. > 「**实现和测试可能共享同一个误解。** 同一个Agent既写实现又写测试，对需求的理解偏差会同时进两边。测试全过，仍不能证明实现符合原始业务意图。」
    > ——第08章

14. > 业务正确性验证缺口四个原因：需求本身很难完整说明；实现和测试可能共享同一个误解；部分业务正确性缺少可计算的判定标准；业务错误的成本可能很高。
    > ——第08章

15. > Prompt Cache五条前缀复用规则：System Prompt/工具定义放前面保持稳定；对话历史追加保存不修改；动态内容追加后面；工具/Skill按需加载；只在必要时接受前缀变化
    > ——第4.1节

16. > 四层工程架构：Prompt Engineering（如何表达）→ Context Engineering（该看什么）→ Harness Engineering（引导约束验证）→ Loop Engineering（触发流转停止）
    > ——第07章

17. > 「**Goal ≠ Loop**」：Goal只定义"要去哪里"，Loop还需要触发器、执行环境、工具、验证信号和停止条件。
    > ——第07章

18. > 两个核心结论：**模型决定能力上限；上下文和Harness决定这个上限能否稳定落地。人负责选择方向、定义标准并承担责任；Agent负责执行、验证和加速迭代。**
    > ——第09章
...
|---|---|---|---|
| F-第一性原理 | ★★★★★ | 模型=无状态函数+两条核心约束→四层工程架构推导 | F可考虑从"一次性分析"扩展为"可基于R反馈修正的动态约束集" |
| A-原子化 | ★★★★★ | Context五动作（Write/Select/Retrieve/Compress/Isolate）+ 渐进式加载 + Prompt Cache前缀稳定 + Sub-agent隔离 | 直接同构；A可借鉴"多级渐进粒度寻优"工程范式 |
| V-对抗性审查 | ★★★★★ | P-G-E三权分立 + 计算/推断控制分层 + Approval Gate + 四层V纵深防御 + 共享误解识别 | V应从"验证层"扩展为"全生命周期纵深防御"，增加证伪导向/偏差防御/审计可溯维度 |
...
| 轮次 | 触发方 | 发现错误 | 修正情况 |
|------|--------|---------|---------|
| 第一轮 | 用户指出WorkBuddy归属错误 | 元数据错误（归属/作者/标题）、四层架构表述错误、F章节"四大公理"错误 | 全部修正 |
| 第二轮 | 系统性逐段比对（独立子代理） | 18处错误（9处编造引文+3处归属错误+2处虚构概念+14处术语漂移+6处章节标注+2处分级错误） | 全部修正 |

所有>块引用均标注章节号，可直接定位原文验证。

### 相关资源

| 资源 | 路径 | 说明 |
|------|------|------|
| 任务复盘（含幻觉事件完整记录） | [retrospective.md](file:///d:/spaces/SpecWeave/.trae/specs/retrospectives-insights/analyze-workbuddy-harness-seven-concepts/retrospective.md) | R-I-E-C链路复盘，4条核心洞察，6项行动项闭环 |
| 团队分享简报 | [team-briefing.md](file:///d:/spaces/SpecWeave/.trae/specs/retrospectives-insights/analyze-workbuddy-harness-seven-concepts/team-briefing.md) | 故事化叙事+四个核心发现+7项检查清单 |
...
| Harness Engineering Wiki | [harness-engineering-wiki/](harness-engineering-wiki/00-overview.md) | 阿里技术Harness Engineering系统化学习Wiki |
| Loop Engineering文章分析 | [harness-loop-engineering-article-analysis.md](harness-loop-engineering-article-analysis.md) | 同系列文章分析：Loop Engineering与AI系统工程范式转移 |
| 四代工程概念演进 | [four-engineering-concepts-wiki.md](four-engineering-concepts-wiki.md) | Prompt→Context→Harness→Loop四代范式演进总结 |
