...
sequenceDiagram
    ...
    User->>Agent: 发送请求\n"帮我查询今天的日程安排"
    note over Agent: Agent进行意图识别\n决定需要调用日历工具
    Agent->>Client: 调用工具请求\n工具名:get_calendar_events\n参数:{date:"today"}
    note over Client: MCP Client构造JSON-RPC请求\n已通过tools/list完成能力发现
    Client->>Server: JSON-RPC Request\nmethod:tools/call\nparams:{name:"get_calendar_events",arguments:{date:"today"}}
    note over Server: MCP Server路由请求\n到对应工具处理函数
    Server->>ExtAPI: 调用外部API/查询数据库\n如Google Calendar API、本地SQLite
    ExtAPI-->>Server: 返回原始数据\n日程列表JSON
    note over Server: Server将结果封装为\nMCP标准响应格式
    Server-->>Client: JSON-RPC Response\nresult:{content:[{type:"text",text:"今日日程:..."}]}
    Client-->>Agent: 返回工具调用结果
    note over Agent: Agent整合工具结果\n生成自然语言回答
    Agent-->>User: 自然语言回复\n"您今天有3个会议:..."
```
...
sequenceDiagram
    ...
    Remote-->>Client: 返回Agent Card\n包含skills、capabilities、auth配置
    note over Client: 检查能力匹配\n确认Remote Agent能完成目标任务\n检查认证要求并完成OAuth流程
    note over Client,Remote: 阶段2: 提交任务
    Client->>Remote: JSON-RPC tasks/sendSubscribe\n开启SSE流式连接\n携带初始任务消息
    Remote-->>Client: SSE事件: status=submitted\n任务已接受，分配taskId
    note over Client,Remote: 阶段3: 执行与状态推送
    Remote-->>Client: SSE事件: status=working\n消息:"正在查询订单系统..."
    note over Remote: Agent开始执行任务\n可能调用内部工具/数据库
    Remote-->>Client: SSE事件: status=working\n消息:"已找到订单，正在检查库存..."
    note over Client,Remote: 阶段4: 需要补充输入
    Remote-->>Client: SSE事件: status=input-required\nmessage:"请提供收货地址的邮政编码"
    note over Client: Client将请求转达给用户\n或从上下文自动补充
    Client->>User: 请求补充信息\n"需要邮政编码以继续"
    User-->>Client: 提供信息\n"100000"
    Client->>Remote: tasks/sendSubscribe\n补充输入:{postalCode:"100000"}
    note over Client,Remote: 阶段5: 继续执行
    Remote-->>Client: SSE事件: status=working\n消息:"已验证邮编，正在安排发货..."
    note over Client,Remote: 阶段6: 任务完成
    Remote-->>Client: SSE事件: status=completed\nArtifact:发货确认单PDF\n物流追踪号
    note over Client: 任务完成，SSE连接关闭
    Client-->>User: 返回最终结果\n"发货成功，物流单号:SF1234567890"
```
...
sequenceDiagram
    ...
    AgentB->>mDNS: 启动时广播服务\n_service._tcp.local\n端口8081
    note over mDNS: 同一子网内所有Agent\n都能收到广播
    AgentA->>mDNS: 查询服务\n"寻找网络中的Agent"
    mDNS-->>AgentA: 返回Agent B的地址\nhttp://192.168.1.100:8081
    note over AgentA,AgentB: 阶段2: 能力交换
    AgentA->>AgentB: GET /agent-card\n(零SDK，原生HTTP)
    AgentB-->>AgentA: 返回Agent Card\ncapabilities:["image_resize","format_convert"]\nREST端点信息
    note over AgentA: 零SDK特点验证:\n无需安装ACP SDK\n仅用HTTP客户端即可
    note over AgentA,AgentB: 阶段3: 提交任务
    AgentA->>AgentB: POST /tasks\nContent-Type: application/json\n{"operation":"resize","input":{"width":1024,"height":768}}
    note over AgentB: 可使用任意HTTP工具:\ncurl -X POST http://.../tasks\nwget、Postman、浏览器均可
    AgentB-->>AgentA: 201 Created\n{"taskId":"t-001","status":"created"}
    note over AgentA,AgentB: 阶段4: 执行任务
    note over AgentB: Agent B执行任务\n可通过本地总线/IPC\n调用本地硬件加速
    AgentB->>LocalBus: 可选：通过IPC调用\n本地设备/硬件加速
    LocalBus-->>AgentB: 返回处理结果
    note over AgentA,AgentB: 阶段5: 获取结果
    note over AgentA: 同步模式：任务秒级完成\n可直接等待响应
    note over AgentA: 异步模式：轮询或callback
    AgentA->>AgentB: GET /tasks/t-001
    AgentB-->>AgentA: 200 OK\n{"status":"completed","output":{"resultUrl":"http://..."}}
    note over AgentA: 获取到最终结果\n协作完成
```
...
sequenceDiagram
    ...
    note over HRAgent: HR Agent作为流程编排者\n协调多个系统
    note over HRAgent,HRMCP: [协议: MCP] 纵向连接内部系统
    HRAgent->>HRMCP: MCP tools/call\n查询员工信息\n创建入职记录
    HRMCP-->>HRAgent: 返回员工详情\n工号E-2025-0864
    note over HRAgent,FinAgent: [协议: A2A] 跨系统委派给财务部门
    HRAgent->>FinAgent: A2A tasks/sendSubscribe\n"为工号E-2025-0864开设工资账户"
    note over FinAgent,FinMCP: [协议: MCP] 财务Agent也通过MCP连接系统
    FinAgent->>FinMCP: MCP tools/call\n创建工资账户\n配置银行卡
    FinMCP-->>FinAgent: 账户创建完成\n账号ACC-8842
    FinAgent-->>HRAgent: A2A SSE:completed\nArtifact:工资账户信息单
    note over HRAgent,ITAgent: [协议: A2A] 跨系统委派给IT部门
    HRAgent->>ITAgent: A2A tasks/sendSubscribe\n"为张三创建办公账号并配发电脑"
    note over ITAgent,DeviceAgent: [协议: ACP] 同机房P2P与设备管理Agent通信
    ITAgent->>DeviceAgent: ACP POST /tasks\n(同数据中心，低延迟P2P)\n"准备MacBook Pro，资产编号IT-5210"
    note over DeviceAgent: 设备管理Agent在本地机房\n通过ACP低延迟通信
    DeviceAgent-->>ITAgent: ACP 任务完成\n设备已准备，装机完成
    note over ITAgent,ITMCP: [协议: MCP] 纵向连接IT系统
    ITAgent->>ITMCP: MCP tools/call\n创建AD账号\n开通邮箱\n配置VPN
    ITMCP-->>ITAgent: 账号创建完成\n邮箱:zhangsan@example.com
    ITAgent-->>HRAgent: A2A SSE:completed\nArtifact:IT资源交付单\n账号信息+设备领取信息
    note over HRAgent: HR Agent汇总所有结果\n生成入职欢迎包
    HRAgent-->>User: "入职流程已完成:\n工号:E-2025-0864\n工资账户:ACC-8842\n邮箱:zhangsan@example.com\n电脑:MacBook Pro(IT-5210)\n请前往IT部门领取设备"
```
...
stateDiagram-v2
    ...
    submitted --> working: Agent接受任务\n开始执行
    submitted --> rejected: Agent拒绝任务\n能力不匹配/权限不足/队列满
