...
flowchart TD
    ...
    subgraph Server_Org[服务端组织信任域]
        Remote_Agent[Remote Agent Server\n提供能力的Agent]
    end
    User --> Client_Agent
    Client_Agent -->|JSON-RPC over HTTPS\n带OAuth Token| Internet
    Internet --> Remote_Agent
    Remote_Agent -->|SSE流式推送\nWebhook通知| Internet
...
flowchart TD
    ...
    subgraph L3[L3 A2A 跨平台协作层]
        A2A_role[Agent↔Agent 跨厂商跨组织\nHTTP/JSON-RPC/SSE\nOAuth2/长任务/Agent Card\nA2A协议]
    end
