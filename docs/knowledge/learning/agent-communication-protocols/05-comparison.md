...
flowchart TD
    subgraph L4[L4 去中心化网络层 ANP]
        ANP_dir[通信方向: Agent↔Agent\n核心问题: 开放公网互信与发现\n类比: 互联网\n网络范围: 全球开放公网]
    end
    subgraph L3[L3 跨平台协作层 A2A]
        A2A_dir[通信方向: Agent↔Agent\n核心问题: 跨厂商/跨组织协作\n类比: HTTP\n网络范围: 跨信任域/企业专网]
    end
    subgraph L2[L2 本地消息层 ACP]
        ACP_dir[通信方向: Agent↔Agent\n核心问题: 本地/内网P2P通信\n类比: 局域网Wi-Fi\n网络范围: 同设备/本地子网]
    end
    subgraph L1[L1 工具连接层 MCP]
        MCP_dir[通信方向: Agent↔Tool/DataSource\n核心问题: 标准化工具/数据访问\n类比: USB-C\n网络范围: 本地进程/可信服务]
    end
    subgraph Ext[外部世界]
        Tools[工具]
        Data[数据源]
...
|---------|-----|-----|-----|-----|
| **发起方/治理** | Anthropic发起\nLinux基金会治理 | IBM Research/BeeAI发起\nLinux基金会AI&Data治理 | Google发起\nLinux基金会治理 | 社区驱动\n规范演进中 |
| **发布时间** | 2024年11月 | 2025年3月 | 2025年4月 | 早期探索阶段 |
| **通信方向** | 纵向\nAgent↔工具/数据 | 横向本地\nAgent↔Agent(P2P) | 横向跨域\nAgent↔Agent(C/S) | 横向公网\nAgent↔Agent(去中心化) |
...
| **发现机制** | 用户配置Server地址\ninitialize能力发现 | mDNS本地广播\n静态离线Agent Card | Well-Known URI\n/.well-known/agent.json | 去中心化发现(DHT/区块链等) |
| **SDK依赖程度** | 需要官方SDK | 零SDK\n原生HTTP即可 | 需要官方SDK | 待定 |
| **交互模式** | 同步请求/响应\nSSE流式通知 | 同步请求/响应\n异步任务轮询/回调 | 同步/SSE流式/Webhook推送 | 待定 |
| **安全模型** | stdio天然安全\n远程OAuth 2.1 | DID + 本地RBAC\nTLS可选 | 企业级OAuth 2.0/OIDC\n强制HTTPS | W3C DID + VC\n去中心化信任链 |
| **典型网络环境** | 本地进程\n可信远程服务 | 同设备IPC\n本地子网\n气隙离线环境 | 公网\n企业专线\n跨云/跨组织 | 完全开放公网\n无预置信任环境 |
| **延迟特性** | 极低（stdio）\n中高（远程HTTP） | 极低（IPC/gRPC）\n低（本地HTTP） | 中高（跨网络） | 高（公网+验证开销） |
| **任务模型** | 无状态工具调用\n单次请求响应为主 | 有状态Task\n四状态简单状态机 | 丰富有状态Task\n支持input-required多轮循环 | 待定 |
| **多模态支持** | Text/Image/Resource（通过content） | MIME类型协商\nmultipart/form-data | TextPart/DataPart/FilePart\n三类型原生混合 | 语义化数据表示 |
| **生态规模/成熟度** | 生产就绪\n数千Server\n主流IDE原生支持 | 快速发展\nBeeAI平台支撑 | 广泛采用\n150+组织支持\n全语言SDK | 早期概念阶段\n无生产部署 |
| **典型应用场景** | IDE插件\n工具调用\n数据读取\nSaaS集成 | 边缘设备/机器人\n内网多Agent\n气隙环境\n嵌入式系统 | 跨企业协作\nSaaS Agent集成\n长时任务工作流\n客服/ERP场景 | 开放Agent市场\n去中心化AI服务\n自主Agent经济 |
...
flowchart TD
    ...
    note[每个Agent既通过MCP连接自己的工具\n也通过ACP/A2A/ANP与其他Agent协作\n两者正交互补，缺一不可]
    Vertical -.-> note
    Horizontal -.-> note
```
...
| 阶段2 | 近期 | **ACP/A2A** | A2A快速增长\nACP发展中 | A2A低\nACP中 | 根据场景选择横向协议，实现Agent间协作 |
