...
| 章节 | 标题 | 内容概要 | 难度 |
|------|------|---------|------|
| 00 | [概述](/index.md)（当前页） | 教程介绍、核心特性、术语表、章节导航、阅读路径 | ⭐ |
| 01 | [安装与配置](/concepts/01-installation.md) | pip/uvx 安装、6 个可选依赖、虚拟环境建议、~/.okf/ 目录结构 | ⭐ |
| 02 | [CLI 命令参考](/references/02-cli-reference.md) | 11 个子命令完整参考、参数说明、使用示例 | ⭐⭐ |
| 03 | [OKF 格式与 Bundle 结构](/concepts/03-okf-format.md) | OKF v0.1 规范、目录结构、frontmatter 字段、state.json 详解 | ⭐⭐ |
| 04 | [核心架构](/concepts/04-core-architecture.md) | 模块架构图、BFS 爬取算法、URL 映射、Fetcher 抽象、Writer 流程 | ⭐⭐⭐ |
| 05 | [增量同步机制](/concepts/05-sync-mechanism.md) | content hash 原理、delta 检测、安全阈值、post_sync 钩子 | ⭐⭐⭐ |
| 06 | [Chat 对话系统](/concepts/06-chat-system.md) | Agent 导航循环、Provider 抽象、零 Key 回退、对话历史、trace 模式 | ⭐⭐⭐ |
| 07 | [MCP 与 HTTP 服务](/concepts/07-mcp-serve.md) | stdio MCP 服务器、4 个 MCP 工具、FastAPI HTTP API、Docker 部署 | ⭐⭐⭐ |
| 08 | [Registry 与可视化](/concepts/08-registry-visualize.md) | awesome-okf-kit 注册表、bundle 发布安装、graph.html 知识图谱 | ⭐⭐ |
| 09 | [扩展与开发](/concepts/09-extension-development.md) | 源码结构、开发环境、测试、自定义 Fetcher、calknowledge 生态 | ⭐⭐⭐ |
| 10 | [FAQ 与排错](/references/10-faq-troubleshooting.md) | JS 站点识别、robots.txt、Provider 错误、模型不存在、Bundle 验证失败 | ⭐⭐ |
| 11 | [总结与资源](/references/11-summary-resources.md) | 核心知识点回顾、命令速查表、生态链接、相关项目对比 | ⭐ |
...
1. 从 [安装与配置](/concepts/01-installation.md) 开始，完成环境搭建
2. 浏览 [CLI 命令参考](/references/02-cli-reference.md)，了解命令体系
3. 通过 [OKF 格式与 Bundle 结构](/concepts/03-okf-format.md) 理解输出格式
...
4. 学习 [MCP 与 HTTP 服务](/concepts/07-mcp-serve.md)，配置 MCP 让编程助手读取文档
...
1. 掌握 OKF 格式规范
2. 深入理解 [核心架构](/concepts/04-core-architecture.md)（BFS 爬取、Fetcher 抽象、Writer 流程）
3. 学习 [增量同步机制](/concepts/05-sync-mechanism.md)
4. 理解 [Chat 对话系统](/concepts/06-chat-system.md) 的 Agent 导航策略
5. 阅读 [扩展与开发](/concepts/09-extension-development.md) 了解源码结构
