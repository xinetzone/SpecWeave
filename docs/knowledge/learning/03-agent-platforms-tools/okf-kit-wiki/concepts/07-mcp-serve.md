...
sources:
  - "https://github.com/vinodborole/okf-kit/tree/main/okf_kit"
date: "2026-08-18"
category: "learning"
tags: ["okf-kit", "mcp", "serve", "http-api", "fastapi", "sse", "claude-code", "cursor", "docker"]
type: "Reference"
description: "okf-kit 提供两种服务暴露方式——stdio MCP 服务器（`okf serve-mcp`）供 Claude Code/Cursor 等 AI 编辑器直接集成本地 bundle，FastAPI HTTP 服务器（`okf serve`）提供 REST API + SSE 流式输出供桌面 GUI 使用，两种方式都复用 bundle_nav.py 共享导航逻辑。"
generated:
