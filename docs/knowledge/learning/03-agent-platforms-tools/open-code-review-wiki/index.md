...
| 章节 | 标题 | 内容概要 | 难度 |
|------|------|---------|------|
| 00 | [概述](/)（当前页） | 教程总览、Benchmark、架构图、核心特性 | ⭐ |
| 01 | [安装与配置](/concepts/01-installation.md) | 四种安装方式、状态目录、规则文件、卸载 | ⭐ |
| 02 | [CLI 命令参考](/references/02-cli-reference.md) | 10 个子命令、review 三种模式、JSON 输出 | ⭐⭐ |
| 03 | [核心架构](/concepts/03-architecture.md) | 六阶段流水线、Agent 模块、三区内存压缩 | ⭐⭐⭐ |
| 04 | [LLM 协议与 Provider](/concepts/04-llm-providers.md) | 19 个内置 Provider、Endpoint 解析、Token 计数 | ⭐⭐⭐ |
| 05 | [内置工具与 MCP 集成](/concepts/05-tools-mcp.md) | 6 个内置工具、Registry 机制、MCP 扩展 | ⭐⭐⭐ |
| 06 | [审查规则系统](/concepts/06-review-rules.md) | 四层规则优先级、JSON 规则文件、glob 模式 | ⭐⭐⭐ |
| 07 | [会话持久化、遥测与查看器](/concepts/07-session-telemetry.md) | JSONL 会话日志、OpenTelemetry、本地查看器 | ⭐⭐⭐ |
| 08 | [集成与扩展](/concepts/08-integrations.md) | CI/CD、委托模式、Agent Skill、VS Code 扩展 | ⭐⭐⭐⭐ |
| 09 | [FAQ 与排错](/references/09-faq-troubleshooting.md) | 常见问题、错误信息、排错思路 | ⭐⭐ |
| 10 | [总结、术语表与资源](/references/10-summary-resources.md) | 知识体系、速查表、术语表、资源链接 | ⭐ |
...
1. 先从 [安装与配置](/concepts/01-installation.md) 开始，完成环境搭建
2. 浏览 [CLI 命令参考](/references/02-cli-reference.md)，了解命令体系
3. 通过 [核心架构](/concepts/03-architecture.md) 理解审查流水线如何工作
...
1. 完成安装与 LLM 配置
2. 掌握 [LLM 协议与 Provider](/concepts/04-llm-providers.md)，理解环境变量与 config key
3. 学习 [集成与扩展](/concepts/08-integrations.md)，嵌入流水线门禁
4. 制定团队 [审查规则](/concepts/06-review-rules.md)，落地规范
...
1. 理解 OCR 的核心设计（本文）
2. 掌握命令体系与 JSON 输出
3. 学习 [集成与扩展](/concepts/08-integrations.md)，让 Claude Code/Cursor 直接调用
4. 扩展 [内置工具与 MCP 集成](/concepts/05-tools-mcp.md)，接入自定义工具
