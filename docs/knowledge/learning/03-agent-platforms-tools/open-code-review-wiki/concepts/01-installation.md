...
category: learning
type: Tutorial
description: OCR 安装与配置指南，涵盖 npm/npx/源码/Release 四种安装方式、状态目录结构、项目规则配置、自动更新、卸载方法与安装验证。
sources:
- id: primary
  resource: "https://open-codereview.ai/docs/installation"
  title: "OCR 官方文档 - 安装指南"
generated:
...
> 详细的规则系统说明见第 6 章[审查规则系统](/concepts/06-review-rules.md)。
...
> 如果输出 `Preview complete` 且文件列表正确，说明 OCR 安装成功且能正常读取 Git 仓库。下一步是配置 LLM provider，详见第 4 章[LLM 协议与 Provider](/concepts/04-llm-providers.md)。
...
- ← [返回目录](/) | [下一章：CLI 命令参考](/references/02-cli-reference.md) →
