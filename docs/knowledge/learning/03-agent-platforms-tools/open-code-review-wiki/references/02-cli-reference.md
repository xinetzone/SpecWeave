...
category: learning
type: Reference
description: ocr CLI 完整参考，覆盖 review/scan/delegate/session/config/llm/rules/viewer 等 10 个子命令、所有 flag、三种审查模式、JSON 输出格式与退出码。
sources:
- id: primary
  resource: "https://open-codereview.ai/docs/cli-reference"
  title: "OCR 官方文档 - CLI 命令参考"
generated:
...
> 详细的会话查看器说明见第 7 章[会话持久化、遥测与查看器](/concepts/07-session-telemetry.md)。
...
> 详细的委托模式与 Agent 集成说明见第 8 章[集成与扩展](/concepts/08-integrations.md)。
...
> 完整的 key 参考、schema 与示例见第 4 章[LLM 协议与 Provider](/concepts/04-llm-providers.md)。
...
可用于排查"为什么我的自定义规则没触发？"——完整的优先级说明见第 6 章[审查规则系统](/concepts/06-review-rules.md)。
...
- [安装与配置](/concepts/01-installation.md)——安装并完成首次评审
- [LLM 协议与 Provider](/concepts/04-llm-providers.md)——参数背后的环境变量与 config key
- [审查规则系统](/concepts/06-review-rules.md)——`--rule` 参数与规则解析
- [集成与扩展](/concepts/08-integrations.md)——从 agent 与 CI 调用 `ocr review`

---

- ← [返回目录](/) | [上一章：安装与配置](/concepts/01-installation.md) | [下一章：核心架构](/concepts/03-architecture.md) →
