...
type: Concept
description: "讲解 The Agency 仓库目录结构，包括 divisions.json 部门清单、tools.json 工具契约及 17 个部门目录与特殊目录的角色。"
sources:
...
> **下一步**：理解了目录结构后，在 [Agent 文件格式解析](/concepts/02-agent-format.md) 中，我们将深入一个 Agent 文件内部，逐字段解析 frontmatter 与正文 8 大章节。

---

- [上一章：概述](/index.md) ←
- [下一章：Agent 文件格式解析](/concepts/02-agent-format.md) →
