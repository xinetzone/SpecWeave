...
type: Tutorial
description: "面向新开发者的可复制粘贴分步演示教程，从环境自检、克隆仓库、预览 Agent、安装到工具、激活第一个 Agent，最后用 5 人团队跑通 MVP 场景。"
sources:
...
| 阶段 | 你将要做的 | 对应 wiki 章节 |
|------|-----------|--------------|
| ① 环境自检 | 检测本机已装的 AI 工具 | [00 概述](/index.md) |
| ② 克隆仓库 | 把 The Agency 拉到本地 | [01 文件夹架构](/concepts/01-architecture.md) |
| ③ 预览 | 看看有哪些部门与 Agent | [03 部门名册](/concepts/03-roster-divisions.md) |
| ④ 安装 | 把 Agent 装到你常用的工具 | [04 脚本体系](/concepts/04-scripts-tooling.md) |
| ⑤ 激活 | 在会话中喊出第一个 Agent | [06 使用示例](/examples/06-usage-examples.md) |
| ⑥ 演示 | 用 5 个 Agent 跑通一个小任务 | [07 策略与运行手册](/concepts/07-strategy-playbooks.md) |
...
> 更多开箱即用的团队组合见 wiki [06 使用示例](/examples/06-usage-examples.md) 第 8 节，以及 `examples/` 目录下的 `workflow-*.md`。
...
> 完整排查流程见 wiki [08 常见问题解答](/references/08-faq-troubleshooting.md)。
...
**一两分钟内你还想做的**：
- 完整阅读本教程，从 [00 概述](/index.md) 开始按学习路径走。
- 想团队落地？看 [09 最佳实践](/concepts/09-best-practices.md) 的"团队落地"章节与 [07 策略与运行手册](/concepts/07-strategy-playbooks.md) 的 NEXUS 策略。
- 想自定义 Agent？学 [02 Agent 文件格式](/concepts/02-agent-format.md)，或浏览各部门目录参考现成档案。
