...
type: Concept
description: "全面讲解 The Agency 的 17 个部门名册，用分类树和汇总表呈现各部门 Agent 数量、核心成员、代表能力与典型场景，并给出跨部门组合协作建议。"
sources:
...
通过对仓库源码的实地统计，The Agency 当前包含 **17 个部门、共 270 个 Agent**（Markdown 文件）。每个 Agent 文件都以统一的 YAML frontmatter 开头（`name`、`description`、`color`、`emoji`、`vibe`），正文则详细定义该 Agent 的身份、核心使命、关键规则、技术交付物、工作流与成功指标——详见 [Agent 文件格式解析](/concepts/02-agent-format.md)。
...
- [上一章：Agent 文件格式解析](/concepts/02-agent-format.md) ←
- [下一章：脚本体系](/concepts/04-scripts-tooling.md) →
