...
maturity: L2
type: Concept
description: 解析 Agent 循环 Turn/Step 精确定义、官方时序图、三类事件分发模式（持久会话、Waterfall、Serial）与模型可见即已记录不变量。
generated:
...
← [05 核心架构](/concepts/05-architecture-everything-plugin.md) | → [07 会话日志与可观测性](/concepts/07-session-log-observability.md)
