...
maturity: L1
type: Tutorial
description: 讲解 dsh 环境准备（Node.js 版本要求）、API Key 获取、npx 一键启动、源码构建、~/.dsh 配置目录与 Windows
  兼容性说明。
generated:
...
> **支持版本**：`^22.19 || >=24`（仅支持 LTS 长期支持版本）
>
> **不支持**：非 LTS 的 Current 版本（如 Node 23 这类奇数版本）不在官方支持范围内；其中 Node 23.0–23.5 会因 `node:sqlite` 和原生 TypeScript 类型剥离仍需实验标志而直接启动失败，Node 23 全线已 EOL（停止维护）
...
← [01 介绍](/concepts/01-introduction-background.md) | → [03 快速上手](/concepts/03-quickstart-first-task.md)
