...
date: "2026-07-04"
tags: ["向日葵", "智能插线板", "P4", "P1Pro", "4G智能插座", "WiFi智能插座", "远程控制", "智能硬件", "独立分控", "电量监控", "Oray", "贝锐科技", "远程办公"]
---
...
P1Pro配备4个新国标五孔插孔，全部支持独立分控：
- 1台P1Pro = 4个智能插座
- 新国标五孔设计（兼容两扁插、两圆插、三扁插）
- 每个插孔独立命名、开关、重启
- 4个插孔满足多数桌面/办公场景需求（电脑主机+显示器+路由器+音箱/台灯）
...
| 功能 | 说明 |
|-----|------|
| **四孔独立分控** | 4个新国标五孔插孔，每个独立开关、独立命名、独立重启 |
| **远程一键开机（延时断电）** | 配合AC Recovery远程开机，关机时3分钟延时温柔断电保护主机 |
| **实时电量监控** | 支持实时电流功耗查看，带历史曲线、月度耗电统计 |
| **智能定时开关（本地定时）** | 定时/倒计时任务存储本地，断网仍可正常执行 |

**电量监控比P4更丰富**：P1Pro不仅有实时数据，还提供历史曲线和月度耗电统计，方便用户了解长期用电情况。

### 4.8 典型应用场景

| 场景类型 | 具体描述 | P1Pro价值 |
|---------|---------|--------|
| **远程办公** | 家里电脑忘关/在公司需要家里电脑资料 | 远程开机→远控→温柔关机→3分钟断电，完整工作流 |
...
| **实验室/测试** | 测试设备定时开关、远程重启 | 本地定时断网可用，自动化测试供电 |
...
| **环境规格** | 工作温度 | -10°C ~ 40°C |
| | 阻燃等级 | V0级阻燃PC，750°C高温阻燃 |
| **功能特性** | 温柔开关机 | 支持（3分钟延时断电） |
| | AC Recovery | 支持（配合主板设置） |
| | 电量监控 | 支持（实时数据、历史曲线、月度耗电） |
| | 本地定时 | 支持（断网可用） |
| | 断电记忆 | 支持 |
| | 指示灯开关 | 支持 |
| **安全升级** | 超功率自动断电 | 支持 |
...
```
【准备阶段】
1. 将电脑主机插在P1Pro的一个插孔上（命名为"办公电脑"）
2. 显示器、台灯等外设插在其他插孔（可选）
3. 进入电脑BIOS设置，开启"AC Recovery"或"Restore on AC Power Loss"选项为"Power On"
4. 电脑安装向日葵远控客户端并设置开机自启、自动登录
5. 在向日葵APP中绑定P1Pro，测试远程开关功能

【远程办公流程】
1. 人在外面需要使用家里/办公室电脑
2. 打开向日葵APP，找到"办公电脑"插孔
3. 点击"开机"，插孔通电
4. 主板检测到电源恢复，自动触发开机（AC Recovery）
5. 等待1-3分钟电脑启动完成，向日葵客户端上线
6. 在APP中点击"远程控制"，进入电脑桌面
7. 正常办公：操作文件、运行程序、远程协作
8. 工作结束，关闭所有文档和程序
9. 在APP中点击"温柔关机"
10. P1Pro发送关机信号 → 电脑正常关机 → 等待3分钟
11. 3分钟后P1Pro自动切断"办公电脑"插孔电源
12. 完成，安全节能
```
...
P1Pro负载能力与P4相同：
- **总功率限制2500W**，4个插孔总功率不超
- 阻性负载2500W，感性/容性负载850W
- 注意：电脑主机属于开关电源类（容性+感性混合），单台主机通常300-600W，留足余量可接1-2台主机加显示器、外设等
...
**Q4：温柔开关机是什么原理？直接断电真的会损坏电脑吗？**

A：温柔开关机的原理是：发送关机指令后，先通过向日葵远控软件向电脑发送正常关机信号，等电脑完成关机流程后（等待3分钟），再切断电源。直接断电（强制断电）不一定每次都会坏，但确实有风险：可能导致正在写入的数据丢失、磁盘文件系统损坏、机械硬盘磁头未归位产生坏道、SSD异常掉电影响寿命。对于长期使用的电脑，温柔关机能显著降低这些风险。

**Q5：没有主板AC Recovery功能，是不是就不能远程开机了？**

A：是的，P1Pro/P4的远程开机依赖主板的AC Recovery（上电自启）功能——插线板能做的就是"给插孔通电"，电脑能不能自动开机取决于主板BIOS设置。如果主板不支持这个功能，通电后电脑不会自动开机，需要手动按机箱电源键。大部分近几年的台式机主板都支持这个功能，笔记本电脑支持情况不一。AC Recovery设置指南：http://url.oray.com/aYJHJr
