...
flowchart TD
    ...
    subgraph ACP_Arch["ACP: 去中心化P2P架构"]
        Agent1["Agent 1<br/>REST服务端+客户端"]
        Agent2["Agent 2<br/>REST服务端+客户端"]
        Agent3["Agent 3<br/>REST服务端+客户端"]
        Agent4["Agent 4<br/>REST服务端+客户端"]
        Bus["本地消息总线/网络<br/>mDNS发现"]
        Agent1 <--> Bus
        Agent2 <--> Bus
        Agent3 <--> Bus
        Agent4 <--> Bus
...
flowchart TD
    subgraph L4["L4 ANP 去中心化网络层"]
        ANP_role["Agent↔Agent 公网去中心化<br/>DID身份/信任/声誉"]
    end
    subgraph L3["L3 A2A 跨平台协作层"]
        A2A_role["Agent↔Agent 跨厂商广域<br/>OAuth2/长任务/流式SSE"]
    end
    subgraph L2["L2 ACP 本地消息层"]
        ACP_role["Agent↔Agent 本地P2P<br/>REST原生/mDNS/零SDK<br/>本地优先/气隙支持"]
    end
    subgraph L1["L1 MCP 工具连接层"]
        MCP_role["Agent↔工具/数据 纵向<br/>JSON-RPC/Tools/Resources"]
    end
    ...
    L2 --> L3
    L3 --> L4
    note["ACP是横向本地层<br/>连接同环境Agent<br/>与MCP纵向互补"]
    ACP_role -.-> note
```
