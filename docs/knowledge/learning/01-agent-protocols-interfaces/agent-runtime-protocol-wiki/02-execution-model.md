...
- **代码式 Runtime**（OpenAI Agents SDK、Claude Agent SDK）：控制流由 Python 代码直接驱动，上手快，工具执行和事件流由 SDK 托管。
- **托管式 Runtime**（OpenAI Assistants）：用户将 runtime 托管给平台，最省心，但控制权和可观测性最少。
...
## Agent Harness：Protocol/Runtime 能力产品化
...
**代价**：调试困难、消息顺序复杂、状态一致性变差。适用于大规模（超过 10 个 Agent 协作）多 Agent 系统。
...
- 上一章：[Protocol 边界与最小生命周期](01-protocol-boundary-lifecycle.md)
- [下一章：状态管理](03-state-management.md) →
