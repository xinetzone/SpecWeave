...
type: Tutorial
description: "介绍使用 GitHub CLI 管理 GitHub Actions 完整生命周期，包括工作流启停、运行追踪重试、缓存清理及密钥变量配置。"
generated:
...
sources:
  - id: github-cli-manual-run
    resource: https://cli.github.com/manual/gh_run
    title: "GitHub CLI 官方手册 - gh run"
---
...
GitHub CLI 的 Actions 相关命令以 `gh workflow`、`gh run`、`gh cache`、`gh secret`、`gh variable` 五个子命令组为入口，覆盖 CI/CD 流水线的全部操作。在开始之前，请确保已完成 [安装与配置](/concepts/01-installation.md) 中的认证步骤。
...
在合并 PR 之前，确保所有 CI 检查通过。结合 [PR 工作流指南](/concepts/03-pr-workflow.md) 中的 `gh pr checks` 命令。
...
- [安装与配置指南](/concepts/01-installation.md) — 安装 `gh` 并完成认证
- [基础命令](/concepts/02-basic-commands.md) — 仓库和 Issue 的日常操作
- [PR 工作流指南](/concepts/03-pr-workflow.md) — Pull Request 的完整生命周期管理
