...
| 章节 | 标题 | 内容概要 | 难度 |
|------|------|---------|------|
| 00 | [概述](/index.md)（当前页） | 教程总览、目标受众、架构图、阅读路径 | ⭐ |
| 01 | [安装与配置指南](/concepts/01-installation.md) | 各平台安装、认证配置、Shell 补全 | ⭐ |
| 02 | [基础命令速通](/concepts/02-basic-commands.md) | 仓库操作、Issue 管理、Gist 使用 | ⭐⭐ |
| 03 | [Pull Request 工作流](/concepts/03-pr-workflow.md) | PR 创建、Review、Merge、Checkout 全流程 | ⭐⭐⭐ |
| 04 | [Actions 与 CI/CD 集成](/concepts/04-actions-cicd.md) | Workflow 管理、Run 追踪、日志查看 | ⭐⭐⭐ |
| 05 | [高级用法与自动化](/concepts/05-advanced-usage.md) | API 调用、Extension 扩展、Alias 别名、批量操作 | ⭐⭐⭐⭐ |
| 06 | [常见问题与排错](/references/06-faq-troubleshooting.md) | 认证失败、权限问题、网络代理、常见错误码 | ⭐⭐ |
| 07 | [速查表](/references/07-cheatsheet.md) | 全部命令速查、常用组合、Shell 别名推荐 | ⭐⭐ |
...
2. 掌握 [基础命令](/concepts/02-basic-commands.md)，熟悉仓库和 Issue 的日常操作
3. 深入 [PR 工作流](/concepts/03-pr-workflow.md)，这是日常开发最高频的场景
...
1. 学习 [Actions CI/CD](/concepts/04-actions-cicd.md)，将 `gh` 嵌入自动化流水线
2. 掌握 [高级用法](/concepts/05-advanced-usage.md)，利用 API、Extension、Alias 定制工作流
3. 配备 [速查表](/references/07-cheatsheet.md)，随时查阅命令组合，提升效率
...
直接跳转到 [常见问题与排错](/references/06-faq-troubleshooting.md)，按症状索引查找解决方案。
