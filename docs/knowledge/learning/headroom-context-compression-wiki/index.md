...
| 章节 | 标题 | 内容概要 | 难度 |
|------|------|---------|------|
| 00 | [概述](/index.md)（当前页） | 背景痛点、项目简介、核心特性、导航 | ⭐ |
| 01 | [核心架构与设计理念](/concepts/01-core-architecture.md) | 中间层定位、拦截内容类型、工作原理、4种接入总览 | ⭐⭐ |
| 02 | [六种压缩算法详解](/concepts/02-compression-algorithms.md) | 内容路由机制、SmartCrusher、CodeCompressor、Kompress-v2-base | ⭐⭐⭐ |
| 03 | [CCR可逆机制深度解析](/concepts/03-ccr-mechanism.md) | Compress-Cache-Retrieve机制、本地存储设计、同类工具对比表 | ⭐⭐⭐ |
| 04 | [四种接入方式详解](/concepts/04-integration-methods.md) | Library/Proxy/Agent Wrap/MCP Server，含代码示例与选型建议 | ⭐⭐ |
| 05 | [效果验证与数据分析](/references/05-performance-data.md) | 各场景压缩率、质量评估数据、"质量不降反升"原因分析 | ⭐⭐ |
| 06 | [进阶功能：跨Agent记忆与自学习](/concepts/06-advanced-features.md) | SQLite+向量库共享记忆、headroom learn自动进化 | ⭐⭐⭐ |
| 07 | [快速上手指南](/concepts/07-quick-start.md) | 环境要求、安装命令、三步上手流程、Docker方式 | ⭐ |
| 08 | [深度洞察与模式萃取](/concepts/08-insights-patterns.md) | 3个可复用设计模式、行业趋势分析、开发者启示 | ⭐⭐⭐⭐ |
| 09 | [FAQ与资源链接](/references/09-faq-resources.md) | 常见问题解答、官方资源、延伸阅读 | ⭐ |
| 10 | [总结与Takeaways](/references/10-summary.md) | 核心要点回顾、5条关键启示、下一步学习建议 | ⭐ |
...
```
1. 读完当前概述了解项目价值
2. 直接跳转到[快速上手指南](/concepts/07-quick-start.md)安装体验
3. 根据需要选择[接入方式](/concepts/04-integration-methods.md)
...
00 → 01 → 02 → 03 → 05 → 08
```
1. 理解[核心架构](/concepts/01-core-architecture.md)和设计理念
2. 深入[6种压缩算法](/concepts/02-compression-algorithms.md)的工作原理
3. 重点研究[CCR可逆机制](/concepts/03-ccr-mechanism.md)——这是Headroom最核心的创新
4. 查看[效果数据](/references/05-performance-data.md)验证实际价值
5. 最后通过[深度洞察](/concepts/08-insights-patterns.md)萃取可复用模式
