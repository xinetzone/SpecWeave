...
| 章节 | 标题 | 内容概要 | 难度 |
|------|------|---------|------|
| 00 | [概述](/index.md)（当前页） | 版本背景、目标受众、核心特性全景、章节导航 | ⭐ |
| 01 | [语言新特性](/concepts/01-language-features.md) | t-strings、延迟注解、无括号 except、finally 警告、内置函数变更、字节码变更 | ⭐⭐ |
| 02 | [自由线程（无 GIL）深度解析](/concepts/02-free-threading.md) | PEP 703/779、QSBR、BRC、关键区段、mimalloc、线程安全模型、C扩展兼容 | ⭐⭐⭐ |
| 03 | [JIT 编译器与新执行模型](/concepts/03-jit-interpreter.md) | Tier 1 特化、Tier 2 uop 优化器、Copy-and-Patch JIT、尾调用解释器 | ⭐⭐⭐ |
| 04 | [新模块详解](/concepts/04-new-modules.md) | annotationlib、concurrent.interpreters、string.templatelib、compression.zstd | ⭐⭐ |
| 05 | [标准库重大改进](/concepts/05-stdlib-improvements.md) | REPL、asyncio、pathlib、pdb、uuid、argparse、json、unittest 等 | ⭐⭐ |
| 06 | [CPython 源码架构总览](/concepts/06-cpython-architecture.md) | 目录结构、三层头文件、解释器循环、对象系统、GC、内存分配、PEG 解析器 | ⭐⭐⭐ |
| 07 | [C API 与扩展开发](/references/07-c-api-changes.md) | PEP 741/757、Limited API 变更、自由线程适配、废弃/移除 API | ⭐⭐⭐ |
| 08 | [构建系统与平台支持](/concepts/08-build-platform.md) | 构建选项、官方二进制新特性、平台支持变化、签名变更 | ⭐⭐ |
| 09 | [迁移指南](/concepts/09-migration-guide.md) | 废弃 API 对照表、行为变更注意事项、C 扩展迁移 checklist | ⭐⭐ |
| 10 | [实战示例](/examples/10-practical-examples.md) | t-strings SQL 构建、FT 基准测试、多解释器并行、zstd 压缩、pdb 远程调试 | ⭐⭐ |
| 11 | [FAQ 与排障](/references/11-faq-troubleshooting.md) | 安装/兼容性/性能 FAQ、已知问题、调试技巧 | ⭐⭐ |
| 12 | [总结与资源](/references/12-summary-resources.md) | 十大变革速记、分角色学习路径、源码速查表、延伸阅读 | ⭐ |
| 13 | [官方文档四大支柱导览](/references/13-official-docs-roadmap.md) | tutorial/library/extending/howto 四支柱全景 + 与 3.14 章节映射、分角色官方文档阅读路径 | ⭐ |
| — | [📋 速查卡片](python314-cheatsheet.html) | 单页 HTML 可视化速查，16 个 PEP、五大特性、迁移表、源码映射 | ⭐ |
| — | [🗺️ 学习路径规划](/concepts/learning-path.md) | 四阶段递进学习模型、五角色专属路径、分天练习任务、自测清单 | ⭐ |
