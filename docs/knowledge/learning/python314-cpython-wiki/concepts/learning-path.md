...
type: Tutorial
description: "Python 3.14 系统化学习路径，从环境搭建到高级特性的分阶段学习指南"
sources:
  - https://docs.python.org/zh-cn/3.14/, https://github.com/python/cpython (tag: v3.14.0)
date: 2026-08-19
category: learning
tags: ["python314", "learning-path", "study-plan", "roadmap"]
generated:
...
```mermaid
graph LR
    P1["🔵 阶段一<br/>认知入门<br/>（知道有什么）"] --> P2["🟢 阶段二<br/>实践上手<br/>（会用新特性）"]
    P2 --> P3["🟡 阶段三<br/>深入理解<br/>（理解原理）"]
    P3 --> P4["🔴 阶段四<br/>生产应用<br/>（能解决问题）"]
...
| 阶段 | 目标 | 产出物 | 适用角色 |
|------|------|--------|---------|
| **阶段一：认知入门** | 知道 3.14 有哪些新东西，能说出核心特性名称 | 速查卡片阅读笔记 | 所有角色 |
...
| 角色 | 必看章节 | 可跳过章节 |
|------|---------|-----------|
| 应用开发者 | 01、04、05、09 | 02（只需了解）、03（只需了解）、06、07 |
| 库作者 | 01、02（线程安全部分）、04、05、07、09 | 03（了解即可）、06（了解目录结构） |
| C 扩展开发者 | 02、03（执行模型）、06、07、08、09 | 04（了解概念即可） |
| 源码学习者 | 02、03、06、07 全部 | 10、11 选读 |
...
- [ ] 阅读 [01-language-features.md §4](/concepts/01-language-features.md#4-pep-750t-strings-模板字符串)
- [ ] 阅读 [04-new-modules.md §3](/concepts/04-new-modules.md#3-stringtemplatelib--模板字符串库pep-750)
- [ ] 练习任务：
...
- [ ] 阅读 [01-language-features.md §3](/concepts/01-language-features.md#3-pep-649--pep-749延迟注解求值)
- [ ] 阅读 [04-new-modules.md §1](/concepts/04-new-modules.md#1-annotationlib--注解操作库pep-749)
- [ ] 练习任务：
  1. 定义一个带前向引用的类（如链表节点），验证不需要 `__future__ annotations`
...
- [ ] 阅读 [01-language-features.md §1-2](/concepts/01-language-features.md#1-pep-758无括号-except-和-except-表达式)
- [ ] 练习任务：
...
**Day 4：内置函数与标准库改进**
- [ ] 阅读 [01-language-features.md §5-6](/concepts/01-language-features.md#5-内置函数变更)
- [ ] 阅读 [05-stdlib-improvements.md](/concepts/05-stdlib-improvements.md)
- [ ] 练习任务：
...
- [ ] 阅读 [04-new-modules.md §2](/concepts/04-new-modules.md#2-concurrentinterpreters--多解释器模块pep-734)
- [ ] 练习任务：
  1. 创建子解释器执行简单代码
...
**Day 6-7：自由线程初体验**
- [ ] 阅读 [02-free-threading.md §1-3](/concepts/02-free-threading.md#1-为什么要去掉-gil)
- [ ] 练习任务：
  1. 在 `python3.14t` 下运行 [10-practical-examples.md §3](/examples/10-practical-examples.md#示例-3自由线程多线程性能基准) 的基准测试
...
**Day 8：迁移指南**
- [ ] 阅读 [09-migration-guide.md](/concepts/09-migration-guide.md)
- [ ] 练习任务：
...
**Day 9-10：JIT 实验**
- [ ] 阅读 [03-jit-interpreter.md §1-6](/concepts/03-jit-interpreter.md#1-python-执行模型演进)
- [ ] 练习任务：
...
- [ ] 阅读 [10-practical-examples.md](/examples/10-practical-examples.md) 获取灵感

**Day 13-14：FAQ 与排障**
- [ ] 阅读 [11-faq-troubleshooting.md](/references/11-faq-troubleshooting.md)
- [ ] 整理你在实践中遇到的问题和解决方案
...
> ⚠️ 此阶段主要面向库作者、C 扩展开发者和源码学习者。应用开发者可以选读感兴趣的部分。

### 学习内容

#### Track A：自由线程深入（C 扩展/库作者必做，1-2 周）

- [ ] 精读 [02-free-threading.md §4-8](/concepts/02-free-threading.md#4-qsbrquiescent-state-based-reclamation)
  - QSBR 原理和静默点机制
  - BRC 批量引用计数和永生对象
  - 关键区段和 Parking Lot
  - mimalloc 内存分配
...
- [ ] 精读 [03-jit-interpreter.md §3-7](/concepts/03-jit-interpreter.md#3-尾调用解释器tail-call-interpreter)
  - 尾调用解释器原理
  - Tier 1 自适应特化
  - Tier 2 uop 优化器
...
- [ ] 精读 [06-cpython-architecture.md](/concepts/06-cpython-architecture.md)
- [ ] 源码阅读路线（按顺序）：
...
- [ ] 精读 [07-c-api-changes.md](/references/07-c-api-changes.md) 和 [08-build-platform.md](/concepts/08-build-platform.md)
- [ ] 练习任务：
...
阶段一（2天）→ 阶段二（1周）→ 阶段四（持续）
  速查卡+00章    语言特性+新模块    迁移项目
  +01+04+05章    +迁移指南+实战     +生产应用
                 +标准库
```
...
阶段一（2天）→ 阶段二（1.5周）→ 阶段三Track A（1周）→ 阶段四
  速查卡+00章    全部语言特性        自由线程线程安全     确保库兼容
  +01+04+07章    +标准库+迁移        模型+C API变更      +发布兼容版本
                 +C API概览
```
...
阶段一（3天）→ 阶段二（1周）→ 阶段三 Track A+D（2-3周）→ 阶段四
  速查卡+00+02   语言特性+C API    自由线程实现原理     适配自由线程
  +06+07章       +自由线程概念     +C API 深入          +优化性能
                                  +构建系统
```

**必学核心**：QSBR/BRC/关键区段原理、`Py_MOD_GIL`/`Py_MOD_FREE_THREADED`、Limited API 不透明化、PEP 741/757
**源码必看**：Python/qsbr.c、Python/critical_section.c、Include/pycore_*.h

### 🔬 源码学习者路径

```
阶段一（3天）→ 阶段二（2周）→ 阶段三 Track A+B+C（4-8周）
  速查卡+00章    全部特性写一遍      自由线程+JIT+架构
  +06章          +读 InternalDocs    全链路源码阅读
                                  +编译自己的 CPython
```
...
| 资源 | 链接 | 阶段 |
|------|------|------|
| Python 3.14 官方文档（中文） | [docs.python.org/zh-cn/3.14](https://docs.python.org/zh-cn/3.14/) | 一 |
| What's New in Python 3.14 | [docs.python.org/3.14/whatsnew/3.14.html](https://docs.python.org/3.14/whatsnew/3.14.html) | 一 |
| CPython GitHub | [github.com/python/cpython (v3.14.0)](https://github.com/python/cpython/tree/v3.14.0) | 三 |
| PEP 索引 | [peps.python.org](https://peps.python.org/) | 二 |
| CPython Dev Guide | [devguide.python.org](https://devguide.python.org/) | 三 |
| CPython InternalDocs | [InternalDocs/](https://github.com/python/cpython/tree/v3.14.0/InternalDocs) | 三 |
...
| 迁移指南 | [09-migration-guide.md](/concepts/09-migration-guide.md) |
| FAQ | [11-faq-troubleshooting.md](/references/11-faq-troubleshooting.md) |

### 推荐延伸阅读
| 主题 | 资源 |
|------|------|
| 自由线程设计 | Sam Gross "nogil" 演讲、PEP 703 设计文档 |
| Copy-and-Patch JIT | Haoran Xu 论文 "Copy-and-Patch Compilation" (2021) |
| QSBR/RCU | Linux Kernel RCU documentation、URCU 项目 |
