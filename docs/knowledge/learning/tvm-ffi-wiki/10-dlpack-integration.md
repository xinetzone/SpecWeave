...
DLPack 是一种**协议（Protocol）**，而非 API 或 ABI——它定义了张量在内存中的标准布局约定。相关概念参见 [Interface/API/ABI/Protocol 教程](../interface-api-abi-protocol-wiki/00-overview.md)。
...
- [Interface/API/ABI/Protocol 四层技术栈教程](../interface-api-abi-protocol-wiki/00-overview.md) - DLPack 作为协议的设计理念
