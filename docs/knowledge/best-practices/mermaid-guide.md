...
从 [safe-starter.md](file:///d:/spaces/SpecWeave/.agents/templates/mermaid-templates/safe-starter.md) 复制代码块。模板代码块内用 `%%` 注释内置了完整安全规则提醒，编辑时直接可见：
...
其他布局模板见 [mermaid-templates/](file:///d:/spaces/SpecWeave/.agents/templates/mermaid-templates/) 目录（含左→右流程图、分层流程图、决策图、时序图、状态图、思维导图）。
...
项目内置的 [check-mermaid.py](file:///d:/spaces/SpecWeave/.agents/scripts/lib/checks/mermaid.py) 可自动检测以下 10 类问题：
...
[ci-check.ps1](file:///d:/spaces/SpecWeave/.agents/scripts/ci-check.ps1#L43-L51) / [ci-check.sh](file:///d:/spaces/SpecWeave/.agents/scripts/ci-check.sh#L44-L52) 第4步已集成 Mermaid 检查，CI 流水线中 Mermaid 检查失败会阻断提交。提交前建议运行完整 CI 检查：
...
- ⚠️ 节点内 `\n` 不会换行
- ⚠️ 空行严格禁止
- 起步推荐：使用 [safe-starter.md](file:///d:/spaces/SpecWeave/.agents/templates/mermaid-templates/safe-starter.md)
...
- 箭头语法：`->>`（虚线箭头）、`-->>`（虚线响应）
- 模板：[sequence-diagram.md](file:///d:/spaces/SpecWeave/.agents/templates/mermaid-templates/sequence-diagram.md)
...
- 状态描述含空格需加引号
- 迁移标签含空格需加引号
- 模板：[state-diagram.md](file:///d:/spaces/SpecWeave/.agents/templates/mermaid-templates/state-diagram.md)

### mindmap（思维导图）

- 语法与 flowchart 差异较大，注意缩进层级
- 模板：[mindmap.md](file:///d:/spaces/SpecWeave/.agents/templates/mermaid-templates/mindmap.md)
...
| 文档 | 用途 |
|------|------|
| [safe-starter.md](file:///d:/spaces/SpecWeave/.agents/templates/mermaid-templates/safe-starter.md) | ⭐ 推荐起步模板（内置安全注释） |
| [mermaid-templates/](file:///d:/spaces/SpecWeave/.agents/templates/mermaid-templates/) | 8种布局模板目录 |
| [mermaid-safe-coding-rules.md](file:///d:/spaces/SpecWeave/docs/retrospective/patterns/code-patterns/mermaid-safe-coding-rules.md) | 六规则详细说明与正反例 |
| [mermaid-trap-cheatsheet.md](file:///d:/spaces/SpecWeave/docs/retrospective/patterns/code-patterns/mermaid-trap-cheatsheet.md) | 9大陷阱速查表 |
| [check-mermaid.py](file:///d:/spaces/SpecWeave/.agents/scripts/check-mermaid.py) | 自动化检查脚本 |
| [ci-check.ps1](file:///d:/spaces/SpecWeave/.agents/scripts/ci-check.ps1) | CI综合检查脚本 |
| [development-standards.md](file:///d:/spaces/SpecWeave/docs/development-standards.md) | 开发规范（含Mermaid章节） |
