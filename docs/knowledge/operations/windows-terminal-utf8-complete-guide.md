...
推荐使用 `.agents/scripts/setup-utf8-env.ps1` 脚本一键配置：
```powershell
# 在项目根目录执行（推荐：用户级持久配置）
.\.agents\scripts\setup-utf8-env.ps1
```
...
非交互模式（自动执行推荐配置）：
```powershell
.\.agents\scripts\setup-utf8-env.ps1 -NonInteractive
```

预览模式（不实际修改，仅查看将执行的操作）：
```powershell
.\.agents\scripts\setup-utf8-env.ps1 -WhatIf
```
...
```powershell
# 以管理员身份运行后执行
.\.agents\scripts\setup-utf8-env.ps1 -Scope System
```
...
## 自动加载验证（sitecustomize.py）

### 为什么需要验证

项目将 `sitecustomize.py` 迁移至 `.agents/scripts/` 后，其自动加载机制依赖 `PYTHONPATH` 环境变量包含 `.agents/scripts/` 目录。Python 解释器启动时由 `site` 模块尝试自动加载 `sitecustomize.py`，但只有当 `.agents/scripts/` 出现在 `sys.path` 中（通常通过 `PYTHONPATH` 持久化配置实现），新终端会话才能在 Python 启动时自动应用 UTF-8 编码配置，无需业务脚本手动 setup。

如果 `PYTHONPATH` 未持久化，即使在当前会话中能通过手动 `import sitecustomize` 加载成功，新开的终端会话仍会回退到 GBK 编码，导致中文输出乱码、emoji 报错。因此需要一个独立脚本来验证自动加载链路是否完整生效。
...
2. `sitecustomize` 是否已被 `site` 模块自动加载（通过 `sys.modules` 检测）
...
同时检测项目根目录是否存在冲突的 `sitecustomize.py`（根目录副本会遮蔽 `.agents/scripts/` 版本，导致加载到错误文件）。

### 三种场景的预期结果

| 场景 | 预期退出码 | 说明 |
|------|-----------|------|
| 裸终端（未配置 PYTHONPATH） | `1` | sitecustomize 未被自动加载，新会话无法享受 UTF-8 自动配置 |
| 已持久化 PYTHONPATH（用户级/系统级） | `0` | sitecustomize 在 Python 启动时被正确自动加载，stdout/stderr 编码为 utf-8 |
| 已加载 profile.ps1 的当前会话 | `0` | profile.ps1 已设置 PYTHONPATH，sitecustomize 被自动加载 |

> 💡 **验证策略**：建议先在**裸终端**（未加载任何 profile）运行脚本确认退出码为 1，再运行 `.\.agents\scripts\setup-utf8-env.ps1` 持久化配置后**新开终端**重新验证，确认退出码变为 0。

### 退出码含义

| 退出码 | 含义 | 处理建议 |
|--------|------|---------|
| `0` | 自动加载正常 | 无需操作，sitecustomize.py 在 Python 启动时被正确自动加载 |
| `1` | 需配置 | 运行 `.\.agents\scripts\setup-utf8-env.ps1` 持久化 PYTHONPATH，或将 `.agents/scripts/` 手动加入 PYTHONPATH 环境变量 |
| `2` | 存在冲突文件 | 项目根目录存在 `sitecustomize.py`，需删除根目录副本后重新验证 |

### CI 集成（--json 模式）

CI/CD 流水线可使用 `--json` 模式获取结构化检测结果，便于自动化判断与告警：

```powershell
python .agents/scripts/verify-sitecustomize-autoload.py --json
```

JSON 输出包含顶层 `ok`、`exit_code`、`exit_code_meaning` 字段以及 5 项检测的详细结果（`sys_path`、`autoload`、`file_path`、`encoding`、`root_conflict`），每项含 `pass` 布尔值与 `detail` 详情。CI 可根据 `exit_code` 判定是否阻断流水线（非 0 即失败）。
...
仅当前会话临时设置（不持久化）：
```powershell
.\.agents\scripts\setup-utf8-env.ps1 -Scope Session
```
...
| 重启终端后配置失效 | 未持久化配置 | 运行`setup-utf8-env.ps1`（位于`.agents/scripts/`）选择User或System范围 |
...
- 相关脚本：
  - `.agents/scripts/setup-utf8-env.ps1` - 一键配置主脚本
  ...
  - `.agents/scripts/sitecustomize.py` - Python启动时自动加载的UTF-8环境配置（需PYTHONPATH包含`.agents/scripts/`）
  - `.agents/scripts/verify-sitecustomize-autoload.py` - sitecustomize.py 自动加载验证脚本（退出码 0/1/2，支持 `--json`）
