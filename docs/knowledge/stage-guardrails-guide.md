...
category: operations
tags: [stage-guardrails, 阶段守卫, 运行时拦截, SG-LOG, guard-operation, 强制执行, 工作流]
date: "2026-06-29"
status: stable
author: "AI团队"
summary: "阶段守卫运行时强制执行层（GuardrailRuntime）的完整使用指南，涵盖8阶段权限速查、必读文档清单、典型日志示例、常见拦截原因与解决方案、CLI工具手册，以及运行时+离线双层验证闭环。"
---
...
- **定义层**：`.agents/rules/stage-guardrails.md` 定义8个阶段、角色权限、退出标准
- **运行时层**：GuardrailRuntime 在操作执行前实时拦截越界行为（本指南重点）
- **验证层**：离线分析工具 `check-stage-guardrails.py` 事后解析日志检测异常

相关代码位于 `.agents/scripts/lib/stage_guardrails/` 模块，包含4个核心组件。
...
if out.is_intercept:
    print(out.user_message)  # ⚠️ 阶段守卫拦截...
    # 终止操作，先走需求澄清流程
...
| 阶段 | 必读文档 |
|------|---------|
| S1 | `AGENTS.md`、`.agents/rules/stage-guardrails.md`、`docs/development-standards.md` |
| S2 | `docs/knowledge/`（架构决策相关）、`.agents/rules/stage-guardrails.md`、S1产出的需求文档 |
| S3 | `.agents/workflows/feature-development.md`、S1+S2产出物 |
| S4 | `docs/development-standards.md`（编码规范）、S3任务分配表 |
| S5 | `docs/development-standards.md`（测试规范）、S4代码 |
| S6 | `.agents/roles/reviewer.md`、S4代码+S5测试 |
| S7 | `.agents/protocols/dependency-management.md` |
| S8 | 全部阶段产出物 |
...
# 2. orchestrator审批（批准后自动进入S2，角色自动切换为architect）
out = rt.approve_jump(record.jump_id, 'orchestrator',
...
# 自动从S1推进到S2，角色从orchestrator切换为architect
```
...
运行时实时拦截和演示工具，位于 `.agents/scripts/check-stage-guardrail-runtime.py`。
...
事后日志分析工具，位于 `.agents/scripts/check-stage-guardrails.py`。解析会话日志中的 `[SG-LOG]` 和 `[PDR-LOG]`，检测异常情况。
...
- [阶段守卫规则定义](../../.agents/rules/stage-guardrails.md)
- [功能开发工作流](../../.agents/workflows/feature-development.md)
- [前置文档读取协议（PDR）](../../.agents/protocols/pre-document-reading.md)
- [阶段守卫离线分析工具check-stage-guardrails.py](../../.agents/scripts/check-stage-guardrails.py)
- [运行时模块源码](../../.agents/scripts/lib/stage_guardrails/)
- [开发流程阶段守卫规范](../../.agents/rules/stage-guardrails.md)
