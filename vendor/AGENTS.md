...
- **git 子模块(third_party)**:第三方只读依赖,外部独立项目,通过 gitlink 固定 commit,禁止本地修改
- **git 子模块(owned_collab)**:自有协作项目,允许子模块内开发,通过 gitlink 跟踪分支
- **手动管理依赖**:不通过子模块引入的依赖,仅提交元数据(README.md),源码/二进制通过 .gitignore 忽略

> **.gitignore 策略**:vendor/ 目录本身不被根 .gitignore 忽略,子模块 gitlink 和主权区元数据文件正常纳入版本控制;手动管理依赖的源码须自行配置忽略规则。
...
| podman-compose | third_party | 无（第三方项目） | containers 官方容器编排工具链：podman-compose 声明式编排（第三方只读依赖，gitlink pin commit） |
| podman-py | third_party | 无（第三方项目） | containers 官方容器编排工具链：podman-py SDK（第三方只读依赖，gitlink pin commit） |
| toolbox | third_party | 无（第三方项目） | containers 官方容器编排工具链：toolbox 工具（第三方只读依赖，gitlink pin commit） |
...
flexloop 的验证与检查脚本存放在 [vendor/flexloop/apps/chaos/.agents/scripts/](flexloop/apps/chaos/.agents/scripts/),包含 25+ 验证脚本。SpecWeave 主权区的脚本不依赖这些,需要跨边界调用时通过沙箱工具运行(见 [VENDOR-INTEGRATION.md 第 4.6 节](../.agents/VENDOR-INTEGRATION.md))。
...
| vendor/README.md | SpecWeave 主权区 | ✅ 是 | vendor 依赖总览 |
...
5. **不修改**:不直接修改 vendor 内的 skill 文件,如需改进走[贡献上游流程](../.agents/VENDOR-INTEGRATION.md)
...
| 子模块未初始化 | 第三层入口 | 读取 `vendor/flexloop/AGENTS.md` 失败或目录为空 | 运行 `git submodule update --init vendor/flexloop` 初始化子模块后重试;仍失败则检查 gitlink 是否损坏(见 [VENDOR-INTEGRATION.md 版本控制章节](../.agents/VENDOR-INTEGRATION.md)) |
| 任务范围未识别 | 任务范围判断 | flexloop 任务范围既非 Chaos 也非 Rebirth | 回退到 `vendor/flexloop/AGENTS.md` 的「30 秒路由」,提示用户明确任务范围或确认是否需要新增应用入口 |
| skill 索引定位失败 | 可用资产索引 | 索引无匹配 skill | 对照 `vendor/flexloop/apps/chaos/.agents/skills/` 实际目录检查 skill 是否已被移除或重命名;若为新增需求,走[贡献上游流程](../.agents/VENDOR-INTEGRATION.md) |
...
- [VENDOR-INTEGRATION.md](../.agents/VENDOR-INTEGRATION.md) — 外部子模块协同集成方案(边界划分/版本控制/更新同步/测试隔离/模式萃取)
