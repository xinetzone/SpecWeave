...
- **FR-7（overlay .env.example）**: 键集合与 compose 插值键严格一致（共 **17 键**：16 个生效键 XMNN_IMAGE_TAG、XMNN_CONTAINER_NAME、XMNN_SSH_PORT(2223)、XMNN_JUPYTER_PORT(8890)、XMNN_WORKSPACE、NPU_TVM_PATH、NPUUSERTOOLS_PATH、MODELS_PATH、USER_PASSWORD、JUPYTER_TOKEN、SSH_PUBLIC_KEY、GRANT_SUDO、OMP_NUM_THREADS、NUITKA_JOBS、PIP_MIRROR、CONDA_MIRROR + 1 个注释态构建参数 BASE_IMAGE）；头部注释优先级链（shell > root client .env > compose 默认；本文件仅供裸 podman-compose）与 Windows 门禁/WSL 路径说明。
