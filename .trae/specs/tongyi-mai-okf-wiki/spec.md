...
- **Affected specs**: 无既有 spec 冲突；本 spec 新建于 `.trae/specs/tongyi-mai-okf-wiki/`
- **Affected code/产物**:
...
1. **R 阶段（事实采集）**：逐模块阅读源码，提取编号事实 F-xxx 写入 `.trae/specs/tongyi-mai-okf-wiki/facts-*.md`，零推测（禁止"用于/目的是"推断词）；G1 门
