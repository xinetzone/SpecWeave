...
source_url: "https://mp.weixin.qq.com/s/ES_KncqKLiQxIzaEZ-58gg?bar_style_type=2&from=industrynews&color_scheme=light#rd"
extracted_at: "2026-09-08T12:00:00Z"
pub_date: "2026-06-18 15:09"
author: "开源日记"
publisher: "开源日记（微信公众号）"
---
...
大部分团队的"数据库版本控制"就是指定期做备份再加上 binlog 回放。有如下两个问题：
...
根据官方 Sysbench 测试结果，在读写方面，Dolt 已经接近于 MySQL 了。在 TPC-C 测试中，TPS 大约为 MySQL 的 54%，对于大部分的应用来说是够用的。
...
整个过程跟 Git 差不多，只不过操作的对象是数据而非常见的文件。
...
分支、合并、回滚等 Git 中的操作，在数据库中也可以直接使用了。
