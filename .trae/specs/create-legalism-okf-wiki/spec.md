...
教程 SHALL 记录 seven-concepts 场景4 方法论痕迹：facts 文件合计 ≥50 条编号事实且无因果推断词（G1，分布于 `.trae/specs/create-legalism-okf-wiki/facts-*.md`，其中《韩非子》≥20 条、其余每部 ≥10 条）、insights ≥4 条带四元组洞察（G2）、≥2 个可复用阅读模式含触发场景/核心步骤/反模式/迁移示例（G3），并在提交前经对抗审查（V）完成原文与事实抽查。
