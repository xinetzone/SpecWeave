...
> 执行链路遵循 `source-code-to-okf-wiki` 的 R→I→E→V→C 五阶段 + `seven-concepts-cmd` 质量门编排。所有产出物写入 `projects/awesome-okf-xs/doc/bundles/`（子模块内），规划文档与原始文本存放 `.trae/specs/classics-knowledge/jianshu-blogs-to-okf-wiki/`。
