#!/usr/bin/env python3
"""Thread scaling test for v2 Conv on 16-core system."""
...
def run_one(proto, model, mean_vals, omp_t, blas_t, batch=1, warmup=8, iters=20, bind="close"):
    code = f'''
import os, time
...
avg=np.mean(lats); med=np.median(lats); p95=np.percentile(lats,95); p99=np.percentile(lats,99)
ps=avg/batch; fps=1000.*batch/avg
print(f"RESULT avg={{avg:.1f}} med={{med:.1f}} p95={{p95:.1f}} p99={{p99:.1f}} ps={{ps:.1f}} fps={{fps:.2f}}")
'''
    ...
    if err:
        print(f"  ERR: {err[-1][:200]}", file=sys.stderr)
    return None

for model_name, (proto, model, mean_vals) in MODELS.items():
    print(f"\n{'='*80}")
    print(f"  {model_name} batch=1 — Thread Scaling (BLAS=1, OMP_PROC_BIND=close)")
    print(f"{'='*80}")
    print(f"{'Threads':>8} {'Avg(ms)':>8} {'P50(ms)':>8} {'P95(ms)':>8} {'P99(ms)':>8} {'FPS':>8} {'Spdup':>7} {'Eff':>6}")
    print("-"*80)
    base_dt = None
    for threads in [1, 2, 4, 8, 16]:
        ...
        if r:
            ...
            spd = base_dt / r["avg"]
            eff = spd / threads * 100
            print(f"{threads:8d} {r['avg']:8.1f} {r['med']:8.1f} {r['p95']:8.1f} {r['p99']:8.1f} {r['fps']:8.2f} {spd:6.2f}x {eff:5.1f}%")
        else:
            print(f"{threads:8d}    FAILED")

    print(f"\n  {model_name} batch=4 — Thread Scaling (BLAS=1, OMP_PROC_BIND=close)")
    print(f"{'Threads':>8} {'Total(ms)':>10} {'Per-sample':>11} {'FPS(total)':>11} {'FPS/sample':>11} {'Spdup':>7}")
    print("-"*80)
    base_ps = None
    for threads in [1, 2, 4, 8, 16]:
        ...
        if r:
            ...
            print(f"{threads:8d} {r['avg']:10.1f} {r['ps']:10.1f}ms {fps_total:11.2f} {r['fps']:11.2f} {spd:6.2f}x")
        else:
            print(f"{threads:8d}    FAILED")
