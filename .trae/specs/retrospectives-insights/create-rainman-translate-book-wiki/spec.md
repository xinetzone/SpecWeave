...
## Constraints
- **Technical**: 文档必须使用 Markdown 格式，遵循项目命名规范，实际放置在 `docs/knowledge/learning/03-agent-platforms-tools/` 目录下（遵循项目现有 wiki 文件约定，同类 Agent 平台工具 wiki 均位于此目录）
...
- **Notes**: 文档实际放置在 `docs/knowledge/learning/03-agent-platforms-tools/rainman-translate-book-wiki/` 目录下，采用原子化目录结构（索引页 + 8 个原子文件 + 9 个 TOML 元数据文件）
...
| F02 | 子代理实际创建文件于 `docs/knowledge/learning/03-agent-platforms-tools/rainman-translate-book-wiki/` |
...
| F07 | 原子提交尚未执行 |
| F08 | 知识库索引（docs/knowledge/README.md）中尚未添加本教程条目 |
...
- **现象**：spec 定义 `docs/knowledge/learning/`，子代理实际输出到 `docs/knowledge/learning/03-agent-platforms-tools/`
...
- **现象**：spec 中引用 `docs/knowledge/README.md`，但实际知识库 README 位于 `docs/knowledge/README.md`
- **根因**：项目知识库管理系统的实际入口在 `.agents/` 下，而非根目录 `docs/`
- **影响**：索引条目尚未添加到实际 README 中，需要补充。
- **建议**：更新 tasks.md 中 L4 的知识库索引更新任务，明确指向 `docs/knowledge/README.md`
