...
flowchart TD
    ...
    AndroidCheck -->|.apk| ABI["unzip -l 扫描 lib/ 目录\n检查 x86_64 ABI"]
    AndroidCheck -->|无| Upload
    ABI --> Upload

    Upload["上传构建到 CI API\nmultipart/form-data (snake_case)"]
    Upload --> Trigger["触发测试运行\nJSON (camelCase)"]
    Trigger --> Output["设置 outputs\nbatch-id, status"]
    Output --> Async["Action 退出\n测试在后台执行"]
    Async --> CheckRun["Check Run 更新\n回贴到 PR"]

    Cancel{"cancel-previous-runs\n且是发布分支?"}
    Trigger --> Cancel
