...
[ -z "$_PKG_PATH" ] && fail "No built package found"

info "Installing package: $_PKG_PATH"
conda install -y --offline --use-local --force-reinstall "$_PKG_PATH" 2>&1 | tail -5
...
if [ -n "$_CAFFE_SO" ] && [ -f "$_CAFFE_SO" ]; then
    ...
    echo "  --- ldd (key deps) ---"
    ldd "$_CAFFE_SO" 2>/dev/null | grep -E 'libtvm_ffi|libopenblas|libprotobuf|not found|caffe_ffi' | sed 's/^/    /'
    echo "  --- end ---"
    ...
    ldd "$_CAFFE_SO" 2>/dev/null | grep -qi 'libtvm_ffi' && \
...
echo ""
echo "Installed package files (sample):"
ls -la "$SP_DIR/caffe_ffi/" | grep -v __pycache__ | head -20
