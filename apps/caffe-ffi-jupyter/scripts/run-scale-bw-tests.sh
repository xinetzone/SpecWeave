...
echo "=== Rebuild caffe-ffi (source code changed) ==="
cd "$CAFFE_FFI_DIR"
rm -rf build
...
SKBUILD_CMAKE_ARGS="$CAFFE_FFI_CMAKE_ARGS" pip install --no-cache-dir --no-build-isolation -e "$CAFFE_FFI_DIR" 2>&1 | tail -15
