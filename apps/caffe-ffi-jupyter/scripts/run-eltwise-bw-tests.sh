...
echo "=== Step 1: Rebuild caffe-ffi (Eltwise Backward added) ==="
cd "$CAFFE_FFI_DIR"
rm -rf build
...
echo ""
echo "=== Step 3: Run Eltwise Backward tests ==="
...
echo ""
echo "=== Step 4: Regression - Scale + Bias + Dropout backward tests ==="
python -m pytest test_scale_backward.py test_bias_backward.py test_dropout_backward.py -v --tb=short 2>&1 | tail -30
