...
#
# Usage (from WSL bash):
#   bash apps/caffe-ffi-jupyter/scripts/fix-conv-test-env.sh
# =============================================================================
set -euo pipefail
...
if docker ps --format '{{.Names}}' | grep -q "^${CONTAINER}$"; then
    ...
    echo "     Fix: cd apps/caffe-ffi-jupyter && docker-compose up -d"
...
if echo "${ENV_CHECK}" | grep -q "Python 3.14"; then
    ...
    echo "     Fix: Rebuild image: cd apps/caffe-ffi-jupyter && bash scripts/build.sh --cn"
    exit 1
fi
...
python -c 'from caffe_ffi import _ffi_api; print(\"cpp_available:\", _ffi_api.is_available())' 2>&1")
    if echo "${FFI_CHECK2}" | grep -q "cpp_available: True"; then
        ...
        echo "       cd apps/caffe-ffi-jupyter && docker-compose build --no-cache && docker-compose up -d"
    fi
fi
...
if [ "${FORCE_REBUILD:-0}" = "1" ] || [ "${REBUILD_CPP:-0}" = "1" ]; then
    ...
    docker exec "${CONTAINER}" bash /SpecWeave/apps/caffe-ffi-jupyter/scripts/_rebuild.sh 2>&1 | tail -40
...
echo "  REBUILD_CPP=1 bash apps/caffe-ffi-jupyter/scripts/run-conv-backward-tests.sh"
echo "=========================================================================="
