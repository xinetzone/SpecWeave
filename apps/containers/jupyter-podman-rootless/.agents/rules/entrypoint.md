...
> 宿主侧的 `<host-uid>` 与 socket 路径由 invoke 任务经共享包
> `jpman_common.connection.host_runtime_uid()`/`podman_sock_path()` 推导后
>  bind-mount/注入（见 invoke-tasks.md「UID 推导历史教训」）；entrypoint
> 只消费已挂载的 socket 与 `HOST_PODMAN_SOCK`，不在容器内臆测宿主 UID。

- 在 devuser 可控目录建符号链接指向宿主 socket，导出 `CONTAINER_HOST` / `XDG_RUNTIME_DIR`
- **独立 shell 桥接（Containerfile Layer 3 烘焙，2026-09-12）**：entrypoint 的 `export CONTAINER_HOST` 只进入 supervisord→jupyter 进程子树；`podman exec` 继承容器 config env，但 **sshd+PAM 派生的 SSH 会话会清洗环境**（实测 `HOST_PODMAN_SOCK`/`CONTAINER_HOST`/`XDG_RUNTIME_DIR` 三变量在 SSH login shell 中全空），Jupyter·IDE 终端同样属于独立 shell——它们都会静默落回容器内 rootless 并撞 `newuidmap ... Operation not permitted`。镜像必须同时提供：① `/etc/profile.d/80-podman-host-socket.sh`（登录 shell）；② devuser `~/.bashrc` source 同一文件（非登录交互 shell）；③ `ENV BASH_ENV=/etc/profile.d/80-podman-host-socket.sh`（非交互非登录 bash / cron，覆盖 podman exec 子进程）；④ 同值写入 `/etc/environment` 的 `BASH_ENV=`——**容器 config 的 ENV 同样会被 sshd 清洗**，只有 PAM pam_env（sshd 栈 `session required pam_env.so` 默认读 /etc/environment）注入的 BASH_ENV 才能覆盖 `bash script.sh` 脚本文件形态（cron 同理）；⑤ **entrypoint 运行时向 `/etc/ssh/sshd_config` 追加 `SetEnv CONTAINER_HOST=unix://<解析路径>`（写后 `sshd -t` 校验、失败回滚）**——实测 bash 仅在执行**脚本文件**时读 BASH_ENV，`bash -c "命令字符串"`（`ssh host "cmd"` 与 sshd 最外层 shell）既不读 BASH_ENV 也不读 profile/.bashrc，SetEnv 是唯一不经任何 shell 启动文件的注入点；回退分支必须同步删除事实文件与 SetEnv 行（防御同容器切换启动模式留下陈旧指向）。**桥接脚本必须做四级解析，不得只依赖 env**：显式 `CONTAINER_HOST`（最高优先，tcp/远程/指向本地任意 daemon 的唯一逃生舱）→ `HOST_PODMAN_SOCK` env → `/etc/podman-host-sock.path` 事实文件（entrypoint B-scheme 分支把解析到的挂载路径写入此文件，644，覆盖 SSH 环境丢失 + 非标准挂载点）→ 标准路径 `/run/user/$(id -u)/podman/podman.sock` 自探测；每级都要求是 socket（`-S`）才导出，全部缺失时 no-op（回退分支零影响）。**验证纪律：禁止只用 `podman exec` 验证独立 shell 行为——必须经真实 SSH（`ssh -p <port> devuser@host`）或 `env -i` 干净环境复核**，podman exec 与 SSH 的环境模型不同（2026-09-12 第二轮故障教训）
...
- **socket 属组衔接（EACCES 修复 · 见 `apps/containers/client/docs/04-troubleshooting-guide.md` C-I2）**：宿主 socket（宿主 `<uid>:<gid>` 0660）经 userns 映射进容器后呈现为 `root:root 0660`，devuser 是非 root UID（固定 1000，≠0）且未入组 → `socket.connect()` 直接 EACCES。故用 `stat -Lc %G`（空/UNKNOWN 回退 `%g`）解析 socket 属组并 `usermod -aG <组> ${NON_ROOT_USER}`；`stat` 失败必须 `log_warn`，禁止静默
