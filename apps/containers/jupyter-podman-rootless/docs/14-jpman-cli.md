...
| 命令 | 说明 |
|------|------|
| `jpman start` | 启动容器（幂等）：若已运行则直接显示信息；若存在但停止则删除重建；自动等待健康检查通过 |
| `jpman stop` | 停止并删除容器 |
| `jpman restart` | 重启（先 stop 再 start） |
...
| 环境变量 | 默认值 | 说明 |
|----------|--------|------|
| `JUPYTER_CONTAINER_NAME` | `jupyter-chaos-test` | 容器名称 |
| `JUPYTER_IMAGE` | `localhost/jupyter-podman-rootless:latest` | 镜像标签 |
| `JUPYTER_SSH_PORT` | `2222` | SSH 端口映射 |
| `JUPYTER_PORT` | `8888` | Jupyter 端口映射 |
| `JUPYTER_PASSWORD` | `devpass123` | 用户密码 |
| `JUPYTER_TOKEN` | `chaostest2026` | Jupyter token |
| `JUPYTER_WORKSPACE` | `SpecWeave根目录` | 工作区挂载路径 |
