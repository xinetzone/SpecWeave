...
| **构建优化** | 多阶段构建（3 阶段运行时链 + toolbox-builder aux 阶段，内嵌编排工具），final 内按变化频率分层，内置计时器 + 语法验证 |
| **运行时检测** | 自动检测 podman/docker，WSL2 路径自动转换 |
| **编排方式** | 宿主机 invoke 三层后端自动降级：podman-compose 声明式（优先）→ podman-py SDK → CLI（pip 安装，与镜像内嵌版本独立）；也可直接使用 `podman-compose up -d` |
| **内嵌编排工具** | 镜像内置 podman-compose / podman-py SDK / toolbox（SpecWeave 根 vendor/ 子模块固定 commit 引入，见 [17-upstream-tools.md](17-upstream-tools.md)） |
...
| **Toolbx 兼容** | 镜像满足 Toolbx 规范（LABEL + /run/host + markers + capsh）；宿主 `toolbox create/enter` 经专用 `:toolbx` 变体支持（见 [07-toolbx-passthrough.md](07-toolbx-passthrough.md)） |
