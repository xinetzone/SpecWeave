...
- 因此本目录下的 `rules/` 保留消费端独有的 4 个主题文件；其余构建端主题（Containerfile/entrypoint/ml-models 等）一律不重复，相关需求回退到父级工作区
...
├── rules/                 ← 项目特有规则（单一职责，按主题拆分；4 个实文件 = 消费端独有）
...
│   └── quant-overlay.md   ← quant.* podman-compose 工作负载栈规范（双门禁、三必需映射、深合并、镜像守卫契约）
...
| 环境变量模板（三清单） | [../.env.example](../.env.example) | 容器级 9 项 + SDK 级 4 项 + quant 栈 12 项完整带注释 |
