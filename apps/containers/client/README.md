...
## 13. 工作负载叠加层：xmnn-dev（xmnn.* 命令，opt-in）
...
- **源码路径**：默认挂载仓库根 `external/chaos/{npu_tvm,npuusertools,models}`；
  ...
  指向 WSL 原生克隆。
- **Windows 原生门禁**：同 quant.*（WSL2 内运行或 `invoke env.run-cmd`）。
...
## 13.5 工作负载叠加层：agent-monetize-dev（monetize.* 命令）
...
- Windows 原生门禁同 quant/xmnn（WSL2 或 `invoke env.run-cmd`）。
