...
> **📎 迁移溯源（2026-09-13）**：本文档技术内容（选型/精度/基准方法论，与容器运行时无关）逐字迁自
> Docker 谱系 `apps/docker-images/devcontainer-base/variants/onnx-quantized/QUANTIZATION-BEST-PRACTICES.md`；
> 运行载体已变为 Podman rootless 叠加镜像 `localhost/onnx-quantized:latest`（见
> [../README.md](../README.md)），容器内 Python 路径仍为 `/opt/conda/envs/main/bin/python`。
> 基准脚本属 Docker 谱系共享资产（未迁移），链接已改指源树，可在把源树挂入 /workspace 后运行。
...
| 脚本 | 用途 | 输出 |
|------|------|------|
| [run_full_benchmark.py](../../../../../docker-images/devcontainer-base/scripts/run_full_benchmark.py) | 4类模型×5格式全量对比 | JSON结果 + 控制台表格 |
| [compare_qdq_vs_qoperator.py](../../../../../docker-images/devcontainer-base/scripts/compare_qdq_vs_qoperator.py) | QDQ vs QOperator专项对比 | JSON结果 + 性能差异百分比 |

**运行方式**（基准脚本属 Docker 谱系共享资产，未迁入叠加镜像；先把源树挂入 /workspace）：
```bash
# 启动栈时让 QUANT_WORKSPACE 指向 SpecWeave 根（或单独挂载源树），容器内：
cd /workspace/apps/docker-images/devcontainer-base
/opt/conda/envs/main/bin/python scripts/run_full_benchmark.py
# 结果输出到 apps/docker-images/devcontainer-base/reports/benchmark-results.json

# QDQ/QOperator专项对比
/opt/conda/envs/main/bin/python scripts/compare_qdq_vs_qoperator.py --model conv --shape 1,3,32,32
```
...
- [run_full_benchmark.py](../../../../../docker-images/devcontainer-base/scripts/run_full_benchmark.py) - 全量基准测试脚本（Docker 谱系共享资产）
- [compare_qdq_vs_qoperator.py](../../../../../docker-images/devcontainer-base/scripts/compare_qdq_vs_qoperator.py) - QDQ/QOperator对比脚本（Docker 谱系共享资产）
- [benchmark-report.html](../../../../../docker-images/devcontainer-base/reports/benchmark-report.html) - 交互式性能对比图表（Docker 谱系）
