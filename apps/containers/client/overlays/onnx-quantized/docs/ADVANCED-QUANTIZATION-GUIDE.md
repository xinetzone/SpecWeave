...
> **📎 迁移溯源（2026-09-13）**：本指南量化技术内容逐字迁自 Docker 谱系
> `apps/docker-images/devcontainer-base/variants/onnx-quantized/ADVANCED-QUANTIZATION-GUIDE.md`；
> 运行载体已变为 Podman rootless 叠加镜像（`localhost/onnx-quantized:latest`，
> 经 [../README.md](../README.md) 的 `invoke quant.*` 或 podman-compose 启动）。
> main 环境路径 `/opt/conda/envs/main/bin/python` 与 torch 缺席约束在两谱系完全一致；
> 指南中所有代码仍可直接运行。
...
- [onnx-quantized 叠加层 README](../README.md) - 快速开始（invoke quant.* / podman-compose）与能力说明
- [量化最佳实践](QUANTIZATION-BEST-PRACTICES.md) - QDQ vs QOperator 选型、精度调优与基准
- [Docker 谱系源变体（只读参考）](../../../../../docker-images/devcontainer-base/variants/onnx-quantized/README.md) - 原 Docker DinD 载体的发布说明 v2.0.0
