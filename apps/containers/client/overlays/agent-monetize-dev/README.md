...
3. **Windows 原生自动桥接**：Windows 原生 CPython 自动桥接到 WSL 发行版
   （默认 `podman-machine-default`——client 专用 rootless 发行版，与
   flapping 的 Podman Desktop 默认 machine 相互独立、镜像存储不互通；
   `COMPOSE_WSL_DISTRO` 可指定、`none` 关闭回退门禁）；也可手动在 WSL2 发行版内运行，或进入 client 自举容器。
