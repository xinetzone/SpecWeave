...
| **预检机制** | ❌ 无 | **7项预检**（含脚本存在性+ABI冲突风险） | 🆕 可靠性 |
| **冒烟测试** | ❌ 手动 | **10项自动验证**（含free-threading检测+C扩展ABI验证+8线程加速检测） | 🆕 CI就绪 |
| **自动基准测试** | ❌ 无 | **ft-benchmark.sh**（构建后自动记录8线程加速比到JSONL日志，阈值校验） | 🆕 性能追踪 |
| **C扩展ABI验证** | ❌ 无 | **verify-cext.sh**（Dockerfile内置+容器内诊断，检测cp314/cp314t混用+defaults channel） | 🆕 兼容性保障 |
...
- **构建前预检（7项）**：Docker状态/BuildKit/磁盘空间（≥10GB）/Dockerfile存在性/Miniforge缓存/构建参数/必需脚本存在性
...
- **自动冒烟测试**：`--test` 参数构建后自动启动容器，执行10项验证（含C扩展ABI兼容性检查）
- **自动性能基准**：冒烟测试通过后自动调用 `ft-benchmark.sh`，记录8线程加速比到 `logs/benchmarks/ft-benchmark-YYYYMMDD.jsonl`，阈值校验（≥2.0x）
- **C扩展验证**：Dockerfile Stage 7内置6项C扩展内联检查 + `verify-cext.sh` 全量检查，镜像构建时自动验证C扩展加载能力
...
|---|-------|------|
| 1 | Python版本正确（3.14.x） | ✅ |
| 2 | Python构建类型（cp314t free-threading，GIL默认禁用） | ✅ |
| 3 | conda命令可用 | ✅ |
| 4 | libmamba solver配置正确 | ✅ |
| 5 | conda channels仅含conda-forge（无defaults） | ✅ |
| 6 | pip可正常安装包 | ✅ |
| 7 | 关键包导入（os, sys, json） | ✅ |
| 8 | conda create/solve可执行（30s超时） | ✅ |
| 9 | free-threading demo并发性能（8线程加速≥2x） | ✅ |
| 10 | C扩展ABI兼容性（verify-cext.sh：brotli/cffi/sqlite3/ssl/zlib/hashlib） | ✅ |
...
### 4.6 自动化验证脚本验证结果（2026-08-14）

镜像 `devcontainer-base:conda-libmamba-ft` (e799ce34a46d) 上执行 `verify-cext.sh` 全量C扩展验证，**11/11项通过**：

| # | 验证项 | 结果 |
|---|-------|------|
| 1 | brotli compress/decompress roundtrip | ✔ PASS |
| 2 | cffi C FFI interface | ✔ PASS |
| 3 | _sqlite3 内置C扩展 | ✔ PASS |
| 4 | _ssl 内置C扩展 | ✔ PASS |
| 5 | zlib 内置C扩展 | ✔ PASS |
| 6 | hashlib (OpenSSL) C扩展 | ✔ PASS |
| 7 | json 纯Python基准 | ✔ PASS |
| 8 | ipykernel Jupyter内核导入 | ✔ PASS |
| 9 | SOABI一致性（cpython-314t匹配free-threading构建） | ✔ PASS |
| 10 | cp314/cp314t ABI混用检测（conda包） | ✔ 无混用 |
| 11 | defaults channel检测 | ✔ conda-forge only，安全 |

**性能基准（不同计算规模下的8线程加速比）**：

| 素数范围 | 单线程时间 | 8线程加速(threading.Thread) | 8线程加速(ThreadPool) | 最佳加速比 |
|---------|-----------|---------------------------|---------------------|-----------|
| 50K (冒烟) | 0.010s | 2.78x | 3.01x | 3.01x |
| 500K (中等) | 0.223s | **4.83x** | 4.73x | **4.83x** |
| 2M (完整) | 1.647s | **5.30x** | 5.28x | **5.30x** |

> **注意**：小规模测试(50K)线程创建/同步开销占比大，加速比仅~3x；中等规模(500K)即可稳定达到4.8x以上，超大规模(2M)可达5.3x。`ft-benchmark.sh` quick模式阈值设为2.0x，仅验证"多线程有加速"而非"加速比高低"；如需验证高加速比使用`--full`模式（2M素数）。

**verify-cext.sh 用法**：
```bash
# 容器内诊断
docker exec <container> bash /usr/local/bin/verify-cext.sh

# 指定Python路径
PYTHON=/opt/conda/envs/main/bin/python bash scripts/verify-cext.sh
```

**ft-benchmark.sh 用法**：
```bash
# 快速冒烟基准（50K primes，CI适用）
bash scripts/ft-benchmark.sh --quick

# 完整性能基准（2M primes）
bash scripts/ft-benchmark.sh --full -t 4.0

# 自定义镜像和阈值
bash scripts/ft-benchmark.sh -i myimage:tag -r 500000 -t 3.0
```

日志输出到 `logs/benchmarks/ft-benchmark-YYYYMMDD.jsonl`（JSON Lines格式），每条记录包含时间戳、镜像ID、Python版本、构建类型、加速比、阈值、CPU核心数等。
...
| 5 | **Miniforge3替代Miniconda3 / defaults channel冲突风险**：从Miniconda3(defaults channel)迁移到Miniforge3(conda-forge only)。⚠️ **defaults channel(Anaconda)仅提供cp314标准构建，不提供cp314t free-threading构建**——添加defaults后conda安装包可能将Python从cp314t降级为cp314，导致free-threading功能失效 | 依赖defaults channel包的用户；`conda config --add channels defaults` | ⚠️ ABI冲突风险 | **强烈建议保持conda-forge only**；构建时Dockerfile Stage 7内置verify-cext.sh自动检测cp314/cp314t混用和defaults channel；build.sh预检检查脚本完整性；如误加defaults导致降级，verify-cext.sh会告警；如需Anaconda特定包必须手动添加defaults channel，且安装后务必验证`sysconfig.get_config_var('Py_GIL_DISABLED') == 1`；或创建独立env使用cp314 |
...
| 11 | **libgcc 16.1.0自动升级**：conda创建main环境时libmamba求解器自动选择libgcc 16.1.0（Miniforge3 base环境为15.2.0） | C扩展运行时兼容性 | ✅ 已验证安全 | GCC运行时向后兼容；5个C扩展包(brotli/cffi/cmarkgfm/PyYAML/psutil)均正常加载运行；Dockerfile Stage 7内置C扩展验证步骤；详见§4.5 C扩展兼容性验证 |
| 12 | **小规模benchmark加速比偏低**：50K primes等极小规模CPU任务8线程加速仅~3x，低于4x预期 | ft-benchmark.sh quick模式 | ℹ️ 物理限制 | 线程创建/同步开销在极小工作负载下占比大；500K primes可达4.8x，2M primes可达5.3x；quick模式阈值设为2.0x仅验证"有加速"；验证高加速比使用`ft-benchmark.sh --full` |
...
| `45882bb2` | fix(devcontainer): Miniforge3替代Miniconda3，双环境架构(base py313/main py314.6 cp314t)，Jupyter free-threading kernel |
| `daf02abe` | docs(devcontainer): 更新RELEASE-v2.md性能验证报告与GCC兼容性分析 - 5.30x加速确认 |
| `169d036f` | feat(devcontainer-base): 新增ft-benchmark.sh自动性能基准+verify-cext.sh C扩展ABI验证脚本，修正GCC版本注释，Dockerfile Stage 7内置C扩展验证，build.sh预检升级7项+冒烟测试升级10项+自动benchmark集成 |
