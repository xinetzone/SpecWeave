...
echo "=== Step 1: Rebuild caffe-ffi (Scale + Bias Backward) ==="
cd "$CAFFE_FFI_DIR"
rm -rf build
...
echo ""
echo "=== Step 3: Run Scale Backward tests ==="
cd "$CAFFE_FFI_DIR/tests/python"
export CAFFE_FFI_PERF_GC_MODE=quick
export CAFFE_FFI_CPP_LOG_LEVEL=2
export PYTHONUNBUFFERED=1
python -m pytest test_scale_backward.py -v --tb=short 2>&1

echo ""
echo "=== Step 4: Run Bias Backward tests ==="
python -m pytest test_bias_backward.py -v --tb=short 2>&1

echo ""
echo "=== Step 5: Regression - Dropout + Pooling backward tests ==="
python -m pytest test_dropout_backward.py test_pooling_backward.py -v --tb=short 2>&1 | tail -40
