...
BASE_BUILD_SCRIPT="$PROJECT_ROOT/apps/docker-images/jupyter-ssh-base/scripts/build.sh"
...
if ! $SKIP_BASE_BUILD; then
    ...
    if docker image inspect jupyter-ssh-base:1.1 &>/dev/null 2>&1; then
        ...
        cd "$PROJECT_ROOT/apps/docker-images/jupyter-ssh-base"
        if $USE_CN_MIRRORS; then
...
if [ $VERIFY_FAIL -eq 0 ]; then
    ...
    echo "  - 详细部署指南参见:    apps/docker-images/caffe-ffi-jupyter/WSL-DEPLOY-GUIDE.md"
    echo ""
    echo -e "${_CLR_GRAY}JSON 事件日志: ${LOG_JSON_OUTPUT}${_CLR_RESET}"
    echo ""
    exit 1
