...
setup_ssh_keys() {
    ...
    local host_ssh_dir="/run/host-ssh"

    # 确保 .ssh 目录存在且可写（不由volume覆盖，宿主机SSH挂载到 /run/host-ssh）
    mkdir -p "$ssh_dir"
    chmod 700 "$ssh_dir"
    chown "${user}:${user}" "$ssh_dir"

    # 从宿主机SSH目录复制 known_hosts（避免git操作时host key验证提示）
    if [ -d "$host_ssh_dir" ] && [ -f "${host_ssh_dir}/known_hosts" ]; then
        cp "${host_ssh_dir}/known_hosts" "${ssh_dir}/known_hosts"
        chmod 644 "${ssh_dir}/known_hosts"
        chown "${user}:${user}" "${ssh_dir}/known_hosts"
        log_info "Copied known_hosts from host SSH directory"
    fi

    # 从宿主机SSH目录复制私钥到容器内（解决Windows drvfs权限问题：SSH拒绝使用755权限的私钥）
    local host_keys_dir="${ssh_dir}/host-keys"
    local host_key_count=0
    if [ -d "$host_ssh_dir" ]; then
        mkdir -p "$host_keys_dir"
        chmod 700 "$host_keys_dir"
        chown "${user}:${user}" "$host_keys_dir"
        for key_type in id_ed25519 id_ecdsa id_rsa; do
            if [ -f "${host_ssh_dir}/${key_type}" ] && ! [[ "$key_type" == *.pub ]]; then
                cp "${host_ssh_dir}/${key_type}" "${host_keys_dir}/${key_type}"
                chmod 600 "${host_keys_dir}/${key_type}"
                chown "${user}:${user}" "${host_keys_dir}/${key_type}"
                host_key_count=$((host_key_count + 1))
                log_info "Copied host SSH key: ${key_type} (permissions fixed)"
            fi
        done
    fi

    # 如果有宿主机私钥，创建SSH config引用它们用于git操作
    if [ "$host_key_count" -gt 0 ]; then
        local config_file="${ssh_dir}/config"
        cat > "$config_file" << 'SSHEOF'
# Auto-generated by entrypoint - references host SSH keys for git operations
Host *
    StrictHostKeyChecking accept-new
    UserKnownHostsFile ~/.ssh/known_hosts
SSHEOF
        for key_type in id_ed25519 id_ecdsa id_rsa; do
            if [ -f "${host_keys_dir}/${key_type}" ]; then
                echo "    IdentityFile ${host_keys_dir}/${key_type}" >> "$config_file"
            fi
        done
    ...
    # 注入 SSH_PUBLIC_KEY 到 authorized_keys
...
REGISTRIES_EOF
        ...
        # 导出 XDG_RUNTIME_DIR 到登录 shell 环境（容器内无 systemd-logind，需手动设置）
        # 所有 login shell（SSH/su -/bash -l）都会 source /etc/profile.d/*.sh
        local _profile_d="/etc/profile.d/podman-runtime.sh"
        cat > "${_profile_d}" << 'PODENV_EOF'
# Podman rootless runtime directory (auto-generated by entrypoint.sh)
# In containers without systemd-logind, XDG_RUNTIME_DIR must be set explicitly
# for rootless podman to locate its runtime socket and temporary storage.
if [ -n "$UID" ] && [ "$UID" -ge 1000 ] && [ -d "/run/user/$UID" ]; then
    export XDG_RUNTIME_DIR="/run/user/$UID"
fi
PODENV_EOF
        chmod 644 "${_profile_d}"
        log_info "Podman XDG_RUNTIME_DIR profile script: ${_profile_d}"

        if su - "${user}" -c "XDG_RUNTIME_DIR=/run/user/${user_uid} podman system migrate" >/dev/null 2>&1; then
