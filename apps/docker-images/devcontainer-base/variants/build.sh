...
VARIANTS=(
    ...
    "onnx-dev|conda-llvm + 纯ONNX生态(onnx/onnxruntime/onnx-simplifier/onnxoptimizer/onnxscript,无PyTorch)|conda-llvm|/opt/conda/envs/main/bin/python -c \"import onnx,onnxruntime,onnxsim,onnxoptimizer,onnxscript;print('onnx-dev ecosystem OK')\"|||/opt/conda/envs/main/bin/python -c \"import sys;assert sys._is_gil_enabled() is False;print('free-threading OK')\"|||/opt/conda/envs/main/bin/python -c \"import importlib.util as u;assert u.find_spec('torch') is None and u.find_spec('torchvision') is None;print('torch absent OK')\""
