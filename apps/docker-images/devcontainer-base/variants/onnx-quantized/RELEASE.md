# DevContainer Base - onnx-quantized 镜像发布清单 (Release Manifest)
...
> 验证依据：[test-onnx-quantized.sh](../scripts/test-onnx-quantized.sh) **24 项测试全部 PASS（0 FAIL）**，连续两轮复跑均通过（2026-08-14）。
...
5. **自建模型量化**：用 `onnx.helper` 构建（参考 README 使用示例），注意 Gemm 权重布局为 `[in_dim, out_dim]` 且 `make_tensor` 需 `raw=True`
