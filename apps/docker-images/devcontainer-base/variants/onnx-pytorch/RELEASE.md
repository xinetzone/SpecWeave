...
> 本清单基于已构建并验证通过的 `devcontainer-base:onnx-pytorch-latest` 镜像生成。
> 运行时元数据源：容器内 `/etc/devcontainer-variant-onnx-pytorch-build-info`（构建时自动写入）。
> 验证依据：[test-onnx-pytorch.sh](../scripts/test-onnx-pytorch.sh) **23 项测试全部 PASS（0 FAIL）**（2026-08-15）。
...
| 构建命令 | `bash variants/build.sh --variant onnx-pytorch --cn`（或直连 `docker build -f variants/onnx-pytorch/Dockerfile ...`） |
...
- [onnx-pytorch 变体 README](./README.md)
- [变体 AGENTS.md（AI 入口）](./AGENTS.md)
- [onnx-quantized 发布清单](../onnx-quantized/RELEASE.md)（量化下游变体）
- [onnx-dev 变体 README](../onnx-dev/README.md)（纯 ONNX 架构对偶）
