...
LOG_SERVICE="build-onnx-dev"
LOG_JSON_OUTPUT="/tmp/build-onnx-dev-events.jsonl"
...
One-click build and verify onnx-dev variant (WSL2/Linux environment).

Build dependency chain:
  devcontainer-base:latest → conda-llvm → onnx-dev
  (script will auto-build dependencies if missing)

Note: the intermediate 'conda' variant has been retired;
conda-llvm builds directly on devcontainer-base:latest.
...
Examples:
  $0                           # Build onnx-dev with China mirrors (WSL2/Linux)
...
print_final_report() {
    ...
    if image_exists "$image"; then
        ...
        log_info "Quick verification (ONNX versions + torch absence + GIL):"
        echo ""
        docker run --rm "$image" /opt/conda/envs/main/bin/python -c "import onnx, onnxruntime, sys; import importlib.util as u; print('  onnx       :', onnx.__version__); print('  onnxruntime:', onnxruntime.__version__); print('  torch      :', 'ABSENT (by design)' if u.find_spec('torch') is None else 'PRESENT (UNEXPECTED!)'); print('  GIL enabled:', sys._is_gil_enabled())" 2>&1 | grep -v "^\[" | sed 's/^/  /'
        echo ""
...
echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║        ONNX-Dev Variant One-Click Build Script (WSL2)        ║"
echo "║     Pure ONNX Ecosystem Runtime (no PyTorch, main env)       ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
log_info "Tag:              ${TAG}"
...
log_info "Dependency chain: base → conda-llvm → onnx-dev"
echo ""
...
if [ "$SKIP_BUILD" = false ]; then
    ...
    # 按依赖链顺序检查并构建（conda 中间变体已下线，conda-llvm 直接基于基础镜像）
