...
10. **大块替换优先级规则**：当单次替换量超过 50 行时，禁止使用多轮 `edit_file`（SearchReplace）逐段修改，必须改用"读取 → 截取头部 → 拼接新尾部 → 整体写入"的幂等策略。多轮 SearchReplace 的可靠性随轮次指数级下降——前序编辑会改变文件内容，导致后续 `old_string` 无法匹配。仅当单次替换 < 20 行时，才适合使用多轮 `edit_file`。
11. **路径确认三步走**：新建文件前必须执行：(1) 确认当前任务域的根目录（如 `d:\AI\`）；(2) 查阅 `.agents/` 中的路径规范或参考同目录下已有文件的存放位置；(3) 将最终产物写入符合项目约定的规范目录，临时文件先放入 `.temp/` 或系统临时目录。禁止将产物直接写入项目根目录。
12. **大块编辑回滚备份**：涉及 > 50 行的替换或重写操作前，必须在临时目录（`.temp/`）保存原始文件的完整副本作为回滚备份。编辑完成后验证通过方可删除备份。
