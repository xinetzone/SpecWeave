...
| [doc-automation-pipeline-reproduction-checklist.md](doc-automation-pipeline-reproduction-checklist.md) | 文档流水线模式跨领域复现检查清单（A-007） | 在第 2 个独立且跨领域（优先非软件域，如 ISO 受控文档体系）的案例中复现"文档自动化生成与验证流水线模式"5 步骤并留证（含基线审计、门禁分层、单一事实源幂等、脚本护栏、阈值实测专项实证），判定线全过且无内核级修改后模式升 L2 |
