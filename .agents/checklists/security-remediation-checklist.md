---
id: "security-remediation-checklist"
title: "敏感信息防泄露安全整改建议清单"
source: "reports/security-remediation-checklist.md"
x-toml-ref: "../../.meta/toml/.agents/checklists/security-remediation-checklist.toml"
maturity: "L1"
---
# 敏感信息防泄露安全整改建议清单
...
- [ ] 在 [ci-check.ps1](../scripts/ci-check.ps1) 中新增第14步：敏感信息扫描
  - 命令：`python ../scripts/check-sensitive-info.py --only-severity high`
  - 退出码1（HIGH风险）时阻断流水线，退出码2（MEDIUM）仅警告
  - bash版本 [ci-check.sh](../scripts/ci-check.sh) 同步添加
...
- [ ] 创建 `.git/hooks/pre-commit` 钩子脚本：
  ```bash
  #!/bin/sh
  python ../scripts/check-sensitive-info.py --only-severity high --path .
  if [ $? -ne 0 ]; then
    echo "❌ 检测到高风险敏感信息，请脱敏后再提交"
    echo "   可运行 python ../scripts/check-sensitive-info.py --fix 自动修复"
    exit 1
  fi
  ```
- [ ] 将钩子脚本模板放入 [../scripts/hooks/pre-commit](../scripts/hooks/pre-commit)，在项目初始化文档中说明启用方式
...
- [ ] GitHub Actions workflow [../../.github/workflows/sensitive-info-scan.yml](../../.github/workflows/sensitive-info-scan.yml) 增强：
...
- [ ] 将脱敏规则从 [../scripts/lib/checks/sensitive_info.py](../scripts/lib/checks/sensitive_info.py) 中的 `fix_func` 提取为可复用的 `lib/security/masking.py` 模块
- [ ] 提供通用脱敏函数供业务代码和日志使用：
...
参照 [../rules/data-security/data-classification/](../rules/data-security/data-classification.md) 规范：
...
参照 [../rules/data-security/incident-response/](../rules/data-security/incident-response.md) 规范：
...
python ../scripts/check-sensitive-info.py --json --output report.json

# 扫描指定目录
python ../scripts/check-sensitive-info.py --path ../../apps/my-app

# 排除额外目录
python ../scripts/check-sensitive-info.py --exclude temp,build,dist
```
