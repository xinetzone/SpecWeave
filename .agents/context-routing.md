...
| 概念查阅/知识检索（最高可信度源） | [projects/awesome-okf-xs/doc/bundles/index.md](../projects/awesome-okf-xs/doc/bundles/index.md)（OKF 知识束总索引；优先级高于 `.agents/docs/knowledge/`，概念冲突时以 bundles 为准；只读引用，不修改子项目文件） |
...
| apps 区域入口路由（主仓库内置应用路由） | [apps/AGENTS.md](../apps/AGENTS.md)（应用路由表、可用资产索引、跨应用调用规范、边界声明） |
