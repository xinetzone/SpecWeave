...
source = ".agents/skills/retrospective-cmd/SKILL.md#7, .agents/skills/insight-cmd/SKILL.md#7, .agents/skills/export-report-cmd/SKILL.md#7, .agents/skills/atomization-cmd/SKILL.md#7, .agents/skills/atomic-commit-cmd/SKILL.md#7, .agents/commands/mermaid.md#CMD-LOG"

[bindings]
rules = ["stage-guardrails.md"]
references = ["../skills/", "../../docs/retrospective/patterns/code-patterns/structured-lightweight-logging.md"]
skills = ["retrospective-cmd", "insight-cmd", "export-report-cmd", "atomization-cmd", "atomic-commit-cmd", "mermaid-cmd"]
---
...
| 步骤 | retrospective | insight | export-report | atomization | atomic-commit | mermaid |
|------|--------------|---------|---------------|-------------|---------------|---------|
| S0 | 启动 | 启动 | 启动 | 启动 | 启动 | 启动与范围确认 |
| S1 | 收集事实 | 数据采集 | 验证源报告 | 源文件分析 | 检查范围 | 图表设计与类型选择 |
| S2 | 分析过程 | 趋势分析 | 准备内容 | 制定方案 | 预提交验证 | Mermaid代码生成 |
| S3 | 提炼洞察 | 根因分析 | 格式转换 | 执行拆分 | 构建信息 | 语法检查 |
| S4 | 生成报告 | 异常检测 | 生成文件 | 更新引用 | 执行提交 | 自动修复 |
| S5 | 归档沉淀 | 建议生成 | 归档索引 | 收尾验证 | 结果验证 | 质量验证 |
