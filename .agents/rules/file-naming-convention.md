...
| 场景 | 说明 | 示例 |
|-----|------|------|
| 参赛作品交付物 | 外部赛事要求的原始交付文件，保留原名以确保可追溯性 | `报名帖_竹简悟道.md`、`竹简悟道_完整版.html` |
| 历史遗留文件 | 项目早期阶段创建的文件，修改成本过高且无实际影响 | 需经过评估后确定 |

**例外申请流程**：
1. 确认文件具有特殊语义价值或修改成本极高
2. 在项目复盘报告中记录例外原因
3. 更新本规范文档的例外场景列表

## 违规处理

对于已存在的违规文件名，应进行重命名并更新所有相关引用。对于例外场景中的文件，需在规范文档中明确记录。
