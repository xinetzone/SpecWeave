...
| 临时依赖管理流程 | [dependency-management.md](./dependency-management.md) | 存放位置、使用规范、清理机制、禁止提交条款 | 第三方依赖管理、临时文件处理、仓库整洁维护 |
...
3. **存放依赖**：将第三方库放入 `vendor/` 对应子目录，记录版本与来源。
4. **确认忽略规则**：确认 `.gitignore` 已配置 `vendor/` 忽略规则。
5. **清理机制**：任务完成后，按清理机制评估是否保留依赖。
