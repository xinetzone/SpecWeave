...
flowchart TD
    ...
    B --> C["按模板搭建目录结构<br/>（src/、tests/、README.md、依赖声明）"]
    C --> D["迭代开发与测试"]
    D --> E{"迁移模式选择？"}
    E -->|"全量迁移<br/>（完整应用）"| F["执行全部测试用例"]
    E -->|"选择性归档<br/>（交付物/快照）"| SA1["验证文件自包含性<br/>（无外部依赖）"]
    F --> G{"测试是否全部通过？"}
    G -->|"否"| D
    G -->|"是"| H["请求代码审查<br/>（至少一位 reviewer）"]
    H --> I{"审查是否通过？"}
    I -->|"否"| J["修复审查意见"]
    J --> D
    I -->|"是"| K{"是否存在阻塞性缺陷？"}
    ...
    N --> O["执行全量迁移<br/>.temp/app-name → apps/app-name"]
    SA1 --> SA2{"存在外部依赖？"}
    SA2 -->|"是"| SA3["迁移依赖或选择自包含版本"]
    SA2 -->|"否"| SA4["创建 apps 子目录<br/>Move 指定文件"]
    SA3 --> SA4
    SA4 --> SA5["编写应用 README.md"]
    SA5 --> SA6["更新 apps/ 索引（§2.3）"]
    SA6 --> SA7["删除已归档源文件<br/>保留 .temp/ 中间产物"]
    SA7 --> SZ["完成：交付物进入版本控制"]
    O --> P["更新 apps/ 索引"]
