...
# --- Configuration ---
$SkillName = 'local-computer-use'
$VenvDir = Join-Path $env:USERPROFILE ".openvino\venv\computer-use"
$ModelsDir = Join-Path $env:USERPROFILE ".openvino\models"
$TempDir = Join-Path $env:USERPROFILE ".openvino\temp"
$PendingFile = Join-Path $env:USERPROFILE ".openvino\pending-request.json"
$PipeName = '\\.\pipe\luicore'
...
$TestCases = @(
    ...
    @{
        Name        = 'Query current power mode'
        Instruction = '现在的电源模式是什么'
        ExpectExit  = 0
    }
)
...
function Clean-Models {
    ...
    Stop-Server
    $cuModels = Get-ChildItem $ModelsDir -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -match 'luicore|computer-use|classifier|toolfinder' }
    foreach ($m in $cuModels) {
...
foreach ($tc in $TestCases) {
    ...
    Write-Host "    Instruction: $($tc.Instruction)"

    $output = & $RunScript $tc.Instruction 2>&1 | Out-String
