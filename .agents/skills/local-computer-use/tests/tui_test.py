"""TUI test program for local-computer-use server (luicore).
...
class ComputerUseApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="request-panel"):
            yield Static("Computer Use Request", classes="panel-title")
            yield Input(placeholder="Command (e.g. 打开设置)", id="input-cmd")
    ...
    def _start_server(self) -> None:
        ...
        venv_python = OPENVINO_ROOT / "venv" / "computer-use" / "Scripts" / "pythonw.exe"
    ...
    def _refresh_status(self) -> None:
        ...
        if reply is None:
            ...
            panel.version = reply.get("luicore_version", "N/A")
            log.write_line(f"[INFO] Status: {json.dumps(reply)}")

    def _send_request(self) -> None:
        input_cmd = self.query_one("#input-cmd", Input)
        response_display = self.query_one("#response-display", Static)
        log = self.query_one("#log-view", Log)

        user_input = input_cmd.value.strip()
        if not user_input:
            log.write_line("[WARN] No command specified")
            return

        log.write_line(f"[INFO] Sending: input={user_input}")
        reply = _send_op({"op": "request", "input": user_input})
        if reply is None:
            ...
            log.write_line(
                f"[INFO] Response: ok={reply.get('ok')} success={reply.get('success')} "
                f"skill={reply.get('actual_skill')} tool={reply.get('actual_tool')}"
            )
