...
# --- Configuration ---
$SkillName = 'local-mineru'
$VenvDir = Join-Path $env:USERPROFILE ".openvino\venv\mineru"
$ModelsDir = Join-Path $env:USERPROFILE ".openvino\models"
$TempDir = Join-Path $env:USERPROFILE ".openvino\temp\local-mineru"
$PendingFile = Join-Path $env:USERPROFILE ".openvino\local-mineru-pending-request.json"
$PipeName = '\\.\pipe\local-mineru'
...
$TestCases = @(
    @{
        Name        = 'Parse JPG image'
        Input       = "$PSScriptRoot\2.jpg"
        ExpectExit  = 0
    },
    @{
        Name        = 'Parse PNG image'
        Input       = "$PSScriptRoot\9.png"
        ExpectExit  = 0
    },
    @{
        Name        = 'Parse PDF document'
        Input       = "$PSScriptRoot\1969_PDF.pdf"
        ExpectExit  = 0
    },
    @{
        Name        = 'Batch parse folder'
        Input       = "$PSScriptRoot"
        ExpectExit  = 0
    }
)
...
function Clean-Models {
    ...
    Stop-Server
    $mineruModels = Get-ChildItem $ModelsDir -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -match 'mineru' }
    foreach ($m in $mineruModels) {
...
foreach ($tc in $TestCases) {
    ...
    Write-Host "    Input: $($tc.Input)"

    $output = & $RunScript $tc.Input 2>&1 | Out-String
