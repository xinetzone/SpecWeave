...
html-ppt 的 **S 键演讲者视图是 `runtime.js` 内置的**。你只需要做两件事：
...
> 💡 **为什么不闪烁**：iframe 初次加载后就常驻，翻页时 presenter 窗口通过 `postMessage({type:'preview-goto', idx:N})` 告诉 iframe 切换到第 N 页。iframe 内的 runtime.js 只切换 `.active` class，**不重新加载、不渲染白屏**。
