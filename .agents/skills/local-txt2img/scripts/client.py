"""Short-lived CLI client for the local-txt2img server.

Talks to ``server.py`` over the Windows named pipe ``\\\\.\\pipe\\txt2img``.
...
first run of the skill needs to download ~2 GB of model weights, the
...
try:
    ...
    print("** Please run 'scripts\\install-env.ps1' to install the local-txt2img python environment firstly. **")
    sys.exit(1)

PIPE_ADDRESS = r"\\.\pipe\txt2img"
AUTHKEY = b"txt2img-local"
SKILL_NAME = "local-txt2img"
...
def _normalize_log_path(log_path: str | None) -> Path | None:
    if not log_path:
        ...
        log_path = f"~/.openvino/log/txt2img-client-py-{timestamp}.log"
...
def _save_pending_request(user_input: str, image_size: str | None, log_path: Path | None) -> None:
    try:
...
def _copy_bin_tree(src: Path, dst: Path, log_path: Path | None) -> None:
    ...
    _log_message(log_path, f"refreshed bin tree under {dst}")
...
def _request_server_start(dog_conn, log_path: Path | None) -> None:
    env = _read_info_json()
    venv_name = env.get("venv_name", "txt2img")
...
def _sync_runtime_scripts(log_path: Path | None) -> None:
    ...
    src_bin = Path(__file__).resolve().parent.parent / "bin"
    runtime_scripts = [
    ...
    bin_outdated = _dir_needs_sync(src_bin, TEMP_BIN)
    if not bin_outdated and not any(_needs_sync(src, dst) for src, dst in runtime_scripts):
        return
    ...
    if bin_outdated:
        _copy_bin_tree(src_bin, TEMP_BIN, log_path)
...
def _ensure_server(log_path: Path | None = None):
    ...
    raise RuntimeError(f"txt2img server did not come up within {SERVER_BOOT_TIMEOUT:.0f}s")
...
def _print_request_reply(reply: dict) -> int:
    ...
    if image_path:
        print(Style.BRIGHT + Fore.GREEN + "✅ 图片已生成: " + Style.RESET_ALL + str(image_path))
...
def _is_abnormal_image(reply: dict, log_path: Path | None) -> bool:
    """Return True when the server reported a successful generation whose image
    file is suspiciously small (< MIN_IMAGE_BYTES), indicating a corrupt output."""
    ...
    try:
        ...
        _log_message(log_path, f"failed to stat generated image {image_path}: {exc}")
        return False
    if size < MIN_IMAGE_BYTES:
        _log_message(
            log_path,
            f"abnormal image detected: {image_path} size={size} < {MIN_IMAGE_BYTES}",
        )
        return True
    return False
...
def _wait_pid_gone(pid: int | None, timeout: float, log_path: Path | None) -> bool:
    ...
    try:
        ...
        # Without psutil we cannot poll the pid; fall back to a short sleep.
...
def _restart_and_wait(log_path: Path | None) -> tuple[str, str]:
    """Shut the server down, bring it back up, and wait until it is running again."""
    old_pid = _query_server_pid(log_path)
    _cmd_shutdown(DEFAULT_SHUTDOWN_TIMEOUT, log_path)
    pipe_deadline = time.time() + 5.0
    while time.time() < pipe_deadline and _try_connect() is not None:
        time.sleep(0.2)
    # Wait for the old process to fully exit so the dog respawns a fresh server
    # instead of returning the still-dying PID.
    _wait_pid_gone(old_pid, DEFAULT_SHUTDOWN_TIMEOUT + 5.0, log_path)
    deadline = time.time() + SERVER_BOOT_TIMEOUT
    try:
        _ensure_server(log_path)
    except RuntimeError as exc:
...
def _cmd_request(user_input: str, log_path: Path | None = None, image_size: str | None = None) -> int:
    _log_message(log_path, f"processing request: {user_input!r} image_size={image_size!r}")
    ...
    if outcome == "timeout":
        _save_pending_request(user_input, image_size, log_path)
    ...
    try:
        ...
        for attempt in range(1, ABNORMAL_IMAGE_RETRY_MAX + 1):
            ...
            _log_message(
                log_path,
                f"generate prompt={user_input!r} (attempt {attempt}/{ABNORMAL_IMAGE_RETRY_MAX})",
            )
            generate_msg: dict = {"op": "generate", "prompt": user_input}
            if width is not None and height is not None:
                generate_msg["width"] = width
                generate_msg["height"] = height
            reply = _send(conn, generate_msg, log_path)
            ...
            if attempt >= ABNORMAL_IMAGE_RETRY_MAX:
                ...
                print(Fore.RED + "❌ 本地文生图服务异常" + Style.RESET_ALL)
                return 1

            restart_outcome, restart_detail = _restart_and_wait(log_path)
            if restart_outcome != "running":
                _log_message(
                    log_path,
                    f"server did not return to running after restart: "
                    f"{restart_outcome} {restart_detail}",
                )
                print(Fore.RED + "❌ 本地文生图服务异常" + Style.RESET_ALL)
                return 1
...
def _cmd_continue(log_path: Path | None = None) -> int:
    ...
    saved_image_size = pending.get("image_size")
    saved_log = _normalize_log_path(pending.get("log_path")) or log_path
    _log_message(saved_log, f"--continue resuming input={saved_input!r} image_size={saved_image_size!r}")
    return _cmd_request(saved_input, saved_log, image_size=saved_image_size)


def _parse_image_size(value: str) -> tuple[int, int]:
    """Parse a 'WIDTHxHEIGHT' string into (width, height) integers."""
    if not value or "x" not in value:
        raise argparse.ArgumentTypeError(f"invalid image size: {value!r} (expected WIDTHxHEIGHT, e.g. 1024x1024)")
    parts = value.split("x", 1)
    if len(parts) != 2:
        raise argparse.ArgumentTypeError(f"invalid image size: {value!r} (expected WIDTHxHEIGHT)")
    try:
        w, h = int(parts[0]), int(parts[1])
    except ValueError:
        raise argparse.ArgumentTypeError(f"invalid image size: {value!r} (width and height must be integers)")
    if w <= 0 or h <= 0:
        raise argparse.ArgumentTypeError(f"invalid image size: {value!r} (width and height must be positive)")
    return (w, h)


def main() -> int:
    parser = argparse.ArgumentParser(description="local-txt2img CLI client")
    parser.add_argument(
        "-i", "--input",
        type=str, default=None,
        help="user prompt for text-to-image generation",
    )
    parser.add_argument(
        "--image-size",
        type=str, default=None,
        help="output image size as WIDTHxHEIGHT (e.g. 1024x1024, 720x1280)",
    )
    parser.add_argument(
        "--log",
        type=str,
    ...
    if args.server_status:
        ...
        if args.input is None:
            print(Fore.RED + "usage: client.py -i \"<prompt>\" | --continue" + Style.RESET_ALL)
            return 3
        exit_code = _cmd_request(args.input, log_path, image_size=args.image_size)
