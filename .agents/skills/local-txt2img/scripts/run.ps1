...
$LogFile = Join-Path $LogDir "txt2img-client-$LogTimestamp.log"
Add-Content $LogFile "[$(Get-Date)] Log initialized."

function Write-Log($msg) { Add-Content $LogFile "[$(Get-Date)] $msg" }

Write-Log "run.ps1 started with arguments: $UserPrompt, ImageSize=$ImageSize"

if (-not $UserPrompt) {
    Write-Log 'No prompt argument provided.'
    Write-Host 'Usage: scripts\run.ps1 "<prompt>" [-ImageSize <WxH>]'
...
if ($UserPrompt -eq '--continue') {
    ...
    $ClientArgs = @('scripts\client.py', '-i', $UserPrompt)
    if ($ImageSize) {
        $ClientArgs += @('--image-size', $ImageSize)
    }
    Write-Log "Launching scripts\client.py with args: $($ClientArgs -join ' ')"
    & $VenvPy @ClientArgs
}
