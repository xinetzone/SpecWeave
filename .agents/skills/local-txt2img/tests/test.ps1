...
# --- Configuration ---
$SkillName = 'local-txt2img'
$VenvDir = Join-Path $env:USERPROFILE ".openvino\venv\txt2img"
$ModelsDir = Join-Path $env:USERPROFILE ".openvino\models"
$TempDir = Join-Path $env:USERPROFILE ".openvino\temp\txt2img"
$PendingFile = Join-Path $env:USERPROFILE ".openvino\txt2img-pending-request.json"
$PipeName = '\\.\pipe\txt2img'
...
function Clean-Models {
    ...
    Stop-Server
    $imgModels = Get-ChildItem $ModelsDir -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -match 'txt2img|stable-diffusion|sd-|flux' }
...
foreach ($tc in $TestCases) {
    ...
    $output = & $RunScript $tc.Prompt 2>&1 | Out-String
