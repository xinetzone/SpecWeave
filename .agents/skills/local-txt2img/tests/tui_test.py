"""TUI test program for local-txt2img server.
...
class Txt2ImgApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="request-panel"):
            yield Static("Image Generation Request", classes="panel-title")
            yield Input(placeholder="Prompt (e.g. a cat sitting on a chair)", id="prompt-input")
            yield Input(placeholder="Negative prompt (optional)", id="neg-input")
            yield Input(placeholder="Width (512)", id="width-input", value="512")
            yield Input(placeholder="Height (512)", id="height-input", value="512")
            yield Input(placeholder="Steps (9)", id="steps-input", value="9")
            yield Input(placeholder="Seed (random)", id="seed-input")
            yield Button("Generate Image", id="btn-send", variant="primary")
    ...
    def _start_server(self) -> None:
        ...
        venv_python = OPENVINO_ROOT / "venv" / "txt2img" / "Scripts" / "pythonw.exe"
    ...
    def _send_request(self) -> None:
        ...
        neg_input = self.query_one("#neg-input", Input)
        ...
        msg = {
            "op": "generate",
            "prompt": prompt,
            "negative_prompt": neg_input.value.strip() or None,
            "width": int(width_input.value or 512),
            "height": int(height_input.value or 512),
            "steps": int(steps_input.value or 9),
        }
        seed_val = seed_input.value.strip()
        if seed_val:
            msg["seed"] = int(seed_val)

        log.write_line(f"[INFO] Sending: prompt={prompt}")
        reply = _send_op(msg)
        if reply is None:
