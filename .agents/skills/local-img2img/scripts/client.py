...
try:
    ...
    print("** Please run 'scripts\\install-env.ps1' to install the local-img2img python environment firstly. **")
    sys.exit(1)

PIPE_ADDRESS = r"\\.\pipe\img2img"
AUTHKEY = b"img2img-local"
SKILL_NAME = "local-img2img"
...
PENDING_REQUEST_PATH = OPENVINO_ROOT / "img2img-pending-request.json"
...
def _normalize_log_path(log_path: str | None) -> Path | None:
    if not log_path:
        ...
        log_path = f"~/.openvino/log/img2img-client-py-{timestamp}.log"
...
def _save_pending_request(reference_image_path: str, prompt: str, log_path: Path | None) -> None:
    try:
...
def _cmd_request(reference_image_path: str, prompt: str, log_path: Path | None = None) -> int:
    ...
    if outcome == "timeout":
        _save_pending_request(str(reference_image), prompt, log_path)
    ...
    try:
        ...
        for attempt in range(1, ABNORMAL_IMAGE_RETRY_MAX + 1):
            ...
            _log_message(
                log_path,
                f"generate image={str(reference_image)!r} prompt={prompt!r} (attempt {attempt}/{ABNORMAL_IMAGE_RETRY_MAX})",
            )
            reply = _send(
                conn,
                {
...
def _cmd_continue(log_path: Path | None = None) -> int:
    ...
    if pending is None:
        ...
        print(Fore.RED + "无待处理请求, 请先使用 `scripts\\run.ps1 \"<image>\" \"<prompt>\"` 发起请求" + Style.RESET_ALL)
        return 1
    ...
    _log_message(saved_log, f"--continue resuming image={saved_image!r} prompt={saved_prompt!r}")
    return _cmd_request(saved_image, saved_prompt, saved_log)
...
def main() -> int:
    ...
    if args.server_status:
        ...
        if args.image_path is None or args.input is None:
            print(Fore.RED + "usage: client.py --image-path \"<image>\" -i \"<prompt>\" | --continue" + Style.RESET_ALL)
            return 3
        exit_code = _cmd_request(args.image_path, args.input, log_path)
