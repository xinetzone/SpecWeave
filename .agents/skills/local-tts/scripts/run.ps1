...
$LogFile = Join-Path $LogDir "tts-client-$LogTimestamp.log"
Add-Content $LogFile "[$(Get-Date)] Log initialized."

function Write-Log($msg) { Add-Content $LogFile "[$(Get-Date)] $msg" }

Write-Log "run.ps1 started with arguments: $UserPrompt $($RestArgs -join ' ')"

if (-not $UserPrompt) {
    Write-Log 'No prompt argument provided.'
    Write-Host 'Usage: scripts\run.ps1 "<prompt>" [--voice <name>] [--language <lang>] [--ref-audio <path> --ref-text <text>] [--output <wav>]'
    Write-Host '       scripts\run.ps1 --continue'
    exit 1
}

if ($UserPrompt -eq '--continue') {
...
if ($UserPrompt -eq '--continue') {
    ...
    Write-Log "Launching scripts\client.py -i `"$UserPrompt`" $($RestArgs -join ' ')"
    & $VenvPy scripts\client.py -i $UserPrompt @RestArgs
}
