...
$LogFile = Join-Path $LogDir "asr-client-$LogTimestamp.log"
Add-Content $LogFile "[$(Get-Date)] Log initialized."

function Write-Log($msg) { Add-Content $LogFile "[$(Get-Date)] $msg" }

Write-Log "run.ps1 started with arguments: $AudioPath $Language"

if (-not $AudioPath) {
    Write-Log 'No audio path argument provided.'
    Write-Host 'Usage: scripts\run.ps1 "<audio_or_video_path|glob|list>" [language]'
    Write-Host '       Example: scripts\run.ps1 "C:\audio\*.wav" [language]'
    Write-Host '       Example: scripts\run.ps1 "C:\audio\1.wav;C:\audio\2.wav" [language]'
    Write-Host '       scripts\run.ps1 --continue'
    exit 1
}

if ($AudioPath -eq '--continue') {
    Write-Host 'Resuming pending ASR request ...'
} elseif ($AudioPath -eq '--server-status') {
    Write-Host 'Querying ASR server status ...'
} elseif ($AudioPath -eq '--server-shutdown') {
    Write-Host 'Requesting ASR server shutdown ...'
} else {
    Write-Host "Received audio input: $AudioPath"
}
...
if ($AudioPath -eq '--continue') {
    ...
    Write-Log "Launching scripts\client.py --audio `"$AudioPath`" --language $Language."
    & $VenvPy scripts\client.py --audio $AudioPath --language $Language --log $LogFile
}
