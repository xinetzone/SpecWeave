"""TUI test program for local-asr server.
...
class ASRApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="request-panel"):
            yield Static("Transcription Request", classes="panel-title")
            yield Input(placeholder="Audio file path (e.g. C:\\audio\\test.wav)", id="audio-input")
            yield Input(placeholder="Language (auto)", id="lang-input", value="auto")
            yield Button("Send Request", id="btn-send", variant="primary")
    ...
    def _start_server(self) -> None:
        ...
        venv_python = OPENVINO_ROOT / "venv" / "asr" / "Scripts" / "pythonw.exe"
    ...
    def _refresh_status(self) -> None:
        ...
        if reply is None:
            ...
            panel.device = reply.get("device", "N/A")
            log.write_line(f"[INFO] Status: {json.dumps(reply)}")

    def _send_request(self) -> None:
        audio_input = self.query_one("#audio-input", Input)
        lang_input = self.query_one("#lang-input", Input)
        response_display = self.query_one("#response-display", Static)
        log = self.query_one("#log-view", Log)

        audio = audio_input.value.strip()
        language = lang_input.value.strip() or "auto"

        if not audio:
            log.write_line("[WARN] No audio file specified")
            return

        log.write_line(f"[INFO] Sending request: audio={audio}, language={language}")
        reply = _send_op({"op": "request", "audio": audio, "language": language})
        if reply is None:
            ...
            log.write_line(f"[INFO] Response received: ok={reply.get('ok')}")
