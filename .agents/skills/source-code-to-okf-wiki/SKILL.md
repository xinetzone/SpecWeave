...
当目标束已存在（常见形态：首轮基于 README 建束，后续多轮分别指定同一项目的不同信源资产——源码主体、docs/ 文档、examples/ 示例、测试用例），每一轮都是一次完整的 R→I→E→V→C，但产出方式是**增量扩展**而非新建：

- **每轮新增一份信源登记**：`references/<资产>-source.md`（如 readme-source → source-code-map → docs-source → examples-source），各自固定版本（tag + commit hash，同一仓库的多轮快照一致时复用同一 hash）。信源先行原则在每轮独立生效。
- **文档编号续接，不重排**：concepts/examples 新文件接现有最大序号（如 00-03 → 04-07 → 08-09）；不回头重命名既有文件、不改写已验证文档的结论（发现错误除外，错误修复走 log 记录）。
- **index 与 log 每轮末尾更新**：各级 index 的表格行与 toctree 条目追加新文档；log.md 顶部追加新版本条目（列清新增/更新/事实来源三段）。
- **新文档可跨轮引用既有信源**：frontmatter `sources` 可同时列多份信源（如示例文档引用 examples-source + source-code-map 做实现印证）；新文档正文可用相对链接引用既有 concepts，不重复阐述已覆盖的主题。
- **R 阶段只对新信源做全量**：既已验证的束内事实视为可信基线，不重复采集；I 阶段洞察允许在既有洞察上延伸（但每篇新文档必须有独立的新洞察，不为凑数拆分）。
- **E 阶段每轮独立分批**：单轮新增文件仍受每批 ≤7 约束。

> **为什么多轮扩展要"续编号 + 新信源登记"而不是重写整束？** 已验证入库的文档构成稳定基线；重写会让既有的交叉链接、读者路径与提交历史失效，且把已通过 V 阶段的内容重新暴露给事实遵循度衰减风险（§10 Gotchas）。podman-compose 束三轮实践（README 建束 → 源码 4 篇 → docs/completion 2 篇 → examples 1 篇，束内容从 4 概念/2 示例增至 10 概念/3 示例、references 从 1 份增至 4 份）验证了该模式。
...
- [ ] **G4-V阶段**：链接无断裂、无虚构API（Grep源码验证每个类名/方法名存在性）、**计数断言全部比对一致**（"X个/Y份/Z处"类陈述经 Glob/Grep 独立复核，且计数脚本口径与对象一致：行数用 `len(splitlines())`、正则容跨行与防前缀误匹配、目录计数沿用官方 gate 口径）、frontmatter字段完整、index无遗漏
- [ ] **多轮扩展（既有束）**：新信源登记落于 references/、新文档编号续接既有最大序号、各级 index（表格+toctree）与 log 均已追加、未重写已验证文档
- [ ] **子模块提交（projects/ 内束）**：`git add` 与 `git commit` 分两次调用；暂存前确认无 `.git/MERGE_HEAD`；显式列出文件（不用 `git add <目录>`）；commit 前用 `git show :<file>` 核暂存 blob 含关键改动、`git diff --cached --name-only` 核暂存集无他方文件混入
...
| V阶段验证脚本本身报红/报绿与直觉不符 | 计数口径错误：行数算成 `count("\n")+1`、正则漏多行装饰器/函数调用、前缀误匹配（如 GlobalOpt 被子命令函数计数）、以"含某文件"判目录类型 | 先修脚本口径再重跑；计数逻辑优先复用官方 gate 脚本（check-bundles-index.py 等）的规则，不自研启发式 |
| 既有束更新时新建了重复主题文档或重排旧编号 | 多轮扩展误走"重建"路线，破坏交叉链接与提交历史 | 按§6.5续编号新增；既有文档只追加事实不重写结论 |
| 子模块全量 gates 长红但变更文件明显无关 | 并行会话 WIP 欠债或远端 merge 带入的问题 | 先过滤本束相关项确认零命中，再报告归属；不擅自修共享索引 |
...
| 11 | 凭目测写数量（"6份Changelog""13个命令"实际是 7 份/14 个） | 计数断言门禁假红或假绿；文档数字与信源不符，可信度受损 | 所有"X个/Y份"先跑独立计数再落字；数字写定后 V 阶段逐一对表（podman-compose 实践两轮实测出错率） |
| 12 | 既有束多轮更新时重写整束/重排编号/重建信源 | 已验证内容暴露二次失真风险；既有交叉链接与读者路径断裂；提交历史无法审阅增量 | 按§6.5：新信源登记 + 续编号新增 + index/log 追加，不动已验证文档结论 |
| 13 | 子模块内一条链式命令完成 add+commit（`git add <dir> && git commit`） | 并行会话暂存的文件被混入提交（共享索引竞态）；PowerShell heredoc 直接报语法错误 | add 与 commit 分两次工具调用；显式列文件；commit 前 `git show :<file>` 核暂存 blob；提交消息写临时文件用 `git commit -F` |
...
- **目测数数会错且错得很自然**：清单类数量（文件份数、命令个数、枚举成员数）凭目测最易出错——同轮实践中"6 份 Changelog"实际 7 份（含补丁版本）、"13 个命令"实际 14 个（漏数 help）。这类数字写进文档后 V 阶段如果也用目测核对会一路假绿；必须用脚本计数，且计数脚本自身也要被结果合理性检验（计数与目录 LS 输出对照）。
- **dummy 定向构建比全量 gates 更适合验证自己的变更**：`python -m sphinx -b dummy -E doc <outdir> <显式文件列表>` 只构建本次变更文件，几分钟内给出 0-error 结论；全量 `invoke gates.toctrees/bundles` 会被并行会话的在途欠债（未注册新束、缺 index 的 WIP）拖红——那些红项不归属你，应过滤确认本束零命中后在报告中说明，不代为修复共享索引（避免并发写竞态）。
- **共享子模块提交的 blob 核验时序**：`git add` 之后、`git commit` 之前，必须有一次独立工具调用做 `git diff --cached --name-only` + `git show :<path>` 关键行抽查。并行会话可能在同一仓库 add；链式命令中你看到的工作树内容不等于最终暂存 blob。commit 后再 `git log -1` 确认提交内容。Windows/PowerShell 环境 heredoc 不可用，提交消息一律写临时文件后 `git commit -F <file>`。
...
- **v1.4.0** (2026-09-10): 新增§6.5「既有束的多轮增量扩展规范」（每轮一份新信源登记、文档编号续接不重排、index/log 末尾追加、新文档跨轮引用既有信源、R 阶段只对新信源全量），源于 podman-compose 束三轮连续扩展实战（README 建束 → 源码主体 → docs/completion → examples，4→10 概念、1→4 信源）；V 阶段计数断言强化——计数脚本口径规范（行数用 `len(splitlines())`、正则容跨行防前缀误匹配、目录计数复用官方 gate 口径）与"目测数数必错"教训（实测 6→7 份 Changelog、13→14 命令两处计数错误）；新增反模式 11（凭目测写数量）、12（多轮更新误重写整束）、13（add+commit 链式命令致暂存竞态）；Gotchas 增 dummy 定向构建替代全量 gates 验证、共享子模块 blob 核验时序、PowerShell 用 `git commit -F`；G4/检查清单与预警信号同步增项；frontmatter version 1.3.0→1.4.0，SKILL.toml 镜像同步。
