...
class BaseSchemaValidator:
    ...
    def repair_whitespace_preservation(self) -> int:
        ...
        for xml_file in self.xml_files:
            try:
                # Use bytes to avoid platform default encoding (e.g. GBK on Windows)
                # and let the XML parser honor BOM / XML declaration.
    ...
    def _validate_single_file_xsd(self, xml_file, base_path):
        ...
        try:
            ...
            # IMPORTANT: open as bytes to avoid Windows locale default encodings (GBK)
            # causing UnicodeDecodeError for UTF-8 OOXML parts.
