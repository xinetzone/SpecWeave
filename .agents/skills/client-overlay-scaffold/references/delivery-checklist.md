...
- [ ] 模板占位符清零：grep `__STACK__|__NS__|__SVC__|__ENV_PREFIX__|__IMAGE__|__SSH_PORT__|__JUPYTER_PORT__|__CONTAINERFILE__` 仅剩注释说明（如技能模板自身），成品无残留
- [ ] `podman-compose -f <abs>/compose.yaml config` 退出 0，断言：
      1 服务、image+build、端口、bind 数（长语法+create_host_path）、
      named volume（如有）、三必需、无 privileged/healthcheck/command
- [ ] `invoke --list`（PYTHONPATH=src 或已 pip install -e .）含全部 `<ns>.*` 任务
- [ ] Windows 原生 `invoke <ns>.ps` 实测 Exit 1 + 双路径中文指引
- [ ] 键集合三方 diff 为空：compose `${VAR}` ≡ overlay .env.example ≡ root .env.example 段
...
- [ ] tasks/__init__.py（import/Collection/configure/docstring）
- [ ] root client .env.example 新段
- [ ] root client README.md 新小节
- [ ] client AGENTS.md（6 处）
- [ ] .agents/README.md（4 处）
- [ ] apps/AGENTS.md（路由表+边界表）
- [ ] .agents/rules/<ns>-overlay.md + P0 C# 约束

## 5. 收口

- [ ] 改动全部落在白名单；既有 overlay（onnx/xmnn）与 quant.py 等零回归
