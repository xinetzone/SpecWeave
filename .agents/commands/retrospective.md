...
- 列出改进行动项与责任人
- 标注优先级与时间计划
- **数据验证三查法（强制，生成README/统计摘要前必须执行）**：
  1. **查关键数据**：报告中所有行数、字数、文件数、提交数等数字必须用 `wc`/`(Get-Content file).Count`/`grep` 实际统计，禁止凭记忆或估算；统计时记录验证命令，可在报告中标注。
  2. **查file:///链接**：使用 `.agents/scripts/check-links.py --path <report-dir>` 验证所有 `file:///` 链接指向真实存在的文件，发现断链立即修复。
  3. **查章节结构**：用 Grep 搜索 `^#` 标题层级，确认预期章节完整存在（如"## 关键决策"/"## 问题根因"/"## 改进建议"），防止章节遗漏或编号错误。
...
- [模式成熟度标准](../../docs/retrospective/patterns/index.md)