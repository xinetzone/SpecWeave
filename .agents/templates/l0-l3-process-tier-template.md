...
### 8.3 探针实现豁免的运行时支持（已实现）

阶段守卫运行时（`check-stage-guardrail-runtime.py`）已实现 `baby-` 前缀探针代码的识别与豁免。`guard_operation()` 方法支持 `baby_code: bool` 和 `file_path: str` 参数，探针代码的跨阶段操作不触发拦截。具体识别规则：
