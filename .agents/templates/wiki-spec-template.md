...
- [ ] 将单文件wiki拆分为目录原子结构（如适用，文件>300行建议拆分）
...
| 原子化结构 | wiki以目录+索引页形式存在（参考karpathy/mopmonk示例），单文件<200行 | 文件结构检查 |
| 链接有效 | 所有内部相对路径可到达，无断链 | check-links.py |
| 原子提交 | 内容创作和原子化拆分为两次独立提交，单一职责 | git log验证 |
