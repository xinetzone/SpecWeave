...
> **L0/L1/L2三层设计**：本清单采用分层校验体系，L0门禁项必须全部通过才能进入下一阶段，L1质量项显著影响产出质量，L2优化项锦上添花。
>
...
| 层级 | 类型 | 验证策略 | 示例 |
|------|------|---------|------|
| **L0** | 门禁项 | 必须全部通过才能进入下一阶段 | 输出文件存在、格式正确、无工具标签污染 |
| **L1** | 质量项 | 显著影响产出质量，必须逐项验证 | 核心模块覆盖、关键API分析、输出完整性 |
| **L2** | 优化项 | 锦上添花，按需验证 | 额外图表、扩展阅读建议、格式美化 |
...
| # | 检查项 | 检查方法 | 发现问题后的处理 |
|---|--------|---------|----------------|
| L0-3.1 | `<seed:tool_call>`标签 | Grep搜索 | 立即删除整个标签块及被污染的段落 |
| L0-3.2 | `<function `标签 | Grep搜索 | 检查是否为误插入的工具调用，若是则删除 |
| L0-3.3 | `<parameter `标签 | Grep搜索 | 同上 |
| L0-3.4 | `TodoWrite`等工具名 | Grep搜索 | 正文/Markdown文档中出现则删除（代码注释中合法） |
| L0-3.5 | `toolcall_result/toolcall_status` | Grep搜索 | 立即删除 |
| L0-3.6 | `<system-reminder>`标签 | Grep搜索 | 立即删除 |
| L0-3.7 | **输出完整性检查** | 人工检查 | 若返回摘要/截断/内容不全：第一次→重新委派并明确强调完整性要求；第二次→切换兜底策略 |
...
- [ ] 增加可视化图表（Mermaid图、架构图等）提升理解效率
- [ ] 添加扩展阅读建议和参考资料链接
- [ ] 对关键概念进行交叉引用，提升文档连通性
- [ ] 优化格式排版（加粗关键词、使用表格对比、代码块语法高亮）
