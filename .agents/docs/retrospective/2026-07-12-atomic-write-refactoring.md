...
将原子写入逻辑抽取为共享工具模块 [atomic_write.py](../../scripts/lib/atomic_write.py)，提供三层接口：
...
| 模块 | 文件 | 场景 |
|------|------|------|
| SpecLoader | [lib/spec_loader.py](../../scripts/lib/spec_loader.py) | 磁盘缓存保存（核心场景） |
| 链接检查 | [check-links.py](../../scripts/check-links.py) | 外部链接缓存 |
| 学术源检查 | [check-academic-sources.py](../../scripts/check-academic-sources.py) | DOI/arXiv验证缓存 |
| 指标导出 | [spec-loader-export-metrics.py](../../scripts/spec-loader-export-metrics.py) | 性能基线JSON导出 |
| Frontmatter迁移 | [lib/migrate_frontmatter/cli.py](../../scripts/lib/migrate_frontmatter/cli.py) | 迁移报告输出 |
...
### 6.1 单元测试（[test_atomic_write.py](../../scripts/tests/test_atomic_write.py)）
...
### 6.2 集成测试（[test_spec_loader.py](../../scripts/tests/test_spec_loader.py)）
...
### 6.3 压测脚本（[spec-loader-stress-test.py](../../scripts/spec-loader-stress-test.py)）
