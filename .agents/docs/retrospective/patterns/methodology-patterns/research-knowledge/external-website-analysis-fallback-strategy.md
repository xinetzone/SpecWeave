...
| 工具增强 | agent-browser（默认 / 反自动化 flag / 桌面 UA） | 第二层 | 沙箱核心策略，详见 [反爬策略预设清单](../../../../../../docs/knowledge/anti-crawler-strategy-playbook.md) |
...
沙箱环境中针对反爬站点（知乎/微博/推特等）的突破策略，参见 [反爬策略预设清单](../../../../../../docs/knowledge/anti-crawler-strategy-playbook.md)，该清单提供：
...
| 案例 | 失败表现 | 根因 | 教训 |
|------|---------|------|------|
| 知乎 637007780 任务 7 策略连败（2026-07-06，真实记录） | 前 6 种策略（WebFetch/defuddle/默认 agent-browser/移动端 UA 等）全部失败，反复在"工具换参数"层面打转，消耗大量时间 | 未识别 40362=自动化识别反爬（与权限型 403 本质不同），缺少反自动化 flag 配置知识；降级决策未按"最多 2 次"时间盒执行 | 先诊断反爬类型再选工具；识别信号（40362/JS challenge）应前置到五秒诊断清单；第 7 策略（反自动化 flag+桌面 UA）才突破 |
| archive.org 不可达仍被作为 fallback 尝试（知乎复盘 A5 行动项） | 沙箱环境网络策略限制下，第四层快照/缓存策略被尝试后无效试错，浪费决策时间 | fallback 链未区分运行环境的能力边界 | fallback 链必须按环境（沙箱/非沙箱）裁剪，不可达策略显式降级为末选（v1.1 修复） |

### 反目标用户/反目标场景

| 反目标用户/场景 | 不适用原因 | 适配策略 |
|----------------|-----------|---------|
| 内部文档/内网系统分析 | 不存在外部访问障碍，四层降级模型无意义 | 不适用：直接访问即可 |
| 主源可访问但内容质量差 | 触发的是信息质量筛选问题，不是可访问性问题 | 中度：走三角验证法与可信度评级，不启动层级降级 |
| 有合法授权凭证的登录内容 | 本环境不使用代理且不持有凭证，突破策略不适用 | 不适用：改用公开文档站等官方替代源 |
| 需要结构化 API 数据的研究（非网页内容） | 四层模型面向网页提取，API 数据走接口调用规范 | 中度：API 文档+移动端 API 路径，不走网页降级链 |
| 反爬对抗强度高于工具能力的站点（强验证码/短信验证） | 工具层突破成功率趋零，强攻违反合规与时间盒 | 重度：禁止反复突破，直接第三/四层替代源+透明标注局限 |

### 早期预警信号

| 预警信号 | 含义 | 应对 |
|---------|------|------|
| 同一 URL 重试超过 2 次 | 违反"不阻塞原则"，正在死磕 | 立即启动五秒诊断与降级 |
| 信息收集总时长逼近 30 分钟时间盒 | 陷入"找更多信息"循环 | 基于已获取信息做有限分析并标注缺口 |
| 核心论据仅剩单源低可信信息 | 降级链产出质量不足 | 补充第三层官方源交叉验证，否则降级结论强度 |
| 报告未披露主源不可访问事实 | 可追溯原则被违反 | 补写信息来源与局限性说明 |
| fallback 尝试的策略在当前环境被证实不可达 | fallback 链未按环境裁剪 | 更新沙箱可用/不可用策略表（v1.1 教训） |

### 适用前提

- 目标为公开互联网网页内容（外部产品/竞品/行业/事件分析）；内网与已授权内部系统不适用
- 运行环境的能力边界已知（沙箱可用/不可用策略表已核对）
- 存在可替代的官方/权威信息源生态（若目标信息全网唯一且无公开替代，降级模型无法挽救，应在 Spec 阶段预判并标注风险）
- 环境不使用代理、不持有目标站点登录凭证——所有突破均限于公开内容的工具层反爬应对
...
2. **反爬策略预设清单库** ✅ 已完成（v1.1）：已创建 [反爬策略预设清单](../../../../../../docs/knowledge/anti-crawler-strategy-playbook.md)，覆盖知乎/微博/推特三类反爬站点的专属突破配置和 agent-browser 参数模板，后续可继续补充小红书/微信公众号等站点
...
| 1.1 | 2026-07-06 | 沙箱环境 fallback 链优化（A5 行动项）：在第四层策略中显式标注 archive.org / Google Cache 为"沙箱环境不可达，降级为末选"；新增"沙箱环境策略选择"章节，区分沙箱可用与不可用策略；搜索引擎技巧中补充沙箱环境跳过缓存/快照的提示；模式演进方向 #2 标记为已完成并交叉引用 [反爬策略预设清单](../../../../../../docs/knowledge/anti-crawler-strategy-playbook.md) |
