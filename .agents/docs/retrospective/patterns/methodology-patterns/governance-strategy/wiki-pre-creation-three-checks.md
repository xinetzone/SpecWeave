...
x-toml-ref: "../../../../../../.meta/toml/.agents/docs/retrospective/patterns/methodology-patterns/governance-strategy/wiki-pre-creation-three-checks.toml"
rules: []
references:
...
3. **frontmatter必填字段**：MDI v1.0规范要求6个必填字段——id、title、source、x-toml-ref、date、tags，创建文件第一步就必须填完所有6个字段；date格式统一为"YYYY-MM-DD"；tags首标签保持系列统一（如"向日葵"）。创建文件时从模板 [mdi-document-template.md](../../../../../templates/mdi-document-template.md) 复制，确保字段零遗漏
...
| 案例 | 失败表现 | 根因 | 教训 |
|------|---------|------|------|
| MopMonk Wiki 任务（2026-07-04，真实事故） | 跳过第一查，凭 project_memory 旧描述使用 TOML（+++）frontmatter，项目实际统一 YAML（---），返工约 8 分钟并补建外部 TOML 元数据 | 依赖记忆而非同目录实例作为格式权威；委派子代理时未显式要求"先查同类" | 格式证据优先于记忆；三查流程由此诞生，之后 4 次正面验证零格式错误 |
| Papi酱 Wiki 脚本参数错误（2026-07-06，部分失败） | 文件名检查脚本凭记忆调用导致参数错误 | 三查覆盖了格式/目录/索引，但未覆盖"工具调用前验证" | 补充"先 --help 再执行"检查项——三查清单本身也需要边界强化迭代 |

### 反目标用户/反目标场景

| 反目标用户/场景 | 不适用原因 | 适配策略 |
|----------------|-----------|---------|
| 编辑已有文档 | 路径/文件名/frontmatter 已固定，三查无检查对象 | 轻度：仅需保持既有格式一致，不做三查 |
| 一次性临时文件（.temp/ 下） | 不进版本控制、无索引需求，三查成本纯浪费 | 轻度：走 file-creation-precheck 第零步判断即可 |
| 由模板直接生成的文件（模板已内置格式） | 格式权威已由模板保证，重复查同类收益趋零 | 中度：降级为"生成后抽查 frontmatter 字段完整性" |
| 非知识库类小文档（会话内问答、待办清单等） | 不需要索引登记与格式体系 | 轻度：仅确认文件名合规 |
| 高频批量生成场景（如批量创建 20 个同构文件） | 逐文件三查时间成本不成立 | 中度：首文件完整三查定格式，余下按既定格式批量生成+抽查 |

### 早期预警信号

| 预警信号 | 含义 | 应对 |
|---------|------|------|
| 连续两个任务的格式判断都来自记忆 | 第一查正在被跳过，成功偏误回潮 | 强制重新读取同类文档，复盘跳过原因 |
| 索引更新与文档创建分离到不同会话 | 第三查"提前确认索引"失效，易产生孤儿文档 | 在创建任务的同一会话内完成索引更新 |
| 同目录下找不到同类文档且未向上查找 | 第一查被静默放弃 | 按流程向上级目录/同类主题目录查找参考 |
| 子代理产出格式错误 | 委派描述中未写明"先查同类文档格式" | 检查任务描述模板是否被省略 |
| frontmatter 字段写完内容后才补 | 违反"第一步填完 6 必填字段"要求 | 回到模板复制流程，恢复字段先行 |

### 适用前提

- 目标目录存在可参照的同类文档（或可向上查找到最近父目录同类）
- 项目维护权威索引文件（如 docs/knowledge/README.md）且索引更新是被认可的动作
- 存在 frontmatter 模板（mdi-document-template.md）与文件名检查脚本可供第二查/收尾使用
- 产出物为需要纳入版本控制与知识体系的正式文档；.temp 临时文件不适用
...
- [wiki-spec-template.md](../../../../../templates/wiki-spec-template.md)（已整合三查流程作为强制检查点）
