...
任务启动
  ...
  └─ 否 ──┬─ Q2: 成本trivial<10分钟? ──是──→ 仅C原子提交
          ...
          └─ 否 ──┬─ Q3: 高影响重大决策?
                  │   │
                  │   ├─ 是 ──┬─ 遇未知问题需根因? ──是──→ F→V→C→R→I→E 问题解决全链路
                  │   │       │
                  │   │       └─ 否 ──┬─ 里程碑→ R→I→E→C 复盘闭环
                  │   │               ├─ 架构决策→ F→V→I→C 决策论证
                  │   │               ├─ 代码审查→ V→C 对抗审查
...
| P1+故障/线上问题 | F→V→C→R→I→E | 版本发布 | C→V→(R) |
...
| **2.问题解决** | F→V→C→R→I→E | 6 | P1+故障/根因不明 | 修复代码(已验证)+根因报告+预防pattern入库 | G1根因可复现修复100%<br/>G2修复通过后才复盘，含预防措施<br/>G3同类pattern入库 |
