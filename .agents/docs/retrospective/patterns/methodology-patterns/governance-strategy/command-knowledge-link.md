...
x-toml-ref: "../../../../../../.meta/toml/.agents/docs/retrospective/patterns/methodology-patterns/governance-strategy/command-knowledge-link.toml"
related_patterns:
...
flowchart TB
    ...
    subgraph RULE_LAYER ["规则层（Rules: 13条）"]
        R1["R1-R5 判定规则<br/>（何时关联/不关联）"]
        R6["R8 内容选择规则<br/>（链接什么/如何标注）"]
        R9["R11 结构规则<br/>（路径/组织/双向）"]
        R12["R13 验证规则<br/>（有效性/验收）"]
    end
    subgraph OPERATION_LAYER ["操作层（Operational）"]
        OP1["系统性三问法"]
        OP2["5类型判定矩阵"]
        OP3["8项验收清单"]
    end
    AXIOM_LAYER --> RULE_LAYER --> OPERATION_LAYER
```
