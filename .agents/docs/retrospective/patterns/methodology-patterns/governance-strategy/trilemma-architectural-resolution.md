...
trigger_conditions:
  ...
  -   - "external: 不存在-局部优化无法突破现有性能/成本/质量瓶颈"
  -   - "external: 不存在-竞争对手在某个维度实现数量级提升（如成本降至 1/N、速度提升 N 倍）"
  -   - "需要判断困境是本质矛盾还是架构遗留"
  -   - "需要制定架构级重新设计的决策"
...
下图展示步骤 2 的核心判定逻辑，帮助决策者快速区分本质矛盾与架构遗留：

```mermaid
flowchart TD
    START["面临三角困境"]
    START --> Q1{"三个顶点是否有<br/>物理/数学定律支撑?"}
    Q1 -->|"是 (CAP/热力学/光速)"| ESSENCE["本质矛盾"]
    Q1 -->|"否"| Q2{"当前架构是否为<br/>目标场景原生设计?"}
    Q2 -->|"是"| REEVAL["重新评估:<br/>可能不是架构遗留"]
    Q2 -->|"否"| Q3{"设计假设是否与<br/>目标场景不匹配?"}
    Q3 -->|"是"| LEGACY["架构遗留"]
    Q3 -->|"否"| Q4{"是否存在其他类型<br/>的架构约束?"}
    Q4 -->|"是"| OTHER["其他类型困境<br/>(需进一步分析)"]
    Q4 -->|"否"| REEVAL
    ESSENCE --> STRATEGY1["取舍策略:<br/>选择两个顶点,放弃一个"]
    LEGACY --> STRATEGY2["架构重定义:<br/>从目标场景倒推"]
    REEVAL --> END1["回到步骤 1<br/>重新识别困境"]
    OTHER --> END2["需补充分析<br/>后重新判定"]
    style START fill:#ffcccc
    style ESSENCE fill:#ffe4b5
    style LEGACY fill:#87ceeb
    style REEVAL fill:#ffffcc
    style OTHER fill:#e6e6fa
    style STRATEGY1 fill:#ccffcc
    style STRATEGY2 fill:#ccffcc
```

**判定要点**：
- **物理/数学定律支撑**：是否有定理证明（如 Brewer 定理、Lynch 证明）、物理常数限制（如光速、热力学第二定律）
- **原生设计**：架构是否"第一天就奔着目标场景设计"，而非"通用架构改造"
- **设计假设不匹配**：如通用模型假设离线推理，目标场景需要实时流式
...
flowchart TD
    ...
    C2 --> D1["取舍策略:<br/>选择两个,放弃一个"]
    C3 --> D2["架构重定义路径"]
    D1 --> E1["明确取舍依据<br/>(业务优先级)"]
    E1 --> F["✅ 困境处理完成<br/>(本质矛盾: 取舍)"]
    subgraph S3 ["步骤 3: 架构重定义"]
        G1["从目标场景倒推架构"]
        G2["三层协同重设计<br/>(训练/架构/部署)"]
        G3["量化突破预期<br/>(数量级提升)"]
        G1 --> G2 --> G3
    ...
    style S1 fill:#f0f0ff,stroke:#6666cc
    style S2 fill:#fff0f0,stroke:#cc6666
    style S3 fill:#f0fff0,stroke:#66cc66
```
...
### 案例分析来源
本案例来自外部文章分析任务，分析报告见 `docs/knowledge/learning/analyze-wechat-article-mainecoon/analysis-report.md`，通过 [external-article-deep-analysis-methodology.md](../research-knowledge/external-article-deep-analysis-methodology.md) 六步法的步骤 6（批判性思考）萃取为本模式。
...
### 失败案例 1：误将 CAP 定理当作架构遗留强行突破（某分布式数据库项目，2023 年）

- **场景**：某团队开发分布式数据库时，面临"一致性/可用性/分区容忍"三角困境，团队认定这是"架构遗留"，试图通过"重写共识算法"实现三者同时满足
- **结果**：投入 18 个月后仍未突破，最终被迫回退到 CAP 定理允许的取舍方案（选择 CP 或 AP），项目延期 2 年，错失市场窗口
- **失败根因**：步骤 2（根因分析）判定错误——CAP 定理是数学证明的本质矛盾（Brewer 定理 + Lynch 证明），不是架构遗留。团队将"物理/数学定律限制"误判为"设计假设问题"，导致在不可突破的困境上浪费资源
- **正确做法**：步骤 2 应严格区分：本质矛盾有物理定律/数学证明支撑（如 CAP、热力学第二定律、光速延迟），只能取舍不能突破；架构遗留在设计假设层面（如"为离线生成设计的架构用于实时场景"），可以通过重定义突破

### 失败案例 2：在旧架构上无限优化未实现数量级突破（某视频编码项目，2024 年）

- **场景**：某团队优化视频编码性能，在 H.264 架构上持续优化 2 年，通过汇编优化、并行化、缓存优化等手段，将编码速度提升了 3 倍（从 30fps 到 90fps）
- **结果**：竞争对手采用全新架构（端到端神经网络编码），直接实现 10 倍速度提升（300fps），团队 2 年的渐进式优化被瞬间超越
- **失败根因**：步骤 1（困境识别）后跳过了步骤 2（根因分析），直接在旧架构上局部优化。H.264 的架构假设（基于块匹配的运动补偿）决定了其性能上限，3 倍提升已接近架构天花板，无法实现数量级突破
- **正确做法**：应在步骤 2 判定困境性质——"块匹配架构假设"是架构遗留（非本质矛盾），应进入步骤 3 从"实时编码场景"倒推新架构（如神经网络编码），而非在旧架构上无限优化

## 反目标用户/场景分析

### 反目标 1：本质矛盾场景（CAP/热力学/光速限制）

- **特征**：困境的三个顶点受物理定律或数学定理约束（如分布式系统的一致性-可用性-分区容忍、热力学的效率极限、光速导致的延迟下限）
- **为什么不适用**：本质矛盾是宇宙规律，不可能通过"架构重定义"突破。本模式的核心假设是"困境源于架构遗留（设计假设问题）"，而本质矛盾不是设计假设问题。强行应用本模式会导致资源浪费在不可突破的困境上
- **替代方案**：使用「取舍决策框架」——明确选择牺牲哪个顶点，优化另外两个。如 CAP 下选择 CP（牺牲可用性）或 AP（牺牲一致性）

### 反目标 2：资源极度受限的早期创业团队

- **特征**：团队人数 <5 人，资金只够维持 6 个月，技术债务高，没有余力做架构级重设计
- **为什么不适用**：架构级重设计的成本（人力/时间/风险）远高于局部优化，早期创业团队承担不起。本模式适合资源充足且面临数量级瓶颈的团队，不适合"先活下来"的阶段
- **替代方案**：使用「最小可行优化」——在现有架构上做 80/20 优化（20% 的努力获得 80% 的收益），先解决生存问题，积累资源后再考虑架构突破

### 反目标 3：渐进式改进场景（性能提升 <2 倍）

- **特征**：当前性能瓶颈是工程层面（如算法复杂度、缓存命中率、I/O 模式），预期提升幅度在 2 倍以内
- **为什么不适用**：本模式的"架构重定义"是数量级突破手段（1/N、N 倍），对于 2 倍以内的渐进式改进，架构重设计的成本收益比极低（重设计成本 >> 渐进优化成本，但收益差距不大）
- **替代方案**：使用「性能剖析-瓶颈定位-针对性优化」工程方法——Profile → 定位热点 → 算法/数据结构/缓存优化

### 边界场景：架构遗留与本质矛盾的混合困境

- **特征**：困境的三个顶点中，部分受本质矛盾约束，部分受架构遗留约束（如"延迟-吞吐-一致性"中，延迟下限受光速约束，但吞吐和一致性受架构假设约束）
- **谨慎使用原因**：需要仔细拆解哪些顶点是本质矛盾（不能突破），哪些是架构遗留（可以突破），不能一刀切
- **适配建议**：步骤 2 分别判定每个顶点的性质，只对架构遗留顶点应用步骤 3（架构重定义），对本质矛盾顶点使用取舍策略

## 早期预警信号

当出现以下信号时，说明本模式可能不适用或需要调整：

| 信号编号 | 预警信号 | 含义 | 建议行动 |
|---------|---------|------|---------|
| 1 | 困境顶点有物理定律/数学定理支撑 | 可能是本质矛盾，不是架构遗留 | 步骤 2 严格验证是否有定理支撑，如有则切换到取舍模式 |
| 2 | 预期提升幅度 <2 倍 | 渐进式改进场景，架构重设计成本收益比低 | 使用工程优化方法而非架构重定义 |
| 3 | 团队资源只够维持 <6 个月 | 无法承担架构重设计的成本和风险 | 先做最小可行优化，积累资源后再突破 |
| 4 | 困境顶点之间有物理依赖（非架构依赖） | 顶点制约是物理规律而非设计假设 | 确认依赖性质，物理依赖不可突破 |
| 5 | 步骤 2 无法找到"设计假设问题" | 可能不是架构遗留，强行重定义会失败 | 如果找不到明确的设计假设问题，不要进入步骤 3 |

## 适用边界与前提条件

**前提条件**：
1. 困境经步骤 2 验证确实是架构遗留（有明确的设计假设问题），而非本质矛盾
2. 预期提升幅度是数量级（1/N 或 N 倍），而非渐进式（<2 倍）
3. 团队有足够的资源（人力/时间/资金）承担架构重设计的成本
4. 目标场景明确且稳定（架构重设计是为特定场景优化，场景频繁变化会导致重设计失效）

**适用边界**：
- ✅ 架构遗留困境、数量级瓶颈、资源充足团队、目标场景明确
- ❌ 本质矛盾（CAP/热力学/光速）、渐进式改进（<2 倍）、资源极度受限、场景不稳定
