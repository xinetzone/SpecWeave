...
trigger_conditions:
  ...
  -   - "external: 不存在-产品/技术存在已知局限性，需要决定是否披露以及如何披露"
...
### 失败案例 1：过度承认局限导致产品印象崩塌（2025 年某 AI 写作工具发布）

- **场景**：某 AI 写作工具在 Product Hunt 发布时，为了"诚实"，在产品介绍中列出了 12 项局限性（包括"长文生成会丢失上下文""不支持多语言""格式控制有限"等）
- **结果**：用户看到 12 项局限后，直观感受是"这产品问题很多"，导致首日转化率比预期低 60%
- **失败根因**：违反了"3-5 项"的数量约束，局限性过多让读者觉得产品不可用；且部分局限（如"不支持多语言"）与核心优势（"中文长文写作"）无关，无法形成衬托
- **正确做法**：应筛选 3-5 项"读者最易发现且可改进"的局限，并将"不支持多语言"表述为设计选择（"专注中文写作场景"）而非缺陷

### 失败案例 2：模糊表述被解读为"问题很多"（2024 年某开源项目 README）

- **场景**：某开源项目在 README 中写"目前还有一些已知问题，我们正在努力修复"
- **结果**：潜在贡献者认为"项目不稳定"而放弃使用，Issue 区被大量"还有哪些问题？"的提问淹没
- **失败根因**：违反了"具体而非模糊"约束，"一些已知问题"让读者自行脑补最坏情况，比具体列出 3 项局限更可怕
- **正确做法**：应具体列出"1. Windows 下 CUDA 支持不完整；2. 文档尚缺 API 参考；3. 大文件处理性能待优化"，并附改进时间表

## 反目标用户/场景分析

### 反目标 1：纯成交导向的销售人员

- **特征**：以快速成交为唯一目标，客户决策路径短（冲动消费/B2B 一次性采购）
- **为什么不适用**：销售人员无法控制局限性披露的节奏和深度，承认局限可能被竞争对手利用作为攻击点；且成交场景下客户信任建立更多依赖关系而非信息诚实性
- **替代方案**：使用异议处理技巧（先认同客户顾虑→转化→成交），而非主动承认模式

### 反目标 2：合规优先的安全关键系统文档

- **特征**：医疗设备、航空系统、金融交易系统等领域的合规性文档（FDA/FAA/监管要求）
- **为什么不适用**：这些场景的局限性披露是法律强制要求（如医疗器械的副作用清单），不披露会违法，披露也不是"为了构建信任"而是"为了满足合规"；本模式的"选择性披露 3-5 项"策略可能违反合规要求（必须披露全部已知风险）
- **替代方案**：严格按合规清单全量披露，使用「风险分级呈现」而非「选择性承认」

### 反目标 3：内部团队技术决策会议

- **特征**：团队内部技术选型/架构决策会议，参与者是技术专家，需要完整信息做决策
- **为什么不适用**：内部决策场景需要全量信息（所有优势 + 所有局限 + 所有不确定性），"选择性承认 3-5 项"会导致决策信息不完整；且团队成员之间已有信任基础，无需通过"承认局限"来构建信任
- **替代方案**：使用「完整信息呈现 + 风险矩阵」模式，列出所有已知问题和不确定性

### 边界场景：早期 MVP 产品发布

- **特征**：MVP 阶段产品功能不完善，局限很多（可能 10+ 项）
- **谨慎使用原因**：MVP 的局限数量通常超过 3-5 项的建议范围，需要谨慎筛选；且 MVP 阶段用户期望值不同（早期采用者更宽容），可以承认更多局限
- **适配建议**：将局限分为"设计选择"（不会改进）和"待优化"（会改进）两类，只对"待优化"类应用本模式，"设计选择"类单独说明取舍逻辑
...
| 信号编号 | 预警信号 | 含义 | 建议行动 |
|---------|---------|------|---------|
| 1 | 局限性清单超过 5 项 | 违反数量约束，用户可能觉得产品不可用 | 筛选最关键的 3-5 项，其余放入附录 |
| 2 | 局限性与核心优势无对照关系 | 无法形成"衬托"效应，承认局限变成纯减分 | 重新选择与优势有对照关系的局限 |
| 3 | 受众是合规审查人员 | 合规场景需要全量披露，不能选择性承认 | 切换到合规清单模式 |
| 4 | 局限性表述含"一些""可能""也许" | 模糊表述降低可信度，比不承认更差 | 改为具体表述（如"不支持 X"） |
| 5 | 承认的局限中包含致命缺陷 | 致命缺陷不应通过"承认"来处理，应先修复 | 先修复致命缺陷，再应用本模式处理次要局限 |

## 适用边界与前提条件

**前提条件**：
1. 产品/技术有真实的核心优势（本模式是"用局限衬托优势"，没有优势则无法衬托）
2. 局限性是可改进的或可解释为设计选择（纯缺陷不适用）
3. 受众有能力理解"取舍逻辑"（完全非技术消费者可能不理解）

**适用边界**：
- ✅ 产品发布内容、技术文章、AI 助手输出、竞品分析报告
- ❌ 销售话术、合规文档、内部决策会议、致命缺陷披露
