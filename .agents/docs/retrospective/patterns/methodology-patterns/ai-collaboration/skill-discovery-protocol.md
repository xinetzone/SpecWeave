...
| 查阅复盘模式库 | docs/retrospective/patterns/ | [docs/retrospective/patterns/README.md](../../README.md) |
...
```
步骤1：读取 [.agents/commands/retrospective.md](../../../../../commands/retrospective.md) 了解完整流程
...
| 案例 | 失败表现 | 根因 | 教训 |
|------|---------|------|------|
| 能力发现层缺失（协议诞生前，Firecrawl 复盘记录的真实问题） | 新会话 Agent 盲目 LS/Glob 遍历 .agents/ 目录、猜触发词、不知道命令集入口，PDR 前置读取负担重 | L0/L1 发现层不存在，能力只有"读过文档才知道" | Agent-First 发现层是启动协议的必要补充，本协议由此设计 |
| 上下文压缩后遗漏 vendor 资产（AGENTS.md 步骤2.2 记录的真实事件类型） | 会话从压缩摘要恢复后只依赖摘要行动，未重新执行路由协议，vendor 方法论资产被跳过，产出物格式/路径连锁出错 | 摘要替代了 L0 发现层：恢复后的会话没有重新读取 ONBOARDING/路由表 | 上下文恢复必须重新执行启动协议的发现步骤，不能信任摘要中的能力清单 |

### 反目标用户/反目标场景

| 反目标用户/场景 | 不适用原因 | 适配策略 |
|----------------|-----------|---------|
| 人类开发者手动查找文档 | 本协议面向 AI Agent 的机器可读索引设计 | 不适用：人类用 README/导航表即可 |
| 简单单文件操作（读一个已知路径的文件） | 完整发现流程的开销远超操作本身 | 轻度：直接操作，跳过发现流程 |
| 用户已明确指定 Skill（"Use Skill: xxx"） | 强匹配信号已存在，再走三层发现是浪费 | 轻度：直接加载指定 Skill |
| 紧急 Hotfix 修复 | 全量发现流程时间成本不成立 | 中度：快速提交方案直通，事后补登记 |
| 低频一次性脚本 | Skill 化维护成本高于收益（违反"一刀切 Skill 化"反模式） | 不适用：保持脚本形态，仅在注册中心登记 |

### 早期预警信号

| 预警信号 | 含义 | 应对 |
|---------|------|------|
| Agent 在新会话仍频繁 LS/Glob 遍历 .agents/ | L0/L1 入口未被使用或索引失效 | 检查 ONBOARDING.md 与注册中心是否过时 |
| 注册中心条目数与实际文件数不一致 | 注册中心漂移，Agent 会被误导 | 重跑生成脚本，将同步检查纳入 CI |
| 某触发词长期零命中或误触发率高 | description 五要素描述失真 | 依据使用统计修订触发词 |
| 门面 Skill 与 commands/ 内容出现不一致 | 门面复制了内容而非仅路由 | 回到"门面只路由不复制"原则 |
| 会话恢复后直接按摘要行动 | 发现步骤被跳过（步骤2.2 场景） | 强制重新执行启动协议步骤 1-2 |

### 适用前提

- 项目维护 AGENTS.md + ONBOARDING.md 两级入口，且注册中心可自动生成或有同步机制
- Skill 均有 SKILL.md 且 description 包含触发词（五要素模型）
- 面向 AI Agent 的会话环境；人类协作者不适用
- 能力数量达到"遍历发现成本 > 索引查询成本"的规模（能力极少的小项目收益有限）
