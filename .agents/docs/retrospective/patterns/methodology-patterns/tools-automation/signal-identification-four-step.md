...
```mermaid
flowchart TD
    subgraph FOUR_STEP_METHOD ["🔧 四步信号转化法"]
        A["人工Checklist规则"] --> B["①规则翻译<br/>代码中看到什么模式触发?"]
        B --> C["②信号强度评估<br/>强/中/弱?"]
        C --> D{"信号强度?"}
        D -->|"强信号"| E["直接报告HIGH"]
        D -->|"中信号"| F["③消歧策略设计<br/>命名/上下文/作用域/类型/测试排除"]
        D -->|"弱信号"| G["仅作为辅助判断<br/>不单独触发"]
        F --> H{"消歧结果?"}
        H -->|"确认问题"| E
        H -->|"确认误报"| I["跳过,不报"]
        H -->|"不确定"| J["降为MEDIUM警告"]
    end
    subgraph VALIDATION_RELEASE ["✅ 验证与发布（工程实践）"]
        K["TDD三重验证<br/>正向测试+反向测试+边界测试"]
        L["真实代码验证<br/>调优规则阈值"]
        M["集成pre-commit钩子<br/>增量快速检查"]
        N["CI层兜底<br/>全量深度扫描"]
    end
    E --> K
    I --> K
    ...
    style A fill:#e3f2fd
    style N fill:#c8e6c9
    style FOUR_STEP_METHOD fill:#f5f5f5,stroke:#9e9e9e,stroke-dasharray: 5 5
    style VALIDATION_RELEASE fill:#fff3e0,stroke:#ff9800,stroke-dasharray: 5 5
```
