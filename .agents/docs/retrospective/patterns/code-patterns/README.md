...
| [dockerfile-runtime-logical-layering.md](dockerfile-runtime-logical-layering.md) | Dockerfile Runtime P1-P6六步逻辑分层+P7同层修改：按变化频率分层（系统包→运行时复制→用户目录→目录准备→配置复制→清理验证），最大化构建缓存命中，每层末尾语法验证，改配置不触发重装包 | L2 已验证 | 多阶段Dockerfile runtime阶段编写、镜像构建缓存优化、Dockerfile审查 |
