...
| [markdown-nested-fence-escalation.md](markdown-nested-fence-escalation.md) | Markdown/MyST嵌套围栏升级法：代码块内含```围栏时外层升级为N+1反引号，首尾对称升级，杜绝围栏泄漏导致的标题跳跃/未知指令/转场异常等多类构建错误 | L1 实验性 | Markdown/MyST文档代码块嵌套、Sphinx/mkdocs/GitHub Markdown渲染错误修复 |
...
| [force-push-submodule-commit-recovery.md](force-push-submodule-commit-recovery.md) | Force-push剪除子模块提交的pull恢复法：定位断链→备份→合流→显式收敛gitlink→推送闭环→验证，6步修复 `not our ref` 子模块拉取失败 | L1 实验性 | 父仓库origin引用被force-push剪除的子模块提交、`git pull`报`not our ref`、子模块断链恢复 |
| [submodule-detached-head-ff-only-landing.md](submodule-detached-head-ff-only-landing.md) | 子模块汇合仓库detached HEAD显式落分支法：提交后检测symbolic-ref→判定父提交=分支tip→ff-only快进落分支→复核→推送闭环，杜绝悬空提交被GC/覆盖 | L2 已验证 | 汇合型子模块仓库（如awesome-okf-xs）检出后直接commit、CI detached检出回写提交、实验性提交落分支 |
...
| [scikit-build-core-pure-python-minimal.md](scikit-build-core-pure-python-minimal.md) | scikit-build-core纯Python项目最小配置：pyproject.toml最小配置+9行CMakeLists.txt（LANGUAGES NONE）+wheel.packages自动发现，避开6个陷阱：勿手动加cmake/ninja依赖、用build.verbose非cmake.verbose、用cmake.args设置Ninja非cmake.generator、勿混用dotted key与显式[table]、纯Python也需CMakeLists.txt、勿手写install(DIRECTORY) | L1-draft | 纯Python包从setuptools迁移scikit-build-core、纯Python CLI/库/invoke任务工具、未来计划添加C扩展的渐进式项目 |
| [invoke-layered-namespace-tasks.md](invoke-layered-namespace-tasks.md) | invoke任务分层命名空间模式：核心命令平铺+领域命令分层（二八原则），80%高频命令根命名空间直达，20%领域命令子命名空间分类，核心命令同时暴露别名，解决全平铺难发现/全分层效率低两种反模式，含tasks/__init__.py完整模板 | L1-draft | Python项目invoke任务管理、容器/开发环境CLI、10-25个任务的工具项目、任务分类组织 |
| [toml-table-declaration-consistency.md](toml-table-declaration-consistency.md) | TOML表声明一致性原则：dotted key（如cmake.version）隐式建表与显式[table]表头（如[tool.scikit-build.cmake]）不可混用同一键路径，违反后果为pip install挂起无输出（异常被吞）；推荐统一使用显式[table]声明，含跨工具速查+5秒自检命令 | L1-draft | pyproject.toml/ruff.toml等TOML配置、scikit-build-core/ruff/mypy/pytest配置、所有[tool.xxx]子表配置 |
...
| [bash-safe-dotenv-loading.md](bash-safe-dotenv-loading.md) | Bash安全.env加载：逐行词法解析（while IFS=read+正则），禁止source/eval，自动剥离CR/引号，含路径后处理钩子，解决反斜杠路径破坏/CRLF污染/代码注入三类风险 | L2 已验证 | WSL/Linux/macOS bash脚本加载Windows生成的.env文件、跨平台CLI工具、Docker entrypoint脚本 |
| [wsl-windows-path-autoconvert.md](wsl-windows-path-autoconvert.md) | WSL Windows路径自动转换：纯Bash正则检测盘符路径→小写化盘符→反斜杠转正斜杠→拼接/mnt/前缀，幂等设计（已转换路径不二次处理），不依赖wslpath | L2 已验证 | WSL bash脚本接受Windows路径参数、.env路径变量后处理、PowerShell→WSL跨Shell参数传递 |
| [multi-entrypoint-config-unification.md](multi-entrypoint-config-unification.md) | 多入口配置接口统一：以.env变量名为命名权威→所有入口对齐→旧名fallback兼容链→开箱即用默认值→优先级文档化，解决"配置了但没用"的接口不一致问题 | L2 已验证 | 多入口项目（CLI+SDK+Compose+脚本）、容器工具配置统一、环境变量命名规范化 |
...
| [fixuid-runtime-uid-mapping.md](fixuid-runtime-uid-mapping.md) | FixUID运行时UID映射模式：三级UID来源优先级+四重安全保护+挂载点智能检测(bind mount跳过chown)+系统目录黑名单+四档chown模式(auto/yes/no/named-only)+启动横幅可观测，解决Docker bind mount UID不匹配导致的Permission denied | L2 已验证 | Dev Containers/VS Code Remote开发容器、bind mount权限治理、多用户共享开发环境、SSH/Jupyter/IDE多入口容器 |
| [dockerfile-runtime-logical-layering.md](dockerfile-runtime-logical-layering.md) | Dockerfile Runtime P1-P6六步逻辑分层+P7同层修改：按变化频率分层（系统包→运行时复制→用户目录→目录准备→配置复制→清理验证），最大化构建缓存命中，每层末尾语法验证，改配置不触发重装包；v1.4新增两类清理本质区分+conda-libmamba优化+悬空符号链接修复 | L2 已验证 | 多阶段Dockerfile runtime阶段编写、镜像构建缓存优化、Dockerfile审查 |
| [docker-cow-same-layer-modification.md](docker-cow-same-layer-modification.md) | P7同层修改原则（COW膨胀防御）：strip/chmod/修改必须在文件创建同层RUN完成，最终清理层仅rm -rf(whiteout)；含strip参数选择指南、docker history验证方法、btrfs/ZFS/Git迁移示例 | L2 已验证 | 多阶段Dockerfile体积优化、strip二进制、chown大目录、避免上层修改低层文件COW膨胀 |
| [docker-deep-slim-8step.md](docker-deep-slim-8step.md) | Docker镜像深度压缩8步法：binutils前置→Go strip --strip-all→C/C++ strip --strip-unneeded→同层清缓存→PYTHONDONTWRITEBYTECODE→手动rm冗余→最终层纯rm(verify→clean顺序)→post-verify sweep；含6个反模式(GPU.so不盲目strip/清理验证顺序)；3案例验证(devcontainer-base+onnx-pytorch+pytorch-base)，实测2.91GB→1.41GB(51.5%) | L2 已验证 | Debian/Ubuntu基础镜像深度压缩、含conda/pip的开发/ML镜像瘦身、>2GB镜像系统化优化、Dockerfile体积审查 |
| [docker-gpu-slimming-sop.md](docker-gpu-slimming-sop.md) | GPU/ML Docker镜像瘦身SOP四步法：分析定位(du+history)→R1基础瘦身(确定项strip+冗余库删除-1.5~2GB)→R2深度瘦身(DT_NEEDED动态依赖分析-100~200MB)→7项验证清单；DT_NEEDED三层分类决策树(readelf→strings→功能验证)；Dockerfile集成模板+5个反模式(cow膨胀/torchgen误删/cuda盲目strip/跳过验证/顺序颠倒)；torch-dev-slim实战10.2GB→8.15GB(-20%) | L2 已验证 | PyTorch/TensorFlow/CUDA GPU镜像瘦身、ML框架变体新增、不确定.so可否删除的场景、GPU镜像Dockerfile审查 |
...
| [oci-image-wsl-rootfs-bridge.md](oci-image-wsl-rootfs-bridge.md) | OCI镜像→WSL2发行版运行时桥接转换模式：用OCI运行时（Podman/Docker）处理叠层合并/whiteout/Unix权限，5步法实现docker save tar.gz→WSL flat rootfs转换，含6个反模式（禁止纯文件解压/禁止直接wsl import等） | L1 实验性 | Docker/Podman镜像转WSL发行版、无Docker Desktop环境镜像恢复、docker-cache缓存恢复为WSL开发环境 |
...
| [wsl-docker-gpu-triage.md](wsl-docker-gpu-triage.md) | WSL Docker GPU三层诊断修复模式：L1驱动→L2工具→L3运行时逐层验证，8步完成nvidia-container-toolkit安装+runtime配置+setsid启动+wsl.conf持久化，含5个反模式+故障排查速查表+跨领域分层诊断迁移 | L1 实验性 | WSL2 Docker GPU支持排障、`could not select device driver`错误修复、新WSL发行版Docker GPU初始化 |
| [wsl-docker-storage-cleanup-five-step-method.md](wsl-docker-storage-cleanup-five-step-method.md) | WSL/Docker存储清理五步法：预检+快照→锚点三层保护带→优先级顺序清理(容器→BuildKit Cache -a→孤儿卷→实验镜像TAG)→三重存活验证(Docker状态/进程/TCP)→空间核验+报告，精确控制保护带避免误删业务容器 | L2 已验证 | WSL2发行版vhdx膨胀、磁盘空间告警、Docker迭代后大量废弃镜像/孤儿卷/BuildKit缓存、ext4.vhdx compact前置清理 |
...
| [wsl-podman-build-bridge.md](wsl-podman-build-bridge.md) | Windows→WSL Podman构建桥接模式：跳过podman machine多余VM层直接调用WSL发行版内已配置podman，固定三必加参数（--format docker修复SHELL被OCI格式忽略/--network=host绕过bridge网络隔离/国内镜像build-arg），含预检脚本+PowerShell路径自动转换包装器+5个反模式（bash单引号内\"误转义/importlib.metadata版本检查/podman-machine-default警告临时修复） | L1-draft | Windows上Podman容器构建、Conda/Python数据科学镜像国内加速构建、WSL内已有rootless podman环境的复用场景 |
...
| [docker-build-four-layer-verification.md](docker-build-four-layer-verification.md) | Docker构建四层验证流水线：L1预检→L2构建内RUN验证→L3容器冒烟→L4性能基准，每层失败立即中止，配套诊断脚本+日志持久化+镜像自动清理 | L2 已验证 | 含新Python发行版/ABI变体/编译工具链升级的镜像构建、复杂多阶段Dockerfile、CI/CD镜像质量门禁 |
| [conda-abi-variant-safe-switching.md](conda-abi-variant-safe-switching.md) | Conda ABI变体安全切换七步防御：发行版选择→Channel锁定→独立环境→PATH优先级→构建时验证→运行时诊断→文档标注，应对free-threading等ABI变体静默降级 | L2 已验证 | Python free-threading(cp313t/cp314t)、debug构建、Pyodide/WASM等新ABI变体集成、conda-forge与defaults channel混用风险场景 |
| [conda-build-performance-triple-optimization.md](conda-build-performance-triple-optimization.md) | Conda构建层性能三联优化：并行度调优(8线程)+单次mamba create合并包安装+原生mamba CLI，配合BuildKit缓存挂载+内联计时，Stage 4从419s→37s(11.3x热构建加速) | L1 实验性 | Dockerfile中conda环境创建耗时长(>2分钟)、CI流水线conda环境是构建瓶颈、需要量化conda步骤性能回归检测 |
| [config-asset-dual-form.md](config-asset-dual-form.md) | 配置资产化双形态：萃取三步法(参数化→双形态→共享目录)+静态模板(COPY即用)与动态脚本(执行/source双模式+自动fallback)双形态交付，内联配置50行→3行脚本调用 | L1 实验性 | Dockerfile/CI内联配置块(conda/npm/pip/apt)需跨项目复用、优化验证通过后萃取配置资产、静态固定配置与动态参数配置并存场景 |
