...
x-toml-ref: "../../../../../.meta/toml/.agents/docs/retrospective/patterns/code-patterns/mermaid-safe-coding-rules.toml"
---
# Mermaid 安全编码七规则
...
9. <br/> HTML标签（VS Code预览不支持，规则8）
10. 带圈数字①-⑩（VS Code预览解析中断，规则8）
11. 中文方括号【】（VS Code预览误解析，规则8）
12. Unicode箭头符号→←↑↓（VS Code兼容性警告，规则8）
13. subgraph内嵌套direction（VS Code布局异常，规则8）

支持 `--fix` 参数自动修复部分问题（空行、引号、`\n`→空格、<br/>移除、带圈数字替换、中文方括号替换、箭头符号替换）。
...
- **开发规范**：`.agents/docs/development-standards.md`（Mermaid 编码规范章节）
