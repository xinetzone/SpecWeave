...
MDI核心模块位于 `.agents/scripts/mdi/`：

| 模块 | 文件 | 职责 |
|------|------|------|
| 模型层 | [models.py](../../../../../scripts/mdi/models.py) | MDIDocument/Interface/Parameter/Response/ErrorCode数据类 |
| 解析层 | [parser.py](../../../../../scripts/mdi/parser.py) | Markdown→Block→结构化模型（含MyST Directive解析，ALLOWLIST待拆分） |
| 验证层 | `validator/` | Profile检测+12项规则验证+评分系统（已拆分为core.py+rules/子包，9模块） |
| 生成层 | `generators/` | 9种目标格式生成器（jest_gen/pytest_gen已拆分为子包） |
| 测试工具 | [mock_data.py](../../../../../scripts/mdi/mock_data.py) | 语义化Mock数据生成 |
| 测试工具 | [example_extractor.py](../../../../../scripts/mdi/example_extractor.py) | 代码块示例提取（JSON/Python/curl/HTTP） |
| 测试工具 | [checklist_converter.py](../../../../../scripts/mdi/checklist_converter.py) | 检查清单→测试断言步骤转换 |
| 版本管理 | `versioning/` | 结构化diff+影响分析+SemVer版本建议（已拆分为5模块包） |
| MCP领域 | `mcp_domain/` | MCP Server领域模型（已拆分为7模块包） |
| Profile层 | `profiles/` | 5种Profile实现（webapi/skill/cli/graphql/clitool） |
| CLI入口 | [__main__.py](../../../../../scripts/mdi/__main__.py) | validate/gen/diff三个子命令 |
| 公共API | [__init__.py](../../../../../scripts/mdi/__init__.py) | parse()/validate()/generate()/diff_files()统一入口 |
