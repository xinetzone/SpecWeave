...
x-toml-ref: "../../../../../../.meta/toml/.agents/docs/retrospective/reports/standards-tools/myst-to-agentspec-migration-analysis/09-six-dimensions-evaluation.toml"
---
...
**现有基础**：项目已有的MDI v1.0解析器在markdown-it-py基础上实现了`{endpoint}`/`{param}`/`{response}`/`{error}`等directive，对应MDIDocument/Interface/Parameter等数据模型（[models.py](../../../../../scripts/mdi/models.py)）。当前解析器在代码层面已预留Directive扩展点，但存量文档使用率为0%。
