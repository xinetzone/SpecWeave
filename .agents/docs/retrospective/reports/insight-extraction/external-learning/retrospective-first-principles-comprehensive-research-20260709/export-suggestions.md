...
| ACT-008 | 开发自动化学术来源验证脚本 | 实现L0-L2：DOI/arXiv ID提取、CrossRef API元数据验证、标题/作者/年份一致性比对（只读检查工具，不做引用计数/自动修复/可信度评级） | 中 | ✅ 完成 (commit 72ed64ff)：脚本`.agents/scripts/check-academic-sources.py`+41个单元测试，支持三级验证（L0提取/L1验证/L2比对）、7天缓存、并发请求、文本/JSON双模式报告；试运行first-principles知识库：12通过/3警告（同句多引用预期行为）/1错误（1980年老DOI未在CrossRef注册） |
