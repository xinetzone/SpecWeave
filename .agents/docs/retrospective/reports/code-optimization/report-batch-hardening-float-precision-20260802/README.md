...
| 行动项 | 优先级 | 建议处理方式 |
|--------|--------|------------|
| 将C¹拐点防护逻辑提取为共享helper函数 | 中 | 如`_avoid_c1_kink(x, h)`放入caffe_test_helpers.py |
| AbsVal/Power层补充数值梯度测试 | 低 | 当前无反向数值梯度测试，但AbsVal在x=0处C⁰不连续 |
| 新增自动化lint规则检测ULP违规 | 低 | 开发pytest插件或pre-commit hook自动扫描`> 1.0 - 1e-N`模式 |
| tanh饱和阈值精确验证 | 低 | tanh(10)精确=±1.0，tanh(9.9)=0.99999994，建议在helper中记录精确阈值 |

---

## 10. 结论

本次批量加固将4个浮点数精度测试核心模式系统应用到caffe-ffi所有激活函数测试文件中：

1. **修复了3处确定性/概率性问题**（1处ULP违规+2处C¹不连续拐点flake风险）
2. **修正了1处严重文档错误**（sigmoid负饱和阈值偏差72个数量级）
3. **扩展了核心模式的适用范围**（区分C¹连续/C¹不连续两类拐点的处理策略）
4. **零回归**：146个测试全部通过
5. **知识闭环**：指南文档、测试代码、专项测试三者一致，形成可复用的最佳实践
