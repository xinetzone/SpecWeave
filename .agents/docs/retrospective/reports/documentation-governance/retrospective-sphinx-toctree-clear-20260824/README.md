...
description: 基于 seven-concepts 编排（R→I→E→C），对 awesome-okf-xs 文档构建中 4888 个 toc.not_included 警告的根因分析、自动化修复脚本开发、867 文件变更与 2 个原子提交的完整复盘
...
| toctree 动态验证法 | 方法论 | `.temp/verify_toctrees.py` BFS 遍历可达性，过滤 toctree 选项行与语法示例 | 把示例代码块当真实引用导致误报 |
...
| # | 行动项 | 优先级 | 验收标准 |
|---|--------|:--:|----------|
| A-1 | OKF bundle 生成规范纳入"必须生成 index.md 并带 toctree"约束 | 高 | bundle 目录扫描零缺失 index.md |
| A-2 | 新增 toctree 与目录文件清单一致性校验脚本 | 中 | 已有 toctree 文件无缺失条目告警 |
| A-3 | 将目录树完整性修复工作流沉淀为可复用模式 | 中 | pattern 入库并登记索引 |
| A-4 | 修复剩余 6406 个非 TOC 警告（myst.header / myst.topmatter） | 低 | 内容格式警告分类归零 |
