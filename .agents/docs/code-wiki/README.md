...
2. **提示词萃取系统**：位于 `apps/prompt_extraction/` 的 Python 子项目，提供从提示词文本或文件中抽取结构化特征、评估质量、生成优化结果和可视化展示的完整流水线。
...
| [模块职责](modules.md) | `.agents/`、`docs/`、`.trae/specs/`、`apps/prompt_extraction/` 等主要模块职责 |
...
- `apps/prompt_extraction/` 是可执行 Python 子项目，核心采用流水线架构，以 `PromptRecord` 贯穿输入解析、文本清洗、特征提取、质量评估、优化生成和结果导出。
...
| [`apps/prompt_extraction/pipeline.py`](../../../apps/prompt_extraction/pipeline.py) | 提示词萃取系统流水线编排器 |
| [`apps/prompt_extraction/models.py`](../../../apps/prompt_extraction/models.py) | 提示词萃取系统核心数据模型 |
| [`apps/prompt_extraction/ui/app.py`](../../../apps/prompt_extraction/ui/app.py) | Streamlit 可视化主应用 |
| [`.agents/scripts/README.md`](../../scripts/README.md) | 自动化验证脚本索引 |

## 维护建议

- 当 `.agents/` 规范、`apps/prompt_extraction/` 源码或运行方式发生变化时，应同步更新本 Code Wiki。
