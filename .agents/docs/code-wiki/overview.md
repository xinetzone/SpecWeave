...
| `.trae/specs/` | Spec 驱动开发的过程资产区 | [context-routing.md](../../context-routing.md#L53-L55) |
| `docs/` | Sphinx 文档站工程 | [docs/tasks.py](../../../docs/tasks.py#L54-L128) |

## 当前工作树快照

当前 checkout 下最值得关注的内容如下：

- `.agents/docs/`：项目文档主容器，承载知识库、复盘、标准、工具说明与本 Code Wiki。
- `.agents/scripts/`：自动化工具主目录，包含大量顶层 CLI 和 `lib/` 共享库。
...
- `docs/`：对外文档站，依赖 Sphinx、MyST、Mermaid 扩展。

## 设计目标

### 1. 统一入口

智能体进入仓库时先读 [AGENTS.md](../../../AGENTS.md#L3-L32)，再根据 [context-routing.md](../../context-routing.md#L21-L100) 只加载任务所需规范，避免一次性读取全仓文档造成上下文膨胀。

### 2. 规则即代码

规范并不只存在于文档中，还在 `.agents/scripts/` 中被实现为检查器、生成器、修复器与聚合工具。最典型的例子包括：

- [docgen.py](../../scripts/docgen.py#L1-L17)
- [check-links.py](../../scripts/check-links.py)
- [build-ref-index.py](../../scripts/build-ref-index.py#L171-L210)
- [repo-check.py](../../scripts/repo-check.py)
...
[global-core-rules.md](../../global-core-rules.md#L19-L20) 明确指出 `.agents/docs/` 是唯一有效的文档主容器，根 `docs/` 更多承担 Sphinx 站点工程角色。因此：

- 读规范与知识库时优先看 `.agents/docs/`
- 构建公开站点时看 `docs/`
