...
3. `apps/dev-tools/prompt_extraction/` Python 子项目内部模块依赖。
4. Python 第三方运行依赖。
...
flowchart TB
    ...
    A --> C["docs 文档体系"]
    A --> D[apps/dev-tools/prompt_extraction]
    B --> E[".agents/scripts 验证脚本"]
    C --> F[docs/knowledge]
    C --> G[docs/retrospective]
    D --> H["Python 第三方依赖"]
    D --> I[apps/dev-tools/prompt_extraction/tests]
```

## Python 第三方依赖

依赖文件：[`apps/dev-tools/prompt_extraction/requirements.txt`](../../../apps/dev-tools/prompt_extraction/requirements.txt)
...
## `apps/dev-tools/prompt_extraction/` 模块依赖图
...
`apps/dev-tools/prompt_extraction/tests/` 覆盖以下层级：
...
`README.md` 是面向人类读者的项目入口，引用 `docs/`、`.agents/` 和 `apps/dev-tools/prompt_extraction/`。`docs/retrospective/` 中的复盘报告和模式库又为后续开发提供可复用方法论。
