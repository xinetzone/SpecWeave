...
flowchart TB
    ...
    Router --> Prompt["apps/dev-tools/prompt_extraction 提示词萃取系统"]
    Agents --> Roles["角色定义"]
...
flowchart LR
    ...
    C -->|"代码实现"| G["apps/dev-tools/prompt_extraction 或 apps"]
...
`apps/dev-tools/prompt_extraction/` 采用典型流水线架构，核心编排器为 `Pipeline`，核心数据载体为 `PromptRecord`。
