...
flowchart TB
    ...
    workspace --> site["docs/<br/>Sphinx 站点"]
```
...
- [context-routing.md](../../context-routing.md#L21-L100)
- [global-core-rules.md](../../global-core-rules.md#L13-L33)
...
- 顶层 CLI 负责命令编排
- `lib/` 提供共享能力
- `sg_dashboard/` 和 `stage_guardrails/` 提供更完整的子系统

最典型的聚合工具是 [docgen.py](../../scripts/docgen.py#L1-L17)，它将导航、看板、应用清单和统计更新组合为统一命令面。
...
- [agents.py 中的 `generate_project()`](../../scripts/agents.py#L171-L242)
- [docgen.py 中的 `cmd_nav()`](../../scripts/docgen.py#L124-L158)
- [build-ref-index.py 中的 `build_index()`](../../scripts/build-ref-index.py#L171-L210)
- [project.py 中的 `resolve_project_root()`](../../scripts/lib/project.py#L18-L53)
- [frontmatter.py 中的 `parse_frontmatter_unified()`](../../scripts/lib/frontmatter.py#L508-L536)

## 阶段守卫架构

阶段守卫是这个仓库里最完整的治理型运行时子系统之一，由 3 个核心组件组成：

- 状态机：[`StageStateManager`](../../scripts/lib/stage_guardrails/state/manager.py#L21-L115)
- 边界校验：[`BoundaryChecker`](../../scripts/lib/stage_guardrails/boundary.py#L450-L549)
- 运行时门面：[`GuardrailRuntime`](../../scripts/lib/stage_guardrails/runtime.py#L83-L207)
...
根 `docs/` 目录是对外静态站点工程，不是规范主容器。其构建入口集中在 [docs/tasks.py](../../../docs/tasks.py#L54-L128)，通过 `invoke` 封装 `sphinx-build -M`：
