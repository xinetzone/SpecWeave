...
| 运行提示词萃取系统 | Python、pip | 安装 `prompt_extraction/requirements.txt` |
| 运行 Web UI | Python、pip、Streamlit | 使用 Streamlit 启动 `prompt_extraction/ui/app.py` |
| 运行测试 | Python、pip、pytest | 执行 `prompt_extraction/tests/` 测试套件 |
...
```powershell
python -m pip install -r prompt_extraction\requirements.txt
```
...
```powershell
python -m streamlit run prompt_extraction\ui\app.py
```
...
```powershell
python -m pytest prompt_extraction\tests
```
...
| 任务 | 命令 |
|---|---|
| 安装提示词萃取依赖 | `python -m pip install -r prompt_extraction\requirements.txt` |
| 启动 Web UI | `python -m streamlit run prompt_extraction\ui\app.py` |
| 运行全部提示词萃取测试 | `python -m pytest prompt_extraction\tests` |
...
| 修改区域 | 建议验证 |
|---|---|
| `prompt_extraction/` 源码 | `python -m pytest prompt_extraction\tests` |
