...
> - **执行依据**：[atomic-commit-cmd Skill](../../skills/atomic-commit-cmd/SKILL.md) + [AGENTS 启动协议](../../../AGENTS.md)
...
- 必须使用 [atomic-commit-cmd Skill](../../skills/atomic-commit-cmd/SKILL.md) 流程，禁止直接 `git commit`
- Windows 平台中文提交信息必须用 [git-commit-utf8.py](../../scripts/git-commit-utf8.py) 处理编码
...
读取 [AGENTS.md](../../../AGENTS.md)、[上下文路由表](../../context-routing.md)、[全局核心规则](../../global-core-rules.md)，完成步骤 1-3.5 自检：
...
| 工具 | 用途 | 调用次数 | 效果 |
|------|------|---------|------|
| [git-commit-utf8.py](../../scripts/git-commit-utf8.py) | Windows 中文提交编码 | 5 次 | 0 乱码 |
| [check-links.py](../../scripts/check-links.py) | 链接有效性验证 | 10+ 次 | 全部通过/甄别完成 |
...
| # | 建议 | 触发原因 | 行动项 | 负责区域 |
|---|------|---------|--------|---------|
| 1 | 提交前全量扫描强化 | 第5类变更未在初始扫描显示 | 在 atomic-commit-cmd Skill 的 S1 步骤增加"每个提交后重新扫描"强制项 | [.agents/skills/atomic-commit-cmd/SKILL.md](../../skills/atomic-commit-cmd/SKILL.md) |
| 2 | check-links.py 支持多路径 | C 分组需循环跑 7 次链接检查 | 增强 `--path` 参数支持多值（或支持 `--paths f1,f2,f3`） | [.agents/scripts/check-links.py](../../scripts/check-links.py) |
...
| 7 | 提交类型选择指引 | C 分组用 docs 还是 refactor 有讨论空间 | 在 [commands/atomic-commit.md](../../commands/atomic-commit.md) 增加提交类型决策树 |
