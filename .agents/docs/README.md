...
- 原根目录 `docs/` 迁移而来、主要面向项目维护者、开发者、贡献者和普通读者的文档。
- 典型用途包括阅读说明、知识检索、学习导航和人工维护。
- 典型入口包括本目录 `README.md`、各主题索引页，以及人工浏览进入的子目录。

## 智能体专属文档范围

- 原生服务于 agent 路由、规则装载、脚本发现与自动执行的文档。
- 主要读者包括 agent、自动化脚本、路由流程和治理流程。
- 典型入口包括 `AGENTS.md`、`.agents/context-routing.md`、规则入口与脚本逻辑。
