...
| 脚本 | 路径 | 作用 |
|------|------|------|
| setup-utf8-env.ps1 | `.agents/scripts/` | 一键配置主脚本 |
| sitecustomize.py | `.agents/scripts/` | Python启动自动加载的UTF-8配置（需PYTHONPATH） |
| check-encoding.ps1 | `.agents/scripts/` | 编码诊断 |
| verify-encoding.ps1 | `.agents/scripts/` | 14项验证测试 |
| verify-sitecustomize-autoload.py | `.agents/scripts/` | sitecustomize.py 自动加载验证（退出码 0/1/2，支持 `--json`） |
...
| 脚本 | 路径 | 作用 | 关联陷阱 |
|------|------|------|---------|
| setup-utf8-env.ps1 | `.agents/scripts/` | 一键UTF-8配置主脚本 | 陷阱1编码 |
| sitecustomize.py | `.agents/scripts/` | Python启动自动加载的UTF-8配置 | 陷阱1编码 |
...
| verify-sitecustomize-autoload.py | `.agents/scripts/` | sitecustomize.py 自动加载验证（退出码 0/1/2，支持 `--json`） | 陷阱1编码 |
| check-file-placement.py | `.agents/scripts/` | 关键配置文件放置校验（退出码 0/1，支持 `--json`/`--fix-hint`） | 陷阱1编码 |
| check-temp-lifecycle.py | `.agents/scripts/` | .temp 生命周期检查与清理（退出码 0/1，支持 `--clean`/`--yes`/`--json`/`--path`） | 跨平台Python |

### 新增验证与治理脚本详情（2026-07）

下表列出 2026-07 新增的三个验证与治理脚本，用于 UTF-8 自动加载验证、关键配置文件放置校验、`.temp/` 临时文件生命周期治理。每项包含脚本名、位置、用途、退出码、关键参数五个字段：
...
- [ ] **编码配置**：是否运行过 `.agents/scripts/setup-utf8-env.ps1`？编码是否为 UTF-8？
