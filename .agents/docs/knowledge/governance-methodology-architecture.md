...
1. **B1**：[stage-guardrails.md](../../rules/stage-guardrails.md) 定义8阶段边界、拦截规则、SG-LOG格式
2. **B2**：[check-stage-guardrails.py](../../scripts/check-stage-guardrails.py) 离线分析SG-LOG日志，`--demo`模式演示
3. **C1**：[check-stage-guardrail-runtime.py](../../scripts/check-stage-guardrail-runtime.py) + `lib/stage_guardrails/`(4734行)运行时门面
4. **C2**：[generate-sg-dashboard.py](../../scripts/generate-sg-dashboard.py) HTML可视化仪表盘
...
| 组件 | 层级 | 成熟度 | 状态 | 位置 |
|------|------|--------|------|------|
| 治理四层递进模型 | 核心架构 | L2 | ✅ 已纳入规范 | [stage-guardrails.md](../../rules/stage-guardrails.md) |
| 四层跳层检测 | B2工具 | L2 | ✅ 已实现 | [check-stage-guardrails.py](../../scripts/check-stage-guardrails.py) |
| 二次暴露治理闭环 | 治理触发 | L2 | ✅ 已纳入检查点 | [pre-document-reading.md](../../protocols/pre-document-reading.md) |
| code-review治理闭环检查 | B2检查 | L1 | ✅ 已加入清单 | [code-review.md](../../workflows/code-review.md) |
...
| 提交粒度预警 | B2工具 | L1 | ✅ 脚本完成，待CI集成 | [check-commit-size.py](../../scripts/check-commit-size.py) |
| CMD-LOG遵循度 | B1规范 | L2 | ⏳ B1刚完成，待稳定后建B2 | [cmd-log-specification.md](../../rules/cmd-log-specification.md) |
| CI编码安全 | B2/B1 | L2 | ✅ 跨平台设置完成 | [ci-check.ps1](../../scripts/ci-check.ps1)/[ci-check.sh](../../scripts/ci-check.sh) |
...
| [.agents/rules/stage-guardrails.md](../../rules/stage-guardrails.md) | 阶段守卫规则定义（含四层递进模型章节） |
| [.agents/protocols/pre-document-reading.md](../../protocols/pre-document-reading.md) | 前置文档强制读取协议（含二次暴露检查点） |
| [.agents/rules/cmd-log-specification.md](../../rules/cmd-log-specification.md) | CMD-LOG结构化日志规范 |

### 7.2 工具脚本

| 脚本 | 层级 | 说明 |
|------|------|------|
| [check-stage-guardrails.py](../../scripts/check-stage-guardrails.py) | B2 | 阶段守卫日志离线分析（含四层跳层检测） |
| [check-commit-size.py](../../scripts/check-commit-size.py) | B2 | 提交粒度预警（四级阈值分级） |
| [generate-sg-dashboard.py](../../scripts/generate-sg-dashboard.py) | C2 | SG日志可视化仪表盘 |
| [check-stage-guardrail-runtime.py](../../scripts/check-stage-guardrail-runtime.py) | C1 | 阶段守卫运行时门面 |
