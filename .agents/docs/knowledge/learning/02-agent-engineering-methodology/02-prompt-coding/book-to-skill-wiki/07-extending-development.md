...
book-to-skill 的解析器采用模块化设计，每个格式对应一个独立的解析器模块。新增格式支持遵循四步标准化流程，参考 [ARCHITECTURE.md:99-103](https://github.com/virgiliojr94/book-to-skill/blob/main/docs/ARCHITECTURE.md#L99-L103)。
...
参考现有解析器的实现：
- [parsers/pdf.py](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/parsers/pdf.py)
- [parsers/epub.py](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/parsers/epub.py)
- [parsers/docx.py](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/parsers/docx.py)
...
在 [config.py](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/config.py) 中注册新格式的文件扩展名：
...
参考：[config.py:16-34](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/config.py#L16-L34)

### 步骤 3：添加依赖探测

在 [dependencies.py](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/dependencies.py) 的 `DEPENDENCY_GROUPS` 列表中添加新格式的依赖探测配置：
...
同时需要在 `prepare_dependencies()` 函数中添加对应的依赖安装提示逻辑，参考 [dependencies.py:166-213](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/dependencies.py#L166-L213)。

### 步骤 4：集成到提取流程

在 [utils.py](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/utils.py) 的 `extract_single_file()` 函数中添加新格式的分支逻辑：
...
参考现有 PDF 格式的 fallback 链实现：[utils.py:484-530](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/utils.py#L484-L530)

## 7.2 修改生成行为

生成行为由 [SKILL.md](https://github.com/virgiliojr94/book-to-skill/blob/main/SKILL.md) 定义，这是 agent 遵循的规范文档。修改生成行为需遵循以下原则：

### 核心原则

1. **保持精简（lean）**：SKILL.md 是每次运行都会加载的核心文档，避免不必要的膨胀
2. **证据支持变更**：根据 [CONTRIBUTING.md](https://github.com/virgiliojr94/book-to-skill/blob/main/CONTRIBUTING.md)，所有修改必须有可测量的证据支持
...
参考：[CONTRIBUTING.md:9-14](https://github.com/virgiliojr94/book-to-skill/blob/main/CONTRIBUTING.md#L9-L14)
...
参考：[tools/discovery_tax.py](https://github.com/virgiliojr94/book-to-skill/blob/main/tools/discovery_tax.py)
...
参考：[tools/validate_skill.py](https://github.com/virgiliojr94/book-to-skill/blob/main/tools/validate_skill.py)
...
参考：[tools/scan_generated_skill.py](https://github.com/virgiliojr94/book-to-skill/blob/main/tools/scan_generated_skill.py)
...
参考：[scripts/extract.py](https://github.com/virgiliojr94/book-to-skill/blob/main/scripts/extract.py)

## 7.4 优雅降级设计原则

优雅降级（Graceful degradation）是 book-to-skill 的核心设计原则之一，参考 [ARCHITECTURE.md:57-58](https://github.com/virgiliojr94/book-to-skill/blob/main/docs/ARCHITECTURE.md#L57-L58)。
...
参考：[utils.py:642-659](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/utils.py#L642-L659)
...
参考：[utils.py:484-526](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/utils.py#L484-L526)
...
参考：[dependencies.py:216-289](https://github.com/virgiliojr94/book-to-skill/blob/main/book_to_skill/dependencies.py#L216-L289)

---

**事实来源**：本章节基于以下事实编号 F-039, F-040, F-041

- [上一章：安装与使用](06-installation-usage.md)
- [下一章：可复用工程模式](08-transferable-patterns.md) →
