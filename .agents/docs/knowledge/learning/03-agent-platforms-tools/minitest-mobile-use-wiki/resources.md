---
title: "资源链接"
category: "learning"
source: "https://www.minitap.ai/docs"
x-toml-ref: "../../../../../../.meta/toml/.agents/docs/knowledge/learning/03-agent-platforms-tools/minitest-mobile-use-wiki/resources.toml"
date: "2026-07-07"
tags: ["resources", "links", "minitest", "mobile-use", "资源", "链接"]
summary: "汇总minitest和mobile-use SDK的官方资源链接，包括文档、GitHub、社区、博客、学术论文等，以及项目内相关Wiki交叉引用。"
---
# 资源链接

> **来源**: https://www.minitap.ai/docs

本页面汇总了minitest和mobile-use SDK相关的所有官方资源、社区链接和项目内交叉引用。

---

## 第一部分：官方资源

### Minitap 官方网站

| 资源 | 链接 | 说明 |
|---|---|---|
| **官网首页** | [https://www.minitap.ai/](https://www.minitap.ai/) | Minitap公司官网，产品介绍、客户案例、定价等 |
| **官方文档** | [https://www.minitap.ai/docs](https://www.minitap.ai/docs) | minitest和mobile-use SDK的完整官方文档（本教程来源） |
| **Benchmark页面** | [https://www.minitap.ai/benchmark](https://www.minitap.ai/benchmark) | AndroidWorld基准测试成绩、技术对比 |
| **Demo页面** | [https://www.minitap.ai/demo](https://www.minitap.ai/demo) | 产品演示 |
| **仪表板（Dashboard）** | [https://app.minitap.ai/](https://app.minitap.ai/) | minitest Web应用入口（需登录） |
| **预约演示** | [cal.com/team/minitap/minitap-get-access](https://cal.com/team/minitap/minitap-get-access?overlayCalendar=true) | 预约产品演示和试用 |

### 官方文档快速链接

#### minitest 文档

| 章节 | 链接 |
|---|---|
| 入门指南 | [https://www.minitap.ai/docs/minitest/get-started](https://www.minitap.ai/docs/minitest/get-started) |
| 什么是minitest | [https://www.minitap.ai/docs/minitest/get-started/what-is-minitest](https://www.minitap.ai/docs/minitest/get-started/what-is-minitest) |
| 认识Mini | [https://www.minitap.ai/docs/minitest/get-started/meet-mini](https://www.minitap.ai/docs/minitest/get-started/meet-mini) |
| 快速开始 | [https://www.minitap.ai/docs/minitest/get-started/quickstart](https://www.minitap.ai/docs/minitest/get-started/quickstart) |
| 用户故事解析 | [https://www.minitap.ai/docs/minitest/suite/anatomy-of-a-story](https://www.minitap.ai/docs/minitest/suite/anatomy-of-a-story) |
| 编写用户故事 | [https://www.minitap.ai/docs/minitest/suite/authoring-stories](https://www.minitap.ai/docs/minitest/suite/authoring-stories) |
| Mini维护套件 | [https://www.minitap.ai/docs/minitest/suite/mini-maintains-your-suite](https://www.minitap.ai/docs/minitest/suite/mini-maintains-your-suite) |
| 提供构建 | [https://www.minitap.ai/docs/minitest/runs/builds](https://www.minitap.ai/docs/minitest/runs/builds) |
| 触发运行 | [https://www.minitap.ai/docs/minitest/runs/triggering-a-run](https://www.minitap.ai/docs/minitest/runs/triggering-a-run) |
| 运行报告 | [https://www.minitap.ai/docs/minitest/runs/run-report](https://www.minitap.ai/docs/minitest/runs/run-report) |
| 问题分类 | [https://www.minitap.ai/docs/minitest/triage/triaging-issues](https://www.minitap.ai/docs/minitest/triage/triaging-issues) |
| Mini建议 | [https://www.minitap.ai/docs/minitest/triage/suggestions](https://www.minitap.ai/docs/minitest/triage/suggestions) |
| Cursor/Claude集成 | [https://www.minitap.ai/docs/minitest/integrations/cursor-and-claude](https://www.minitap.ai/docs/minitest/integrations/cursor-and-claude) |
| GitHub集成 | [https://www.minitap.ai/docs/minitest/integrations/github](https://www.minitap.ai/docs/minitest/integrations/github) |
| Slack集成 | [https://www.minitap.ai/docs/minitest/integrations/mini-in-slack](https://www.minitap.ai/docs/minitest/integrations/mini-in-slack) |
| 能力范围 | [https://www.minitap.ai/docs/minitest/reference/capabilities](https://www.minitap.ai/docs/minitest/reference/capabilities) |
| CLI命令 | [https://www.minitap.ai/docs/minitest/reference/cli-commands](https://www.minitap.ai/docs/minitest/reference/cli-commands) |
| 术语表 | [https://www.minitap.ai/docs/minitest/reference/glossary](https://www.minitap.ai/docs/minitest/reference/glossary) |
| MCP工具 | [https://www.minitap.ai/docs/minitest/reference/mcp-tools](https://www.minitap.ai/docs/minitest/reference/mcp-tools) |
| Mini命令 | [https://www.minitap.ai/docs/minitest/reference/mini-commands](https://www.minitap.ai/docs/minitest/reference/mini-commands) |
| GitHub Action | [https://www.minitap.ai/docs/minitest/reference/github-action](https://www.minitap.ai/docs/minitest/reference/github-action) |

#### mobile-use SDK 文档

| 章节 | 链接 |
|---|---|
| SDK介绍 | [https://www.minitap.ai/docs/mobile-use-sdk/introduction](https://www.minitap.ai/docs/mobile-use-sdk/introduction) |
| 安装指南 | [https://www.minitap.ai/docs/mobile-use-sdk/installation](https://www.minitap.ai/docs/mobile-use-sdk/installation) |
| 本地快速开始 | [https://www.minitap.ai/docs/mobile-use-sdk/local-quickstart](https://www.minitap.ai/docs/mobile-use-sdk/local-quickstart) |
| 平台快速开始 | [https://www.minitap.ai/docs/mobile-use-sdk/platform-quickstart](https://www.minitap.ai/docs/mobile-use-sdk/platform-quickstart) |
| 云设备快速开始 | [https://www.minitap.ai/docs/mobile-use-sdk/cloud-quickstart](https://www.minitap.ai/docs/mobile-use-sdk/cloud-quickstart) |
| BrowserStack快速开始 | [https://www.minitap.ai/docs/mobile-use-sdk/browserstack-quickstart](https://www.minitap.ai/docs/mobile-use-sdk/browserstack-quickstart) |
| iOS真机设置 | [https://www.minitap.ai/docs/mobile-use-sdk/physical-ios-setup](https://www.minitap.ai/docs/mobile-use-sdk/physical-ios-setup) |
| 架构概览 | [https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/overview](https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/overview) |
| Agent核心类 | [https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/agent](https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/agent) |
| Builder模式 | [https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/builder-pattern](https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/builder-pattern) |
| 可观测性 | [https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/observability](https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/observability) |
| Agent配置 | [https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/agent-profiles](https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/agent-profiles) |
| 任务定义 | [https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/tasks](https://www.minitap.ai/docs/mobile-use-sdk/core-concepts/tasks) |
| 示例代码 | [https://www.minitap.ai/docs/mobile-use-sdk/examples](https://www.minitap.ai/docs/mobile-use-sdk/examples) |
| SDK参考 | [https://www.minitap.ai/docs/mobile-use-sdk/sdk-reference](https://www.minitap.ai/docs/mobile-use-sdk/sdk-reference) |
| 故障排除 | [https://www.minitap.ai/docs/mobile-use-sdk/troubleshooting](https://www.minitap.ai/docs/mobile-use-sdk/troubleshooting) |
| 反馈指南 | [https://www.minitap.ai/docs/mobile-use-sdk/feedback](https://www.minitap.ai/docs/mobile-use-sdk/feedback) |

---

## 第二部分：开源与代码资源

### GitHub 仓库

| 资源 | 链接 | 说明 |
|---|---|---|
| **mobile-use 主仓库** | [github.com/minitap-ai/mobile-use](https://github.com/minitap-ai/mobile-use) | mobile-use SDK开源代码（2.5k+星标） |
| **AndroidWorld基准** | [github.com/google-research/android_world](https://github.com/google-research/android_world) | Google DeepMind的Android基准测试环境，minitest在此达到100%成功率 |

### PyPI 包

| 资源 | 链接 | 说明 |
|---|---|---|
| **minitap-mobile-use** | [pypi.org/project/minitap-mobile-use](https://pypi.org/project/minitap-mobile-use/) | mobile-use SDK Python包 |

安装命令：
```shellscript
pip install minitap-mobile-use
# 或使用uv（推荐）
uv add minitap-mobile-use
```

---

## 第三部分：学术资源

### 论文

| 资源 | 链接 | 说明 |
|---|---|---|
| **mobile-use: Open-Source Mobile Agent SDK** | [arxiv.org/abs/2602.07787](https://arxiv.org/abs/2602.07787) | mobile-use SDK的arXiv学术论文 |

---

## 第四部分：博客与媒体

### 官方博客

| 发布日期 | 标题 | 阅读时长 | 链接 |
|---|---|---|---|
| 2026-05-05 | Announcing minitest: the agent owns the suite now | 12分钟 | [链接](https://www.minitap.ai/blog/announcing-minitest-the-agent-owns-the-suite-now) |
| 2026-05-17 | When our QA agent made its own account | 8分钟 | [链接](https://www.minitap.ai/blog/when-our-qa-agent-made-its-own-account) |
| 2025-12-01 | Raise announcement | - | [链接](https://www.minitap.ai/blog/raise-announcement) |

博客首页：[https://www.minitap.ai/blog](https://www.minitap.ai/blog)

### 媒体报道

| 媒体 | 标题 | 日期 | 链接 |
|---|---|---|---|
| **Forbes** | Startup Minitap tops DeepMind's mobile AI benchmark, raises $4.1 million seed round | 2025-12-01 | [链接](https://www.forbes.com/sites/charliefink/2025/12/01/startup-minitap-tops-deepminds-mobile-ai-benchmark-raises-41-million-seed-round/) |

---

## 第五部分：社区与支持

### 社区渠道

| 资源 | 链接 | 说明 |
|---|---|---|
| **Discord社区** | （请查看官网获取最新链接） | 用户社区、技术讨论、获取帮助 |
| **GitHub Issues** | [github.com/minitap-ai/mobile-use/issues](https://github.com/minitap-ai/mobile-use/issues) | 报告Bug、请求功能 |
| **Twitter/X** | [@minitap_ai](https://x.com/minitap_ai) | 官方动态、产品更新 |

### 技术支持

| 联系方式 | 用途 |
|---|---|
| **邮件支持** | [support@minitap.ai](mailto:support@minitap.ai) | 私人咨询、合作机会 |
| **GitHub Issues** | Bug报告、功能请求（推荐） |
| **Discord** | 社区互助、快速问答 |

---

## 第六部分：项目内相关Wiki交叉引用

本教程与项目内其他Wiki文档形成完整知识体系：

### Minitap 系列文档

| 文档 | 内容说明 | 链接 |
|---|---|---|
| Minitap.ai官方Wiki完整学习教程 | 产品深度解析、AndroidWorld基准、客户案例、融资报道、竞争对手分析 | [minitap-official-wiki.md](../minitap-official-wiki.md) |
| mobile-use深度学习分析 | SDK技术架构、核心模块、实现细节深度解析 | [mobile-use-deep-learning-analysis.md](../mobile-use-deep-learning-analysis.md) |
| **本文档：Minitest & Mobile Use SDK官方文档教程** | 官方文档系统化中文翻译与学习指南（当前所在） | [minitest-mobile-use-official-docs-wiki.md](../minitest-mobile-use-official-docs-wiki.md) |

### 架构模式参考

| 文档 | 内容说明 | 链接 |
|---|---|---|
| 多代理闭环执行架构 | minitest多代理协作的闭环执行机制、任务规划与验证流程的架构模式复用参考 | [multi-agent-closed-loop-execution.md](../../../../retrospective/patterns/architecture-patterns/multi-agent-closed-loop-execution.md) |
| 规范化坐标抽象 | mobile-use中跨平台坐标系统一抽象技术细节 | [normalized-coordinate-abstraction.md](../../../../retrospective/patterns/architecture-patterns/normalized-coordinate-abstraction.md) |

### 复盘文档

| 文档 | 内容说明 | 链接 |
|---|---|---|
| mobile-use深度学习复盘 | mobile-use技术研究过程的完整复盘、关键洞察与经验总结 | [mobile-use-deep-learning-analysis/](../../../../../../.trae/specs/retrospectives-insights/mobile-use-deep-learning-analysis/spec.md) |

---

## 第七部分：相关技术资源

### 依赖技术

| 技术 | 链接 | 说明 |
|---|---|---|
| **Python** | [python.org](https://www.python.org/) | mobile-use SDK要求Python 3.12+ |
| **uv** | [docs.astral.sh/uv](https://docs.astral.sh/uv/) | 快速Python包管理器，推荐用于安装SDK |
| **LangGraph** | [github.com/langchain-ai/langgraph](https://github.com/langchain-ai/langgraph) | mobile-use使用的Agent框架 |
| **Pydantic** | [docs.pydantic.dev](https://docs.pydantic.dev/) | 结构化输出数据验证库 |
| **ADB (Android Debug Bridge)** | [developer.android.com/tools/adb](https://developer.android.com/tools/adb) | Android平台调试工具 |
| **IDB (iOS Development Bridge)** | [fbidb.io](https://fbidb.io/) | iOS平台调试工具 |

### 测试工具对比参考

| 工具 | 类型 | 链接 |
|---|---|---|
| **Appium** | 传统移动自动化框架 | [appium.io](https://appium.io/) |
| **Maestro** | 移动端UI测试框架 | [maestro.mobile.dev](https://maestro.mobile.dev/) |
| **Playwright** | Web自动化框架（也支持移动） | [playwright.dev](https://playwright.dev/) |
| **Cypress** | Web E2E测试框架 | [cypress.io](https://www.cypress.io/) |

### LLM提供商

| 提供商 | 说明 | 链接 |
|---|---|---|
| **OpenAI** | GPT-5、o4-mini等模型 | [openai.com](https://openai.com/) |
| **Google Gemini** | Gemini 2.5 Flash/Pro等模型 | [ai.google.dev](https://ai.google.dev/) |
| **xAI** | Grok模型 | [x.ai](https://x.ai/) |
| **OpenRouter** | 多模型聚合平台 | [openrouter.ai](https://openrouter.ai/) |

---

## 附录：相关基准测试

| 基准 | 链接 | 说明 |
|---|---|---|
| **AndroidWorld** | [github.com/google-research/android_world](https://github.com/google-research/android_world) | Google DeepMind的Android代理基准，minitest在此达到100%任务成功率 |

AndroidWorld覆盖的20个真实应用类型：
- Calendar（日历）
- Notes（笔记）
- Maps（地图）
- VLC（播放器）
- Messages（短信）
- Settings（设置）
- Web Browser（浏览器）
- Audio Recorder（录音）
- Camera（相机）
- Clock（时钟）
- Contacts（联系人）
- Expense Tracker（费用追踪）
- 以及其他8个应用

---

## 文档导航

- [返回教程首页](../minitest-mobile-use-official-docs-wiki.md)
- [常见问题解答](faq.md)
- [最佳实践](best-practices.md)
- [综合术语表](glossary.md)
