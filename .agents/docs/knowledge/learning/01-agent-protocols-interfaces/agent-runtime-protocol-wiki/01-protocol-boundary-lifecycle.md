---
id: "agent-runtime-protocol-wiki-01"
title: "Protocol 边界与最小生命周期"
source: "微信公众号文章《Agent Protocol：一个生产级的 Agent Runtime 应该包含什么？》（阿里云开发者）https://mp.weixin.qq.com/s/0N-RnpGVy_PLSDHMwAIFNg"
x-toml-ref: "../../../../../../.meta/toml/.agents/docs/knowledge/learning/01-agent-protocols-interfaces/agent-runtime-protocol-wiki/01-protocol-boundary-lifecycle.toml"
---

# 01 Protocol 边界与最小生命周期
...
## Runtime Protocol：外部世界如何理解一个 Agent
...
| 生命周期阶段 | 主要协议对象 | 对应章节 |
|------------|------------|---------|
| **创建任务** | Agent / Thread / Run | 执行模型、Runtime Loop（[第02章](02-execution-model.md)） |
| **携带上下文** | Thread / Message / Workspace | 状态管理、Workspace/Sandbox（[第03章](03-state-management.md)） |
| **执行步骤** | Step / Tool Call / Subagent task | 执行模型、工具协议、多Agent协作（[第02](02-execution-model.md)、[05](05-tools-streaming.md)、[06章](06-multi-agent.md)） |
| **观察事件** | Event / Trace / State Snapshot | 流式输出、可观测性（[第05](05-tools-streaming.md)、[07章](07-observability-evaluation.md)） |
| **中断恢复** | Checkpoint / Interrupt / Resume | 状态管理、中断恢复、错误恢复（[第03](03-state-management.md)、[04章](04-interrupt-error-recovery.md)） |
| **产生产物** | Artifact / Workspace file | 状态管理、流式输出、Harness（[第03](03-state-management.md)、[05章](05-tools-streaming.md)） |
| **评测审计** | Step / Event / Artifact / Trace | 可观测性与可评测性（[第07章](07-observability-evaluation.md)） |
...
- [下一章：执行模型](02-execution-model.md) →
