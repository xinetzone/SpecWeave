...
> ║  🚨 CRITICAL: 关于备份的最重要警告                          ║
> ║                                                              ║
> ║  ⚠️  网盘中的 backups/ 目录不是真正的备份！                  ║
> ║                                                              ║
> ║  backups/ 目录和 repos/ 裸仓库在同一个百度网盘同步空间中，  ║
> ║  它们共享同一个故障域：                                       ║
> ║  • 百度网盘账号被封 → 裸仓库和备份同时丢失                   ║
> ║  • 同步机制导致文件损坏 → bundle 文件也可能损坏               ║
> ║  • 误删整个同步目录 → 主数据和备份一起删除                   ║
> ║                                                              ║
> ║  ✅ 真正的备份必须满足：                                     ║
> ║  1. 存储在百度网盘同步空间之外（本地其他目录、移动硬盘、    ║
> ║     其他云存储）                                             ║
> ║  2. 不会被自动同步/覆盖                                      ║
> ║  3. 定期验证可用性（每季度至少从备份恢复一次测试）           ║
> ║                                                              ║
> ║  ❌ 禁用自动 gc：执行 setup-git-config 后会自动设置          ║
> ║     gc.auto=0，因为 gc 过程中如果网盘同步会导致灾难性后果。  ║
> ║     手动 gc 前必须停止网盘同步，gc 后验证 fsck。             ║
> ╚══════════════════════════════════════════════════════════════╝
> ```
