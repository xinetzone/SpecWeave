...
> ⚠️ **健康检查不是可选操作，是必须执行的维护流程**

| 检查类型 | 频率 | 不执行的风险 |
|---------|------|-------------|
| `git-doctor`（quick 模式） | 每次跨设备切换工作前 | 拉取到半同步/损坏状态的仓库 |
| `git-doctor full`（含 fsck） | **每周至少一次** | 静默损坏潜伏数周后才发现，备份链可能已全部被污染 |
| 冷备份验证 | 每季度一次 | 以为有备份，灾难发生时发现备份也损坏了 |
| force-unlock 审计日志检查 | 每月一次 | 发现滥用 force-unlock 的危险行为 |

**注意**：
- 静默损坏可能在仓库中潜伏数周而不表现症状，等到发现时最近的备份可能也包含损坏
- 每周一次 full 检查是"最低频率"，不是"建议频率"
- git-sync-pull 每次拉取后会执行轻量级 fsck（仅检查 error），但这不能替代每周 full 检查
