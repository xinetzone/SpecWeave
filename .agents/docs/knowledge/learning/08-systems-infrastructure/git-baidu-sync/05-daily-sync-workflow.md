...
### 1.4 电源与睡眠管理（笔记本用户必读）
- **合盖/睡眠前必须确认同步完成**：等待脚本返回命令行提示符，确认输出"推送成功"或"同步完成"
- **执行同步时设置电源不睡眠**：Windows 电源选项临时设置为"从不睡眠"，macOS 可使用 `caffeinate` 命令
- **唤醒后先诊断再操作**：系统从睡眠唤醒后，先执行 `git-diag` 检查状态，不要立即 push/pull
- **原因**：睡眠会挂起脚本进程，锁文件可能残留但未同步到云端，裸仓库可能处于半写入状态

### 1.5 百度网盘客户端注意事项
- **客户端更新后首先检查同步目录配置**：自动更新可能重置同步目录列表，确认 git-sync 目录仍在同步列表中
- **不要暂停同步后忘记恢复**：暂停同步后所有 push 都只是本地写入，其他设备看不到
- **传输列表定期检查**：每周查看一次百度网盘"传输"页面，没有失败/卡住的任务
- **客户端版本更新后**：如果出现异常，尝试暂停-恢复同步或重启客户端
