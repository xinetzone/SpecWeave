...
> 正向饱和→1.0是ULP舍入效应（1-sigmoid(x) < ULP(1)/2时发生），阈值在x≈17；
> 负向饱和→0.0需要exp(-x)溢出为inf（即sigmoid(x) = 1/(1+inf) = 0），阈值在x < -89。
> 在-89 < x < -17区间，sigmoid(x)是亚正规数（subnormal）但不为精确零。

| 激活函数 | 饱和值 | 精确饱和输入阈值（float32） |
|----------|--------|---------------------------|
| sigmoid(x) | 1.0 | x ≥ 17（sigmoid(16.6)≈0.99999994，sigmoid(17)精确=1.0） |
| sigmoid(x) | 0.0（精确） | x ≤ -89（exp(89)溢出为inf，1/inf=0） |
| sigmoid(x) | < 1e-37（近似零） | x ≤ -85（亚正规数区间，非精确零） |
| tanh(x) | ±1.0 | \|x\| ≥ 10（tanh(9.9)≈0.99999994，tanh(10)精确=±1.0） |
...
**✅ 正确**的饱和区断言方式：
```python
# 方式1：精确相等（推荐，语义最清晰）——适用于x≥17或x≤-89的极端输入
assert sigmoid(80) == 1.0      # x=80≥17，正向ULP饱和
assert sigmoid(-100) == 0.0    # x=-100≤-89，exp溢出为inf，精确为0
assert tanh(100) == 1.0        # tanh对称饱和
assert tanh(-100) == -1.0
